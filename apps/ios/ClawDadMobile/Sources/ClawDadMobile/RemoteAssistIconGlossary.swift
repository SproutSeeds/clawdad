    ...
    ScrollView {
      LazyVStack(alignment: .leading, spacing: 20) {
        ...
        section("Assistant call bar") {
          entry("rectangle.3.group", "Main Workspace", "Save multiple named Terminal setups. Save captures exactly one window; Update replaces that snapshot’s lineup and retains a previous version. Restore reopens exact conversations with their latest history, ready for your next instruction.", "Shell only means a saved directory without an agent conversation. Review identities flags legacy or unavailable bindings. Restored and already open mean verified tabs; waiting explains missing drives/history or ownership. Restore never sends continue, replays queues or enables supervisors.")
          entry("xmark.rectangle", "Close Terminal window", "Swipe left on a Terminal window row, use its context menu or accessibility action, or open Close chosen window in Main Workspace. Inspect and confirm the exact tab count, running agents and unsent drafts.", "Save snapshot and close proceeds only after identities and drafts verify. Closing stops running work; reopening restores conversation history, not an in-memory computation. Cancel leaves the window open. An uncertain receipt requires review before another close.")
          ...
          entry("phone.down.fill", "End voice conversation", "Ends the call and microphone capture. Messages accepted by the Mac keep working while ClawDad is running there. Visible unsent words are saved for review.", "Red handset; available while a call is connecting or connected. To stop an accepted Assistant response, use Cancel response on its request card. Already accepted project work continues separately.")
        }
        ...
        section("Remote Assist: workspace") {
          entry("flask", "Research autonomy", "Opens per-thread research authorization, pause/off controls, steering, weekly allowance settings and decision history in Assistant Workspace.", "Off until explicitly enabled for an exact agent and objective. There is no automatic app-wide allowance reserve. A stopping percentage applies only when explicitly configured for this supervisor and account, through the current weekly cycle. 0% allows using the remaining allowance until exhausted. Resets never clear a pause; running tasks stay intact. The main Assistant can also manage these settings on your explicit instruction.")
