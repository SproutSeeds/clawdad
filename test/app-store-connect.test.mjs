...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 74.",
    ...
    "Assigned build 74 to ClawDad Internal.",
    "Added TestFlight test instructions to build 74.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 74 to ClawDad Founding Customers.",
    "Submitted build 74 for Beta App Review.",
  ]);
