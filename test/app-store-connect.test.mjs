...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 35.",
    ...
    "Assigned build 35 to ClawDad Internal.",
    "Added TestFlight test instructions to build 35.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 35 to ClawDad Founding Customers.",
    "Submitted build 35 for Beta App Review.",
  ]);
