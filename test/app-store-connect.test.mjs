...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 69.",
    ...
    "Assigned build 69 to ClawDad Internal.",
    "Added TestFlight test instructions to build 69.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 69 to ClawDad Founding Customers.",
    "Submitted build 69 for Beta App Review.",
  ]);
