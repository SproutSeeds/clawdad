...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 48.",
    ...
    "Assigned build 48 to ClawDad Internal.",
    "Added TestFlight test instructions to build 48.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 48 to ClawDad Founding Customers.",
    "Submitted build 48 for Beta App Review.",
  ]);
