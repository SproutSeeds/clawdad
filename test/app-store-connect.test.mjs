...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 38.",
    ...
    "Assigned build 38 to ClawDad Internal.",
    "Added TestFlight test instructions to build 38.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 38 to ClawDad Founding Customers.",
    "Submitted build 38 for Beta App Review.",
  ]);
