...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 70.",
    ...
    "Assigned build 70 to ClawDad Internal.",
    "Added TestFlight test instructions to build 70.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 70 to ClawDad Founding Customers.",
    "Submitted build 70 for Beta App Review.",
  ]);
