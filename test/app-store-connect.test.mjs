...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 81.",
    ...
    "Assigned build 81 to ClawDad Internal.",
    "Added TestFlight test instructions to build 81.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 81 to ClawDad Founding Customers.",
    "Submitted build 81 for Beta App Review.",
  ]);
