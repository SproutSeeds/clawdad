...
test("app release configuration is complete, scoped, and idempotent", async () => {
  ...
  assert.deepEqual(first.actions, [
    ...
    "Recorded export-compliance exemption for build 83.",
    ...
    "Assigned build 83 to ClawDad Internal.",
    "Added TestFlight test instructions to build 83.",
  ]);
  assert.equal(
    client.calls.some(
...
test("external beta submission requires certification and is retry-safe", async () => {
  ...
  assert.deepEqual(first.actions, [
    "Assigned build 83 to ClawDad Founding Customers.",
    "Submitted build 83 for Beta App Review.",
  ]);
