  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--model": {
        ...
        options[arg === "--model" ? "model" : "chimeraBinary"] = value;
        index += 1;
        break;
      }
      default:
...
function ollamaApiBase() {
  const configured = process.env.OLLAMA_BASE_URL || "http://localhost:11434/v1";
...
async function readOllamaModels() {
  const url = `${ollamaApiBase()}/api/tags`;
  try {
...
async function diagnose(options) {
  ...
  const ollama = await readOllamaModels();
  checks.push({
    label: "Ollama",
  ...
  if (!ollama.ok) {
    suggestions.push("Start Ollama before using Chimera local models.");
  }
