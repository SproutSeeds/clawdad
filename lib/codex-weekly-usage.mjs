...
export async function readCodexAccountUsage({binary = process.env.CLAWDAD_CODEX || 'codex', launch = spawn, timeoutMs = 15_000} = {}) {
  const child = launch(binary, ['app-server'], {cwd: os.homedir(), stdio: ['pipe', 'pipe', 'ignore']});
  ...
  try {
    ...
    return normalizeWeeklyUsage(result);
  } finally { clearTimeout(timer); lines.close(); child.stdin.end(); child.kill(); }
}

export function normalizeWeeklyUsage(value) {
  ...
  return {accountKey: hash('codex-account:' + value.accountId), remainingPercent: 100 - usedPercent, resetsAt};
}
...
export class CodexWeeklyUsageMonitor {
  ...
  async sample() {
    ...
    try {
      ...
      if (previous && reading.resetsAt < previous.resetsAt) throw new Error('Codex returned an older allowance cycle. Checking again.');
