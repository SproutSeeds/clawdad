...
export class CodexAccountSharedRPC {
  ...
  async account(client){
    ...
    for(let attempt=0;attempt<3;attempt++){
      ...
      catch(error){if(attempt===2)throw fail('shared_account_read_unavailable','The subscription identity service is temporarily unavailable. Preserve the switch receipt and retry its account check.');
  ...
  async observe({operationId,source,target,capture=false}={}){
    ...
    const client=await this.connection(owner),account=await this.account(client),ids=await rpcPages(client,'thread/loaded/list',{limit:100});
    ...
    return {...owner,kind:'server',socketPath:this.socketPath,accountKey:account.accountKey,accountVerified:true,
