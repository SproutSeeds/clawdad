...
export class CodexAccountSharedRPC {
  ...
  async account(client){
    ...
    for(let attempt=0;attempt<3;attempt++){
      try{limits=await client.request('account/rateLimits/read');break;}
          ...
          throw fail('shared_authentication_expired','The shared server holds an expired subscription login.');
  ...
  async thread(client,id,{capture=false,operationId,owner}={}){
    ...
    const turns=await rpcPages(client,'thread/turns/list',{threadId:id,limit:100,itemsView:'full'});
  ...
  async observe({operationId,source,target,capture=false}={}){
    ...
    let account,emptySourceAccountUnavailable=false;
    try{account=await this.account(client);}catch(error){
      const originalSource=source?.emptySourceAccountUnavailable===true&&source.pid===owner.pid&&source.processIdentity===owner.processIdentity;
      if(error.code!=='shared_authentication_expired'||!capture&&!originalSource||ids.length||client.serverRequests?.size
        ||!await this.processes.dispatchHeld(operationId))throw error;
      await this.permit({operationId,effect:'inspect_empty_expired_server',owner});
      emptySourceAccountUnavailable=true;
    }
    const threads=[];
    for(const id of ids){
  ...
  async reconcile({requestId}){const {value}=await this.receipt(requestId);return {state:value?.state||'not_dispatched',durable:true};}
