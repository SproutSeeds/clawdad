...
export function assistantPresentation(state, catalog, diagnostics = {}) {
  ...
  const tasks = state.jobs.filter(job => !hiddenJobs.has(job.id) && !resolved.has(job.id) && !taskParent(job)
    && (taskActions.has(job.action) || (job.status === 'attention' && ['message','computer.open','computer.input','computer.clear','computer.replace','computer.shortcut','terminal.new','terminal.key','terminal.images','terminal.close','terminal.close.resolve','files.publish','files.update','remote.clipboard'].includes(job.action))))
    .slice(-20).map(({fingerprint,result,...job}) => {
      ...
      return {...job,args,displayName:job.action === 'message' ? 'Assistant' : job.action.startsWith('computer.') ? 'Mac' : assistantTaskName(title),
        ...
        requestText:job.action === 'terminal.clear' ? 'Clear the inspected draft input' : args.text || owners.get(job.parentRequestId)?.args?.text
