...
const taskActions = new Set(['terminal.send','terminal.queue','terminal.insert','terminal.clear','terminal.replace','terminal.append','terminal.native.type','appserver.send','appserver.queue','appserver.draft','appserver.clear','appserver.create','appserver.resume','appserver.restore']);
  ...
  const tasks = state.jobs.filter(job => !hiddenJobs.has(job.id) && !resolved.has(job.id) && !taskParent(job)
    ...
    .slice(-20).map(({fingerprint,result,...job}) => {
      ...
      return {...job,args,displayName:job.action === 'message' ? 'Assistant' : job.action.startsWith('computer.') ? 'Mac' : assistantTaskName(title),
        ...
        requestText:job.action === 'terminal.clear' ? 'Clear the inspected draft input' : job.acceptedText || args.text || owners.get(job.parentRequestId)?.args?.text
