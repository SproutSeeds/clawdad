...
const turnControls = new Set(['appserver.steer','appserver.interrupt']);
const taskActions = new Set(['terminal.send','terminal.queue','terminal.insert','terminal.clear','terminal.replace','terminal.append','terminal.native.type','appserver.send','appserver.queue','appserver.draft','appserver.clear','appserver.create','appserver.resume','appserver.restore',...turnControls]);
const taskParent = job => job.originalDraftRequestId || job.parentTaskId || (job.source === 'task-update' && job.id.startsWith('update:') ? job.id.slice(7) : null);
...
export function assistantPresentation(state, catalog, diagnostics = {}) {
  ...
  const tasks = state.jobs.filter(job => !hiddenJobs.has(job.id) && !resolved.has(job.id) && !taskParent(job)
    && (taskActions.has(job.action) || (job.action==='message'&&['queued','running','interrupted','cancelled'].includes(job.status)) || (job.action==='terminal.key' && job.acceptedText && job.turnId) || (job.status === 'attention' && ['message','computer.open','computer.input','computer.clear','computer.replace','computer.shortcut','terminal.new','terminal.key','terminal.images','terminal.close','terminal.close.resolve','files.publish','files.update','remote.clipboard'].includes(job.action))))
    .slice(-20).map(({fingerprint,result,...job}) => {
      const turnControl=turnControls.has(job.action);
      // A readable receipt must survive a missing/cancelled Assistant reply.
      // Keep raw owner identities and authorization records in diagnostics.
      if(turnControl)delete job.controlReceipt;
      const tabId = job.args.tabId;
      const known = tabId && state.jobs.findLast(j => j.args?.tabId === tabId && j.tabTitle && !j.tabTitle.includes(' — '));
      const title = job.action.startsWith('appserver.') ? job.threadName || 'ClawDad' : job.tabTitle || catalog?.tabs?.find(t => t.id === tabId)?.title || known?.tabTitle;
      ...
      return {...job,args,displayName:job.action === 'message' ? 'Assistant' : job.action.startsWith('computer.') ? 'Mac' : assistantTaskName(title),
        error:assistantReadableError(job.error), response:turnControl
          ? `${result?.summary||'Control outcome needs inspection; read the original receipt before taking another action.'} Turn: ${job.turnId}. Receipt: ${job.id}.`
          : update?.response || job.response,
        requestText:turnControl ? (job.action==='appserver.steer' ? args.text : 'Stop the inspected ClawDad turn')
          : job.action === 'terminal.clear' ? 'Clear the inspected draft input' : job.acceptedText || args.text || owners.get(job.parentRequestId)?.args?.text
          || (job.action.startsWith('terminal.close') ? 'Close the requested Terminal tab' : 'Update the inspected Mac input')};
    });
