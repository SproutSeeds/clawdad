...
export class AssistantAppServer {
  ...
  async inspect(threadId){
    ...
    return {thread:{...thread,cwd:row?.cwd||thread.cwd},owner,queue,draft,targetToken,expiresInSeconds:45,
      capabilities:{read:true,draft:true,resume:owner.kind==='saved',send:['saved','app_server'].includes(owner.kind)&&Array.isArray(queue),queue:['saved','app_server'].includes(owner.kind)&&Array.isArray(queue)}};
  }
  async history(args){
  ...
  async control(action,args,requestId,meta={}){
    ...
    return this.transaction(async()=>{
      ...
      if(!['appserver.create','appserver.resume','appserver.send','appserver.queue', 'appserver.reconcile',...['appserver.draft','appserver.clear']].includes(action))throw Error('Unsupported app-server action.');
