...
import {CodexSharedClient,rpcPages,codexProcessOwners,classifyThreadOwner,assertSharedThreadOwner,acquireThreadControl,validThreadId,readSharedControlGeneration} from './codex-thread-control.mjs';
import {AssistantTurnControl,isTurnControl,observeTurnControl} from './assistant-turn-control.mjs';
...
import {agentPermissionParams} from './agent-access-settings.mjs';
...
export class AssistantAppServer {
  constructor({root,client=new CodexSharedClient(),workspaces=async()=>({roots:[]}),readIndex=readCodexThreadIndex,
    readOwners=codexProcessOwners,readControlGeneration=readSharedControlGeneration,lease=acquireThreadControl,deliveryLease=(cwd,id,requestId)=>acquireCodexDeliveryClaim(cwd,{threadId:id,requestId,timeoutMs:100,pollMs:25}),mayDispatch=()=>true,onJob=async()=>{},onCreated=async()=>{},now=Date.now}={}){
    Object.assign(this,{root,client,workspaces,readIndex,readOwners,readControlGeneration,lease,deliveryLease,mayDispatch,onJob,onCreated,now});
    this.turnControls=new AssistantTurnControl(this);
  ...
  async loadState(){
    ...
    for(const j of this.state.jobs){if(isTurnControl(j.action))this.turnControls.recover(j);
      else if(j.status==='sending'){j.status='attention';j.uncertain=true;j.error='The Mac restarted before delivery was confirmed. Reconcile the original request; it will not be repeated.';}}
  }
  async save(){await researchSave(path.join(this.root,'state.json'),this.state);
  ...
  async inspect(threadId){
    ...
    const [read,owners,loaded,index]=await Promise.all([this.client.request('thread/read',{threadId,includeTurns:true})
      .catch(async controlReadError=>{try{return {...await this.client.request('thread/read',{threadId,includeTurns:false}),controlReadError};}catch(error){return {error};}}),
      this.readOwners({socketPath:this.client.socketPath}),rpcPages(this.client,'thread/loaded/list',{limit:100}),this.readIndex().catch(()=>[])]);
    ...
    if(read.error){
      ...
      return {thread:{id:threadId,name:row.name||row.title,cwd:row.cwd,archived:!!row.archived},owner,draft,
        ...
        activeTurn:null,canSteer:false,canInterrupt:false,capabilities:{read:false,draft:true,restore:false,resume:false,send:false,queue:false,steer:false,interrupt:false}};
    }
    const {thread}=read;
    ...
    const {binding,...turnControl}=await this.turnControls.inspect(thread,owner);
    if(read.controlReadError){turnControl.canSteer=false;turnControl.canInterrupt=false;turnControl.controlUnavailable='Exact active-turn history could not be read: '+read.controlReadError.message;}
    if(archived){turnControl.canSteer=false;turnControl.canInterrupt=false;}
    const targetToken=uuid();this.inspections.set(targetToken,{threadId,owner:fingerprint(owner),at:this.now(),control:archived||read.controlReadError?null:binding});
    for(const [key,value] of this.inspections)if(this.now()-value.at>45000)this.inspections.delete(key);
    return {thread:{...thread,turns:[],cwd:thread.cwd||row?.cwd,archived},owner,queue,draft,targetToken,expiresInSeconds:45,...turnControl,
      ...
      capabilities:{read:true,draft:true,steer:turnControl.canSteer,interrupt:turnControl.canInterrupt,restore:archived&&owner.kind==='saved',resume:!archived&&owner.kind==='saved',send:!archived&&['saved','app_server'].includes(owner.kind)&&Array.isArray(queue),queue:!archived&&['saved','app_server'].includes(owner.kind)&&Array.isArray(queue)}};
  }
  ...
  async control(action,args,requestId,meta={}){
    ...
    return this.transaction(async()=>{
      if(isTurnControl(action))return this.turnControls.control(action,args,requestId,meta);
      ...
      if(!editable&&!this.accountControls?.withWorkAdmission)await this.accountControls?.assertAdmission();
      ...
      const persist=async admission=>{Object.assign(job,admission);this.state.jobs.push(job);await this.save();return job;};
      if(this.accountControls?.withWorkAdmission)await this.accountControls.withWorkAdmission({id:requestId,action,fingerprint:fp,
        parentRequestId:meta.accountParentRequestId},persist);
      else await persist({});
      // Publishing acquires the Assistant conversation lock, outside admission.
      await this.onJob(structuredClone(job));
      let claim;
      try{
        if(action==='appserver.create'){
          const cwd=await this.allowedProject(args.project);
          if(this.accountControls?.assertDelivery)await this.accountControls.assertDelivery(job);
          else await this.accountControls?.assertAdmission();
          // Matches ClawDad's approve-mode workspace sandbox. Existing threads
          // are resumed without overriding their stored permissions.
          const policy=await this.accessSettings?.resolve();
          const result=await this.client.request('thread/start',{cwd,...(policy?{modelProvider:'openai',...agentPermissionParams(policy,cwd).thread}:{sandbox:'workspace-write',approvalPolicy:'never'})});
          ...
          job.result.thread=verified.thread;await this.onCreated(verified.thread,cwd);
          await this.client.request('thread/unsubscribe',{threadId:id});
          job.status='completed';return this.changed(job);
        }
        ...
        if(action==='appserver.resume'){await this.client.request('thread/unsubscribe',{threadId:id});job.status='completed';job.result={threadId:id,owner};return this.changed(job);}
        ...
        const modelSettings=await this.selectedSettings(args,before.thread.cwd,content.images.length>0);
        job.agentPolicy=await this.accessSettings?.resolve();
        const settings=job.agentPolicy?{...modelSettings,...agentPermissionParams(job.agentPolicy,before.thread.cwd).turn}:modelSettings;
  ...
  async recordApproval(message){
    const owned=await this.transaction(async()=>{
      ...
      for(const job of this.state.jobs.filter(j=>!isTurnControl(j.action)&&j.threadId===threadId&&activeStates.has(j.status)))await this.reconcile(job.id);
      const job=this.state.jobs.findLast(j=>!isTurnControl(j.action)&&j.threadId===threadId&&j.turnId===message.params?.turnId&&activeStates.has(j.status));
  ...
  async dispatchWaiting(job){
    ...
    try {
      ...
      if(job.agentPolicy){
        const tools=await prepareAgentTools({root:path.dirname(this.root),threadId:job.threadId,requestId:job.id,text:job.args.text,
          policy:job.agentPolicy,source:'manual',accountEpoch:job.accountEpoch,accountReceipt:job,
          authorizationText:job.nativeAuthority?.text||'',authorizationSource:job.nativeAuthority?.source||'assistant'});
        await this.client.request('thread/unsubscribe',{threadId:job.threadId});
        await this.client.request('thread/resume',{threadId:job.threadId,modelProvider:'openai',...agentPermissionParams(job.agentPolicy,job.cwd).thread,config:tools.config});
        await verifyAgentToolConfiguration(this.client,job.threadId,job.agentPolicy);
        await tools.bind(job.threadId);
      }
  ...
  async reconcile(id){
    ...
    if(isTurnControl(job.action))return this.turnControls.reconcile(job);
  ...
  poll(){
    ...
    this.polling=this.transaction(async()=>{
      for(const job of this.state.jobs.filter(j=>isTurnControl(j.action)?observeTurnControl(j):activeStates.has(j.status))){try{await this.reconcile(job.id);}catch{job.connectionNotice='Waiting to reconnect to this task’s owning app server.';}}
      const seen=new Set();
