...
class CodexCatalogClient {
  constructor(binary, cwd, timeoutMs,accountLaunch) {
    this.binary = binary;
    this.cwd = cwd;
    this.timeoutMs = timeoutMs;
    this.accountLaunch=accountLaunch;
    this.child = null;
    this.buffer = "";
    this.stderr = "";
    this.nextId = 1;
  ...
  async start() {
    this.child = spawn(this.binary, withCodexAccountLaunch(["app-server", "--stdio"],this.accountLaunch), {
      cwd: this.cwd,
      env: this.accountLaunch?.env||process.env,
...
export async function readCodexModelCatalog({
  ...
  const client = new CodexCatalogClient(codexBinary, projectPath, timeoutMs,accountLaunch);
  try {
    await client.start();
    let authenticated;
    if (requireAuthenticated||accountLaunch) {
      const auth = await client.request("account/read", {refreshToken: false});
      authenticated = !!auth?.account;
      if (!authenticated) throw new Error("Sign in to Codex on this Mac before choosing Assistant models.");
      if(accountLaunch){
        ...
        if(JSON.stringify(auth.account)!==JSON.stringify(after.account)||after.account?.type!=='chatgpt'
          ||normalizeWeeklyUsage(limits,{account:after.account}).accountKey!==accountLaunch.account.key)
          throw Error('Check the selected saved account before choosing models. Its identity changed.');
      }
    }
    ...
    do {
      ...
      if (cursor && cursors.has(cursor)) throw new Error("Model catalog pagination repeated a cursor.");
