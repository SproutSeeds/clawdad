...
export async function readAccountWork(runtime){
  const files=[path.join(runtime.root,'state.json'),path.join(runtime.appServer?.root||path.join(runtime.root,'AppServer'),'state.json'),path.join(runtime.root,'Auxiliary','state.json')];
    ...
    try{
      ...
      for(const job of state.jobs){
        ...
        jobs.set(job.id,{id:job.id,action:job.action,fingerprint:job.fingerprint,...accountWorkEvidence(job),
          ...
          threadId:job.threadId||job.args?.threadId});
      }
