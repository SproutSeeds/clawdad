...
const actions=new Set(['observe','status','stop','launch','draft']);
...
export class CodexAccountNativeTransport {
  ...
  async settleCancellation(operationId){
    return this.transaction(async(state,save)=>{
      let changed=false;
      ...
      return state.requests.filter(r=>r.operationId===operationId).every(r=>['completed','not_dispatched'].includes(r.state));
    });
  }
  ...
  async complete({id:requestId,workerId,dispatchId,result,reasonCode}){
