...
async function defaultSocketOwnerResolver(socketPath, options = {}) {
  ...
  return {
    ...
    executable,
    cwd: await captureCommand('/usr/sbin/lsof', ['-nP', '-a', '-p', String(owner.pid), '-d', 'cwd', '-Fn'],
      {env, timeoutMs, spawnImpl: spawnChild}).then(r => r.stdout.split('\n').find(s => s.startsWith('n'))?.slice(1) || '').catch(() => ''),
  };
}

export async function sharedRuntimeUpgradeEligibility(owner, loadedThreads, exists = file => stat(file).then(() => true, () => false)) {
  if (!owner?.executable || !owner?.cwd) return false;
  if (await exists(owner.executable) && await exists(owner.cwd)) return false;
  return Array.isArray(loadedThreads) && loadedThreads.length === 0;
}

async function loadedThreadsAtSocket(socketPath) {
  const WS = await defaultWebSocketImpl();
  const ws = new WS(codexSharedWebSocketUrl(socketPath), {handshakeTimeout: 1500, perMessageDeflate: false});
  return new Promise((resolve, reject) => {
    let requestId = 1; const rows = [], cursors = new Set();
    const finish = (error, result) => { clearTimeout(timer); ws.terminate(); error ? reject(error) : resolve(result); };
    const timer = setTimeout(() => finish(new Error('Shared ownership inspection timed out')), 3000);
    ws.once('error', error => finish(error));
    ws.once('open', () => ws.send(JSON.stringify({id: requestId, method: 'initialize', params: {clientInfo: {name: 'clawdad_recovery', version: '1'}, capabilities: {experimentalApi: true}}})));
    ws.on('message', data => {
      let m; try { m = JSON.parse(String(data)); } catch { return; }
      if (m.id !== requestId) return;
      if (m.error) return finish(new Error('Shared ownership inspection was rejected'));
      if (requestId === 1) ws.send(JSON.stringify({method: 'initialized', params: {}}));
      else {
        if (!Array.isArray(m.result?.data)) return finish(new Error('Incomplete shared ownership inventory'));
        rows.push(...m.result.data);
        if (!m.result.nextCursor) return finish(null, rows);
        if (cursors.has(m.result.nextCursor)) return finish(new Error('Repeated shared ownership cursor'));
        cursors.add(m.result.nextCursor);
      }
      ws.send(JSON.stringify({id: ++requestId, method: 'thread/loaded/list', params: {limit: 100, ...(m.result?.nextCursor ? {cursor: m.result.nextCursor} : {})}}));
    });
  });
}

async function idleUpgradeCandidate(socketPath, options) {
  if (!options.checkLaunchPaths) return null;
  const owner = await resolveSocketOwner(socketPath, options);
  if (!verifiedCodexSocketOwner(owner, socketPath, options.codexBinary)) return null;
  if (!owner.executable || !owner.cwd || (await lstatIfPresent(owner.executable) && await lstatIfPresent(owner.cwd))) return null;
  const loaded = await loadedThreadsAtSocket(socketPath).catch(() => null);
  return await sharedRuntimeUpgradeEligibility(owner, loaded) ? owner : null;
}
...
export async function ensureCodexSharedRuntime(options = {}) {
  ...
  if (probe.ready && !await idleUpgradeCandidate(socketPath, options)) {
  ...
  try {
    ...
    if (probe.ready) {
      const obsolete = await idleUpgradeCandidate(socketPath, options);
      if (!obsolete) return sharedReadyResult({ requestedMode, probe, paths, reused: true, started: false });
      // Recheck under the common startup lock. A loaded thread, even idle,
      // keeps its owner. No forced shutdown or transfer of accepted work.
      const before = await lstatIfPresent(socketPath);
      const verified = await idleUpgradeCandidate(socketPath, options);
      if (verified?.pid !== obsolete.pid) return sharedReadyResult({requestedMode, probe, paths, reused: true, started: false});
      await signalProcess(verified.pid, 'SIGTERM', options);
      if (!await waitForProcessStop(verified.pid, 3000)) throw new Error('The obsolete idle shared server is still stopping. ClawDad will retry after it exits.');
      const after = await lstatIfPresent(socketPath);
      if (before?.isSocket() && after?.isSocket() && before.dev === after.dev && before.ino === after.ino) await unlink(socketPath);
      probe = await probeCodexSharedRuntime(probeOptions);
    }
    ...
    try {
      ownedChild = (options.spawnImpl || spawnChild)(
        codexBinary,
        withCodexAccountLaunch(["app-server", "--listen", listenUrl],options.accountLaunch),
        {
