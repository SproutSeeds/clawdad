...
export class CodexAccountSwitchProfiles {
  ...
  async verifyModels(home,email,accountKey,entries){
    const selections=entries.flatMap(e=>[e.native,...(e.window?.entries||[]).map(t=>({model:t.model,reasoningEffort:t.effort})),...(e.shared?.threads||[]).map(t=>t.settings)]).filter(v=>v?.model);
  ...
  async prepare({operation,target}){
    ...
    const sourceHomes=[...new Set(operation.recovery.entries.flatMap(e=>e.window
      ?Object.values(e.window.launches).map(l=>l.authorizationHome):[e.native?.authorizationHome||e.shared?.authorizationHome]))];
    ...
    const directories=[...new Set(operation.recovery.entries.flatMap(e=>[e.directory,...(e.window?.entries||[]).map(t=>t.directory),...(e.shared?.threads||[]).map(t=>t.cwd)]).filter(Boolean))].sort();
