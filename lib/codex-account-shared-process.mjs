...
export class CodexAccountSharedProcess {
  ...
  async observe(){
    ...
    if(matches.length!==1)throw failure('shared_native_owner_changed','The shared server process changed during inspection. Refresh its inventory.');
    if(row.kind!=='app_server')throw failure('shared_process_kind_unverified','The socket owner is not a verified Codex app server.');
    if(row.alternateAuthentication!==false)throw failure('shared_authentication_route_unverified','The shared server has an unverified authentication route. Its current account was preserved.');
    if(row.reasonCode)throw failure('shared_native_launch_unverified','The native shared-server launch check failed ('+row.reasonCode+').');
    if(!path.isAbsolute(row.authorizationHome||''))throw failure('shared_home_unverified','The shared server credential location could not be verified.');
    if(!path.isAbsolute(row.executable||''))throw failure('shared_executable_unverified','The exact running shared-server executable could not be verified. Refresh its native inventory.');
    if(!valid(row.processLifetime))throw failure('shared_lifetime_unverified','The shared server process lifetime could not be verified.');
    if(!Array.isArray(row.serverOptions))throw failure('shared_options_unverified','The shared server restart options could not be verified.');
