...
import {readAccountWindow} from './codex-account-window-switch.mjs';
...
export class MainWorkspaceResumeClaims {
  ...
  async claim({id,sessionId,accountWindow=false}){
    ...
    let roster;
    if(accountWindow){
      const adapter=this.runtime.accounts?.adapter,op=await adapter?.permit({operationId:id});
      if(op?.strategy!=='window-rebuild-v1'||op.phase!=='transition'||!adapter.windows)throw Error('No authorized account window restore is running.');
      const record=await readAccountWindow(adapter.root,id),expected=op.recovery?.entries.find(e=>e.window)?.window;
      if(record?.stage!=='restoring'||record.captureHash!==expected?.captureHash||record.target?.accountKey!==op.destinationAccountKey)throw Error('Account window recovery identity changed');
      roster={entries:record.entries};
    }else{
