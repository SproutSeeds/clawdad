...
export class CodexAccountSwitchAdapter {
  ...
  async inspect({operation=null}={}){
    if(operation)await this.accounts.switchStage(operation.id,'checking_window');
    ...
    for(const owner of scoped.consumers){
      ...
      if(owner.kind==='terminal_codex'){
        ...
        try{
          await this.accounts.switchStage(operation.id,'checking_project_threads');
          ...
          consumers.push({...owner,accountKey:source.accountKey,accountVerified:source.accountVerified,
            emptySourceAccountUnavailable:source.emptySourceAccountUnavailable===true,recoverable:true,busy:false,sessionId:null,directory:null,draft,pendingReceipts:[],reason:''});
