...
export function verifyAccountSwitchInventory(inventory,operation) {
  const current=accountSwitchScope(inventory,operation);
...
export function connectCodexAccountSwitchRuntime({accounts,runtime,binary,socketPath,canonicalHome,root=accounts.root}={}){
  ...
  const verifyManaged=async({operation})=>{
    return verifyAccountSwitchInventory(await inventory(),operation);
  };
  adapter=new CodexAccountSwitchAdapter({root,accounts,runtime,inventory,profiles,sharedDriver,verifyPending,verifyManaged,windowRebuild:true});
