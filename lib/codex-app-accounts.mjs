...
const selectionRequired=()=>Object.assign(Error('Choose and activate a saved account for ClawDad before starting app work.'),{code:'app_account_required'});
...
export class CodexAppAccounts {
  constructor({root=codexAccountsRoot(),usage,readActive=null,requireSelection=!!process.env.CLAWDAD_NATIVE_RUNTIME_VERSION,readWork=async()=>({complete:true,jobs:[]}),adapter=null,authorizations=null,clock=Date.now,lease=acquireCodexDeliveryClaim}={}) {
    Object.assign(this,{root,usage,readActive,requireSelection,readWork,adapter,authorizations,clock,lease});
  ...
  async selectedLaunch({allowUnselected=false}={}){
    // Native app work requires an explicitly selected isolated profile. Only
    // an exact previously accepted legacy request may drain an existing owner.
    const unselected=()=>{if(this.requireSelection&&!allowUnselected)throw selectionRequired();return null;};
    ...
    catch(error){if(error.code==='ENOENT')return unselected();throw Error('The selected account checkpoint needs recovery.');}
    ...
    if(!state.selected)return unselected();
  ...
  reconcileWork(state,work,{importExisting=false}={}){
    ...
    const pending=[];
    const item=(entry,row,status)=>({id:entry.id,action:entry.action,status,
      ...(row?.projectPath||entry.projectPath?{project:path.basename(row?.projectPath||entry.projectPath)}:{}),
      ...(row?.threadId||row?.sessionId?{threadId:row.threadId||row.sessionId}:{})});
    for(const entry of Object.values(state.work)){
      ...
      if(row){
        if(hash(row.fingerprint)!==entry.fingerprint||row.action!==entry.action||![entry.epoch,entry.admittedEpoch].includes(row.accountEpoch??entry.epoch)){
          pending.push(item(entry,row,'receipt_identity_changed'));continue;
        }
        if(entry.status!==row.status||!entry.committed){entry.status=row.status;entry.committed=true;changed=true;}
      }
      if(!settledWork.has(entry.status))pending.push(item(entry,row,row?entry.status:'acceptance_needs_reconciliation'));
    }
    ...
    for(const row of rows.values())if(pendingWork.has(row.status)&&!row.accountSwitchHold&&!row.accountReadOnly&&!state.work[row.id])
      pending.push(item(row,row,'unregistered_accepted_work'));
  ...
  async withWorkAdmission({id,action,fingerprint,parentRequestId=null,allowHold=false,readOnly=false,expectedEpoch=null},persist){
    ...
    return this.transaction(async(state,save)=>{
      ...
      if(this.requireSelection&&!state.selectedRuntime&&!op?.fenced&&!readOnly)throw selectionRequired();
  ...
  async snapshot(){
    ...
    const observed=this.readActive?await this.readActive().catch(()=>({status:'unavailable',source:'app-server',
      message:'The running ClawDad account could not be verified. Check the Mac connection.'})):null;
    return this.transaction(s=>{
      ...
      const activeAccountId=observed?(observed.status==='current'&&observed.accountKey===selected?.accountKey
        &&observed.authorizationHome===selected?.authorizationHome?selected.accountId:null):this.readActive?null:selected?.accountId||null;
      const current=observed?(({usage,...identity})=>identity)(observed):null;
      return {version:1,scope:'clawdad-app',revision:s.selectionRevision,journalRevision:s.revision,epoch:s.epoch,
        ...
        selectedAccountId:selected?.accountId||null,requiresActivation:this.requireSelection&&!selected,current,
  ...
  async admission(){return this.transaction(s=>{
    const op=s.operations[s.activeOperationId],missing=this.requireSelection&&!s.selectedRuntime;
    return {allowed:!op?.fenced&&!missing,epoch:s.epoch,operationId:op?.fenced?op.id:null,phase:op?.fenced?op.phase:missing?'unselected':null,
      reasonCode:op?.fenced?'account_switch_pending':missing?'app_account_required':null,
      reason:op?.fenced?'ClawDad account activation is in progress. Accepted app work keeps its account.':missing?selectionRequired().message:''};
  });}
  async assertAdmission(){const gate=await this.admission();if(!gate.allowed)throw Object.assign(Error(gate.reason),{code:gate.reasonCode||'account_switch_pending'});return gate;}
  async deliveryAdmission(job){
    if(!appAccountAction(job.action)||job.accountReadOnly)return {allowed:true};
    const observed=this.readActive?await this.readActive().catch(()=>null):null;
    return this.transaction(async(s,save)=>{
      const entry=s.work?.[job.accountWorkId||job.id],op=s.operations[s.activeOperationId];
      if(this.requireSelection&&!s.selectedRuntime&&!op?.fenced)return {allowed:false,reasonCode:'app_account_required',reason:selectionRequired().message};
      ...
      if(this.readActive&&s.selectedRuntime&&(observed?.status!=='current'
        ||observed.accountKey!==s.selectedRuntime.accountKey||observed.authorizationHome!==s.selectedRuntime.authorizationHome))
        return reject('The running ClawDad account does not match the selected profile. Check account activation before sending this saved request.');
  ...
  async retry({operationId,requestId,confirmed}){
    ...
    return this.transaction(async(s,save)=>{
      ...
      op.status='checking';op.reasonCode=null;op.retryAt=null;op.nativeReadRetries=0;s.requests[requestId]={operationId,fingerprint};await save();return copy(op);
    });
  }
  async advance(){
    ...
    try{
      ...
      // A missing native reader is a retryable observation, including a saved
      // build-162 timeout. Every transition still reconciles its own effects;
      // uncertain delivery and changed ownership always require recovery.
      const nativeUnavailable=op?.reasonCode==='app_process_reader_unavailable';
      if(!op?.fenced||op.cancelRequested||op.status==='needs_attention'&&!nativeUnavailable)return;
      if(nativeUnavailable&&Number.isFinite(op.retryAt)&&this.clock()<op.retryAt)return;
      const update=async fields=>{op=await this.transaction(async(s,save)=>{
        const current=s.operations[op.id];if(current.cancelRequested)throw Error('Activation cancelled');
        Object.assign(current,fields);if(fields.phase)Object.assign(current,{retryAt:null,nativeReadRetries:0});await save();return copy(current);
      });};
      ...
      try{
        if(op.phase==='preflight'){
          ...
          if(!drain.ready){
            const blocked=drain.pending.filter(j=>!['queued','sending','running','submitted','working','agent_queued'].includes(j.status));
            const first=blocked[0]||drain.pending[0],kind=first?.action==='message'?'Assistant':first?.action==='research.review'?'supervisor review':first?.project?`project ${first.project}`:'project thread';
            const label=first?`ClawDad ${kind} (request ${first.id.slice(0,8)})`:'';
            return await update({status:!drain.complete||blocked.length?'needs_attention':'waiting',reasonCode:blocked.length?'app_work_needs_reconciliation':'accepted_app_work',pendingWork:drain.pending,
              reason:!drain.complete?'ClawDad could not read accepted app-work receipts. Retry after reconnecting.':blocked.length?
                `${label} needs receipt recovery (${first.status}). Retry activation to check it again.`:
                `Waiting for ${label} to finish on its current account.${drain.pending.length>1?` ${drain.pending.length} app requests remain.`:''}`});
          }
          await update({pendingWork:[]});
        ...
        await this.transaction(async(s,save)=>{
          ...
          if(code==='app_process_reader_unavailable'){
            current.nativeReadRetries=Math.min((current.nativeReadRetries||0)+1,10);
            const delay=Math.min(30000,3000*2**(current.nativeReadRetries-1));
            Object.assign(current,{status:'waiting',reasonCode:code,retryAt:this.clock()+delay,
              reason:'Waiting for the Mac connection. Activation will continue automatically when its process check is ready.'});
            await save();return;
          }
