...
export const appReleaseCatalog = Object.freeze({
  ...
  beta: Object.freeze({
    ...
    whatsNew: [
      "Build 67: Assistant voice turn ending. Use with Mac build 79 or later.",
      "• Assistant automatically sends your speaking turn after four seconds without new words. New speech can continue the thought; repeated partial text and background noise do not keep resetting the deadline.",
      "• Final recorded words finish transcribing before submission. The conversation stays connected, and Send now remains available.",
      "• Turn on Think aloud in the Assistant conversation to keep listening through long pauses until you tap Send. This new opt-in setting starts off.",
      "• Timing diagnostics distinguish final transcription, message submission, response arrival, and playback startup while retaining connection recovery and saved chat/image drafts.",
      "• Assistant replaces stale connections automatically before starting a call or sending a message. Retry Connection now creates a fresh connection.",
      "• Messages retain their original delivery ID through recovery, so reconnecting cannot deliver an accepted message twice. Unsent text and image drafts stay saved.",
      "• Simultaneous speech and status replies now wait their turn on the connection, preventing a valid response from being rejected.",
      "• Remote Assist now groups labeled controls into Assistant, Mac input, and Workspace. Chat and Call remain separate choices.",
      "• Photo to Terminal, Assistant image attachments, and Quick Chat presets have distinct destinations and labels. Back returns one step; controls scroll when space is limited.",
      "• Ask Assistant to queue an authorized follow-up in a working Codex tab. It uses the native Terminal input and presses Tab once, preserving existing drafts and other tabs.",
      "• Queued in agent confirms the native queue entry. Working and Completed track the follow-up's own turn. Unsupported or uncertain deliveries require inspection and are never automatically repeated.",
      "• Native queue delivery is verified with Codex 0.153.4. The targeted agent must be working with an empty, readable input and its Tab queue binding available.",
      "• Assistant text chat supports image previews, removal, and locally saved drafts that survive closing, reopening, and failed sends.",
      "• Existing local dictation, Read Aloud, shared Files, notifications, phone/address links, and verified input editing remain available.",
      "Keep ClawDad running and unlocked on the Mac for native Terminal input. Report issues from https://clawdad-cloud.frg.earth/support.",
    ].join("\n"),
