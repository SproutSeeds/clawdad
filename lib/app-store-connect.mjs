...
export const appReleaseCatalog = Object.freeze({
  ...
  beta: Object.freeze({
    ...
    whatsNew: [
      "Build 61: Tap business phone numbers and addresses in Assistant replies, and edit drafts through verified native controls. Use with Mac build 73 or later.",
      "• Tap a phone number in an Assistant response to open the iPhone call flow.",
      "• Tap a street address to open Google Maps, with Apple Maps as a fallback.",
      "• Ask the Assistant to clear or replace an inspected Terminal draft or a supported app input. Editing verifies the target and resulting text without pressing Enter.",
      "• Changed drafts, busy agents and unsupported inputs remain protected; permissions and application restrictions are retained.",
      "• Tap Send now in the black call bar to finish recording and send the completed transcription once.",
      "• Brief pauses keep the same thought open. Choose Wait for Send in the Assistant conversation's Voice sending menu for longer pauses.",
      "• Ready calls avoid repeated startup checks before every message. Local timing diagnostics separate capture, transcription, and delivery delays.",
      "• The Assistant can leave exact text in an empty Terminal agent input without pressing Enter. Native tab identity and draft checks verify the destination.",
      "• Microphone input pauses throughout spoken replies and resumes afterward. Interject stops playback so you can speak immediately.",
      "• Tap the message bubbles in the call bar to see your transcriptions and the Assistant's replies while the call continues.",
      "• Local transcription previews update while you speak; only the completed transcription is submitted to the Assistant.",
      "• Matching capture and playback formats fixes the microphone startup timeout on iPhone.",
      "• Audio callbacks safely return to the interface thread, fixing a crash when microphone audio arrives.",
      "• Microphone failures are identified separately from connection failures, with a Retry microphone control.",
      "• Assistant recognizes quieter speech and brief pauses between syllables, then sends the completed thought automatically.",
      "• Microphone activity is visible in the compact call controls. Capture startup and stalled audio streams are checked and recovered.",
      "• Speech playback keeps the call audio format stable so it can continue listening between replies.",
      "• The baby-in-a-claw artwork now appears on the Home Screen, in notifications and Settings, and in TestFlight.",
      "• Enable Agent response notifications in Settings and allow iPhone notifications.",
      "• Completed Terminal agent turns include the directory, conversation reference, and completion time. Tap an alert to open the exact conversation.",
      "• Completion detection runs on the Mac, including tabs you have not visited on iPhone. Reconnects and ordinary Terminal activity do not replay alerts.",
      "• Tap the headset to start talking immediately, with compact Mute, End, and Messages controls.",
      "• One persistent background Codex conversation reconnects without creating Terminal windows. Optional text and history remain available.",
      "• Conversation uses GPT-6 Astra with low reasoning. Speech uses your selected local Mac STT and TTS models and voice.",
      "• Ask about existing Terminal tabs, inspect their context, and send tasks to the intended agent. Watch the work in Remote Assist.",
      "• Conversation audio continues across navigation, with Mute and End controls. Busy agents queue work and existing drafts are preserved.",
      "• Requests use durable delivery receipts. Reconnecting cannot silently submit the same task twice.",
      "",
      "Update the Mac before starting a call. Existing project agents keep their own models and settings.",
      "Keep ClawDad running on the Mac. Notification delivery has been repaired and confirmed on iPhone. Test an agent response while the iPhone is locked, tap the alert, and confirm the correct conversation opens.",
      "Existing Quick Chat, inline dictation, speaker playback, photo transfers, and shared Files remain available.",
