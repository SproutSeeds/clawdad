...
export class LocalFileLibrary {
  ...
  publicItem(item) {
    const { groupKey, uploadOwner, uploadId, ...metadata } = item;
    return { ...metadata, versions: item.versions.map(({ sourcePath, storagePath, ...version }) => version) };
  }

  async list({ query = "", project = "", archived = false, pinned = false, format = "", category = "documents", cursor = 0, limit = 40 } = {}) {
    const catalog = await this.catalog();
    const needle = cleanText(query).toLowerCase();
    if (!["documents", "receivedImages", "all"].includes(category)) throw new Error("Choose Documents or Received Images.");
    const all = catalog.items.filter((item) => (category === "all" || (item.category || "documents") === category) && Boolean(item.archived) === Boolean(archived) && (!pinned || item.pinned) && (!project || item.project === (project === "__personal__" ? "" : project)) && (!format || item.versions.some((version) => version.format === format)) && (!needle || [item.title, item.project, ...item.versions.map((version) => version.fileName)].join("\n").toLowerCase().includes(needle)))
  ...
  async resolve(id, versionId) {
    ...
    const file = version.storagePath ? this.receivedImagePath(version) : path.join(this.root, "objects", versionId);
    const parent = await realpath(path.dirname(file));
    if (!parent.startsWith(await realpath(this.root) + path.sep)) throw new Error("This saved file is outside the library.");
  ...
  receivedImagePath(version) {
    const expected = path.join("received", version.id, version.fileName);
    if (!uuidPattern.test(version.id) || version.storagePath !== expected || path.basename(version.fileName) !== version.fileName) throw new Error("Invalid received image path.");
    return path.join(this.root, expected);
  }
