  ...
  for (;;) {
    try {
      ...
      if (atomicPublication) {
        // A killed initializer may leave a private staging directory, but can
        // never publish an empty/malformed owner at the contended claim path.
        const staging = path.join(claimRoot, `.${key}.prepare-${randomUUID()}`);
        await mkdir(staging, { mode: 0o700 });
        try {
          const handle = await open(path.join(staging, 'owner.json'), 'wx', 0o600);
          try { await handle.writeFile(`${JSON.stringify(owner, null, 2)}\n`, 'utf8'); await handle.sync(); }
          finally { await handle.close(); }
          const directory = await open(staging, 'r');
          try { await directory.sync(); } finally { await directory.close(); }
          try { await rename(staging, claimDir); }
          catch (error) { if (['EEXIST', 'ENOTEMPTY'].includes(error.code)) error.code='EEXIST'; throw error; }
        ...
        try { await writeFile(ownerFile, `${JSON.stringify(owner, null, 2)}\n`, {
          ...
          // The legacy delivery path retains its existing behavior. Account
          // controllers opt into fully published owner metadata above.
