...
function parseArgs(argv) {
  const options = {
    ...
    turnIdleTimeoutMs: parseNonNegativeInteger(process.env.CLAWDAD_CODEX_TURN_IDLE_TIMEOUT_MS, 5 * 60 * 1000),
...
class CodexTurnIdleTimeoutError extends Error {
  constructor(message, { completedAgentMessages = [], agentDeltaTexts = new Map(), idleTimeoutMs = 0 } = {}) {
    super(message);
...
function recoveredIdleResultText(partialText, idleTimeoutMs) {
  const seconds = Math.max(1, Math.ceil(Number(idleTimeoutMs || 0) / 1000));
  const recoveryNote =
    `Clawdad recovered this response because the Codex turn made no live progress for ${seconds}s before it finished. ` +
    "The run has been closed so the project will stop spinning.";
...
async function waitForTurnCompletion(
  ...
  turnId,
  timeoutMs = 0,
  { onAgentText = null, idleTimeoutMs = 0 } = {},
) {
      ...
      if (idleTimeoutMs > 0) {
        idleTimeoutId = setTimeout(() => {
          ...
          reject(new CodexTurnIdleTimeoutError(
            `codex turn made no live progress for ${Math.ceil(idleTimeoutMs / 1000)}s before completion`,
            {
...
async function main() {
  ...
  try {
    ...
    if (error instanceof CodexTurnIdleTimeoutError) {
      ...
      const resultText = recoveredIdleResultText(partialText, error.idleTimeoutMs);
      await liveReporter.flush(resultText);
      printJson({
        ok: true,
        recovered: true,
        recovery_reason: "turn_idle_timeout",
        session_id: sessionId || "",
        thread_source: threadSource || null,
        thread_path: threadPath || null,
        thread_goal_mode: goalSync.mode,
