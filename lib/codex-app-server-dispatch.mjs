...
function parseArgs(argv) {
  const options = {
    ...
    toolIdleTimeoutMs: parseNonNegativeInteger(process.env.CLAWDAD_CODEX_TOOL_IDLE_TIMEOUT_MS, 90 * 1000),
...
function recoveredIdleResultText(partialText, idleTimeoutMs, recoveryReason = "turn_idle_timeout") {
  ...
  const subject =
    recoveryReason === "tool_idle_timeout"
      ...
      : "the Codex turn";
  const recoveryNote =
    `Clawdad recovered this response because ${subject} made no live progress for ${seconds}s before it finished. ` +
        ...
        for (const item of pending) {
          ...
          if (!message) {
            ...
            await updateInterjectionHistory(projectPath, requestId, {
              ...
              status: "failed",
              exitCode: 1,
              response: "Clawdad could not interject this message because it was empty.",
            }).catch(() => {});
            continue;
          }
          ...
          try {
            ...
            await updateInterjectionHistory(projectPath, requestId, {
              ...
              status: "answered",
              exitCode: 0,
              response: "Interjected into the active Codex turn. The active response may include this message.",
            }).catch(() => {});
          } catch (error) {
            const errorText = describeError(error, "failed to interject into active Codex turn");
            await markInterjectionFile(item.filePath, item.payload, "failed", {
              sessionId: threadId,
              error: errorText,
            });
