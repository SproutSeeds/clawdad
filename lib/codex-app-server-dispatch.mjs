...
import {acquireThreadControl,assertSharedThreadOwner} from './codex-thread-control.mjs';
import { acquireCodexDeliveryClaim } from "./codex-delivery-claim.mjs";
import {accountRouteForLegacyRequest,ensureAccountSharedRuntime} from './codex-account-runtime-routing.mjs';
import {AgentAccessSettings,agentPermissionParams} from './agent-access-settings.mjs';
...
const THREAD_TURNS_UNAVAILABLE = Symbol("threadTurnsUnavailable");
const clientsWithoutThreadHistory = new WeakSet();
...
function buildThreadParams(options) {
  ...
  if(options.agentPolicy)Object.assign(params,{modelProvider:'openai'},agentPermissionParams(options.agentPolicy,options.projectPath).thread);
  if(options.agentTools)params.config={...params.config,...options.agentTools.config};
  return params;
}
...
function buildTurnParams(threadId, options) {
  ...
  if(options.agentPolicy)Object.assign(params,agentPermissionParams(options.agentPolicy,options.projectPath).turn);
  return params;
}
...
class AppServerClient {
  ...
  async start({ experimentalApi = false } = {}) {
    ...
    if(process.platform==='darwin'&&process.env.CLAWDAD_NATIVE_RUNTIME_VERSION)
      this.accountRoute=await accountRouteForLegacyRequest({binary:this.binary,requestId:this.accountRequestId,projectPath:this.cwd});
    if (this.appServerMode === "isolated") {
      this.#startIsolated();
    } else {
      const runtime = await this.ensureShared({
  ...
  ensureShared(options){return this.accountRoute?ensureAccountSharedRuntime(options,this.accountRoute):ensureCodexSharedRuntime(options);}
  ...
  #startIsolated() {
    ...
    this.child = spawn(this.binary, withCodexAccountLaunch(["app-server"],this.accountRoute?.launch), {
      cwd: this.cwd,
      stdio: ["pipe", "pipe", "pipe"],
      env: this.accountRoute?.launch?.env||process.env,
      detached: true,
    });
...
export function serverRequestResultFromDecision(message, decision = {}) {
...
export async function handleCodexServerRequest(
  ...
  const automatic = automaticServerRequestResult(message, options.agentPolicy?'approve':options.permissionMode);
...
async function waitForTurnCompletion(
  ...
  return new Promise((resolve, reject) => {
    ...
    const probeLiveness = async () => {
      ...
      try {
        const latestThread = await readSharedThread(client, threadId, livenessProbeTimeoutMs);
        if (settled) {
          return;
        }
        const turns = Array.isArray(latestThread?.turns) ? latestThread.turns : [];
        const turn = turns.find((entry) => String(entry?.id || "").trim() === turnId) || null;
        const turnStatus = turn
          ? String(turn.status || "inProgress").trim() || "inProgress"
          : threadTurnsUnavailable(latestThread)
            ? "unknown"
            : "inProgress";
        ...
        armIdleTimer();
        if (turn && ["completed", "failed", "cancelled", "canceled", "interrupted"].includes(turnStatus.toLowerCase())) {
          finishResolve({
            turn,
            completedAgentMessages,
            agentDeltaTexts,
...
async function readTurnResult(client, threadId, turnId, completedAgentMessages, agentDeltaTexts) {
  ...
  const latestThread = await readSharedThread(client, threadId);
  const turns = Array.isArray(latestThread?.turns) ? latestThread.turns : [];
...
function threadTurnsUnavailableError(error) {
  return /\blist_turns\b.*\bnot supported\b|\bturn history\b.*\bnot supported\b/iu
    .test(describeError(error));
}

function threadTurnsUnavailable(thread = {}) {
  return Boolean(thread?.[THREAD_TURNS_UNAVAILABLE]);
}
...
async function readSharedThread(client, threadId, timeoutMs = undefined) {
  if (!clientsWithoutThreadHistory.has(client)) {
    try {
      ...
      if (!threadTurnsUnavailableError(error)) {
        throw error;
      }
      clientsWithoutThreadHistory.add(client);
    }
  }
  ...
  Object.defineProperty(thread, THREAD_TURNS_UNAVAILABLE, { value: true });
  return thread;
}
...
async function routeSharedTurn(client, threadId, initialThread, options, {
  ...
  for (;;) {
    ...
    if (!activeTurn && threadTurnsUnavailable(thread) && threadStatusType(thread) !== "idle") {
      // A summary-only read cannot identify or safely steer another client's
      // active turn. Hold the delivery claim and wait until the thread is idle
      // instead of risking a duplicate turn/start.
      deferUntilIdle = true;
      await sleep(100);
    ...
    try {
      if(options.agentPolicy){
        accessClaim=await acquireThreadControl(threadId);
        await assertSharedThreadOwner(client,threadId);
        thread=await readSharedThread(client,threadId);
        if(thread?.status?.type==='active')continue;
        const root=serverRuntimeRoots().assistant;
        options.agentTools=await prepareAgentTools({root,threadId,requestId:options.requestId,text:options.message,
          policy:options.agentPolicy,source:'manual',accountReceipt:client.accountRoute?.receipt,
          authorizationText:process.env.CLAWDAD_MANUAL_REQUEST_TEXT||'',
          authorizationSource:process.env.CLAWDAD_MANUAL_REQUEST_TEXT?'user':'assistant',
          accountEpoch:(await client.accountRoute?.accounts.admission())?.epoch});
        // Resume overrides only rebuild an idle cache entry with no subscribers.
        // Release this dispatch client's initial rejoin before applying tools.
        await client.request('thread/unsubscribe',{threadId});
        await client.request('thread/resume',{...buildThreadParams(options),threadId});
        await verifyAgentToolConfiguration(client,threadId,options.agentPolicy);
        await options.agentTools.bind(threadId);
      }
      ...
      await sleep(100);
    }finally{await accessClaim?.release();}
  }
}
...
async function main() {
  ...
  const client = new AppServerClient(options.codexBinary, options.projectPath, {
    ...
    accountRequestId: options.requestId,
  });
  ...
  try {
    ...
    if(process.env.CLAWDAD_NATIVE_RUNTIME_VERSION&&!options.goalOnly){
      if(client.effectiveMode!=='shared')throw Error('Reconnect the selected ClawDad app server before sending this request. No fallback model process was used.');
      const root=serverRuntimeRoots().assistant;
      options.agentPolicy=await new AgentAccessSettings({file:path.join(root,'agent-access.json')}).resolve(options.permissionMode);
      // The exact idle-thread lock in routeSharedTurn prepares native tools.
      // A direct interjection leaves the running turn's permissions intact.
    }
    ...
    if ((options.sessionSeeded || options.goalOnly) && sessionId) {
      let ownershipClaim;
      if(client.effectiveMode==='shared'&&process.platform==='darwin'&&process.env.CLAWDAD_NATIVE_RUNTIME_VERSION){
        ownershipClaim=await acquireThreadControl(sessionId);
        try { await assertSharedThreadOwner(client,sessionId); }
        catch(error){await ownershipClaim.release();throw error;}
      }
      try {
      threadResult = await client.request(
        "thread/resume",
        client.effectiveMode === "shared"&&!options.agentTools
          ? { threadId: sessionId }
          : { ...buildThreadParams(options), threadId: sessionId },
        options.resumeTimeoutMs,
      );
      } finally { await ownershipClaim?.release(); }
    ...
    await options.agentTools?.bind(sessionId);
    ...
    if(['start','deferred_start'].includes(routedTurn.deliveryMode))await options.agentTools?.bind(sessionId,turnId);
    ...
    try {
      await options.agentTools?.close();
