...
function parseArgs(argv) {
  const options = {
    ...
    model: "",
    sessionSeeded: false,
    turnTimeoutMs: Number.parseInt(process.env.CLAWDAD_CODEX_TURN_TIMEOUT_MS || "", 10) || 0,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--turn-timeout-ms":
        options.turnTimeoutMs = Number.parseInt(String(argv[index + 1] || "").trim(), 10) || 0;
        index += 1;
        break;
      case "--session-seeded":
...
function extractAgentMessageText(items = []) {
  const agentMessages = items.filter((item) => item && item.type === "agentMessage" && typeof item.text === "string");
  ...
  return String(finalMessage.text || "");
}
...
async function waitForTurnCompletion(client, threadId, turnId, timeoutMs = 0) {
    ...
    removeListener = client.onNotification((message) => {
      ...
      if (message.method === "item/agentMessage/delta") {
        ...
        if (params.threadId === threadId && params.turnId === turnId && params.itemId) {
          ...
          agentDeltaTexts.set(params.itemId, existing + String(params.delta || ""));
        }
        return;
      }
...
async function readTurnResult(client, threadId, turnId, completedAgentMessages, agentDeltaTexts) {
  ...
  const fromRead = extractAgentMessageText(Array.isArray(turn?.items) ? turn.items : []);
  if (fromRead) {
    return fromRead;
  }
  ...
  const lastDelta = Array.from(agentDeltaTexts.values()).findLast((value) => value && String(value).trim());
