...
export class CodexAccountLayout {
  ...
  async verify(receipt){
    ...
    // Config/instruction files are routinely atomically replaced. Their exact
    // canonical path and owned symlink remain authoritative; an old inode is
    // not an account or history identity. Directory replacement still stops.
    const identity=plan=>({...plan,fingerprint:undefined,entries:plan.entries.map(entry=>entry.kind==='file'?{...entry,inode:undefined}:entry)});
    if(current.fingerprint!==receipt.plan.fingerprint&&digest(identity(current))!==digest(identity(receipt.plan)))
      throw invalid('layout_changed','The saved runtime layout changed before launch. Inspect it again.');
