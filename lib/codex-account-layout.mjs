...
export async function inspectCodexAccountLayout({canonicalHome,profileHome,sqliteHome=canonicalHome}){
  ...
  const names=await fs.readdir(canonicalHome);
  ...
  for(const name of selected){
    ...
    if(!stat){
      ...
      if(await present(destination))throw invalid('profile_resource_conflict',`The saved sign-in has an independent ${name} resource. Review its history/configuration before adoption.`);
      continue;
    }
    ...
    if(existing&&(!existing.isSymbolicLink()||await fs.readlink(destination)!==source||await fs.realpath(destination)!==target))
      throw invalid('profile_resource_conflict',`The saved sign-in already has a different ${name} resource. Nothing was replaced.`);
