...
async function runServe(argv) {
  ...
  assistantRuntime.appServer = new AssistantAppServer({root:path.join(assistantRuntime.root,'AppServer'),
    ...
    onCreated:(thread,cwd)=>registerImportedSessionLocally(cwd,{sessionId:thread.id,title:thread.name||'Assistant thread',file:thread.path,cwd}, {}, {makeActive:false})});
  ...
  const handleRequest = async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/v1/dispatch") {
      await handleDispatch(req, res, {...options,accountWork:accountSwitchRuntime?accountLegacyWork:null}, auth.actor);
      return;
    }
