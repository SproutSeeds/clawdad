...
import { voiceSettings, runtimeWithVoice } from "./local-voices.mjs";
...
async function runServe(argv) {
  ...
  await new Promise((resolve, reject) => {
    ...
    server.once("close", () => { weeklyUsage.stop(); void assistantRuntime.close(); });
  ...
  if (process.platform === 'darwin' && process.env.CLAWDAD_NATIVE_RUNTIME_VERSION) { weeklyUsage.start(); assistantRuntime.research.start(); }
