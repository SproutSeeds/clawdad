...
import { chooseDelegateSession, shouldClearPendingDelegatePause } from "./delegate-state.mjs";
...
const delegateDefaultHardStops = Object.freeze(["paid", "needs_human", "auth_required", "compute_limit"]);
...
function normalizeDelegateConfig(payload = {}) {
  ...
  let maxStepsPerRun = normalizeOptionalPositiveInteger(rawMaxSteps, { max: 200 });
  const payloadVersion = Number.parseInt(String(payload.version || "1"), 10) || 1;
  if (payloadVersion <= 1 && maxStepsPerRun === delegateLegacyDefaultMaxStepsPerRun) {
...
- Treat paid services, another human, credentials/auth, and low weekly Codex compute reserve as hard stops.
- Mention uncertainty plainly.
...
function buildDelegateStepPrompt(project, delegateSession, brief, latestPlan, latestSummary, sourceEntries, status) {
  ...
  const stepLabel = maxSteps ? `${currentStep} of ${maxSteps}` : `${currentStep} (no default step cap)`;
...
\`\`\`json
{"state":"continue|blocked|completed","stop_reason":"none|paid|needs_human|auth_required|compute_limit|step_limit|unknown","next_action":"short string","summary":"short string"}
\`\`\`
...
function normalizeDelegateDecision(payload = {}) {
  ...
  const normalizedStopReason =
    ["none", "paid", "needs_human", "auth_required", "compute_limit", "step_limit", "unknown"].includes(stopReason)
      ? stopReason
      : "unknown";
...
async function runTrackedSessionDispatchWait(projectPath, sessionId, message, { permissionMode = "approve", model = "" } = {}) {
  ...
  const startResult = await startClawdadDetached(args, {
    env: workerTimeoutMs ? { CLAWDAD_CODEX_TURN_TIMEOUT_MS: String(workerTimeoutMs) } : {},
  });
  if (!startResult.ok) {
...
async function pauseDelegateRun(projectDetails) {
  ...
  const isActive = currentStatus.state === "running" || currentStatus.state === "planning";
  const nextStatus = await writeDelegateStatus(projectDetails.path, {
    ...currentStatus,
    state: isActive ? currentStatus.state : "paused",
    pauseRequested: isActive,
    completedAt: isActive ? currentStatus.completedAt : new Date().toISOString(),
    error: "",
  });
