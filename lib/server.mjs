...
async function runWatchtower(argv) {
  ...
  const scanOnce = async () => {
    ...
    const cards = await readWatchtowerReviewCards(resolved.projectPath, { limit: 12 });
  ...
  if (rawOptions.once || rawOptions.json) {
    ...
    if (rawOptions.json) {
      ...
      console.log(`Watchtower indexed ${payload.scan.indexedEvents} update(s); ${payload.reviewCards.length} review card(s).`);
