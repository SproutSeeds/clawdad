...
function parseArgs(argv) {
  const options = { _: [], allowUser: [], liveApp: [] };
...
function delegateCatalogStatusIsLive(status = {}) {
  ...
  return state === "planning" || state === "running";
}
...
- If Clawdad or Watchtower reports unclassified paths, sensitive files, failed gates, or a large diff that needs review, make the next action hygiene/review/checkpoint before widening the work.
...
async function runSqlite(dbFile, sql, { json: jsonMode = false } = {}) {
...
INSERT INTO feed_events (
  id, project_path, run_id, at, source_type, source_ref, event_type, title, body,
  active_orp_item, worker_summary, files_changed, tests_gates, current_decision,
  risk_flags, review_status, payload_json, created_at
) VALUES (
  ${sqlLiteral(event.id)},
...
INSERT OR IGNORE INTO review_cards (
  id, event_id, project_path, run_id, at, trigger, title, summary, review_status,
...
async function insertWatchtowerEvent(projectPath, event, cards = []) {
  ...
  SELECT id FROM review_cards WHERE event_id = ${sqlLiteral(event.id)}
);`,
    `DELETE FROM review_cards WHERE event_id = ${sqlLiteral(event.id)};`,
    `DELETE FROM feed_events_fts WHERE event_id = ${sqlLiteral(event.id)};`,
    `DELETE FROM feed_events WHERE id = ${sqlLiteral(event.id)};`,
    watchtowerSqlEventInsert(event),
    ...cards.map(watchtowerSqlCardInsert),
    "COMMIT;",
  ].join("\n");
  await runSqlite(dbFile, sql);
}
...
async function readWatchtowerTail(projectPath, { limit = 40 } = {}) {
  const dbFile = await ensureWatchtowerDb(projectPath);
  const rows = await runSqlite(
    dbFile,
...
async function searchWatchtowerFeed(projectPath, query, { limit = 40 } = {}) {
  const dbFile = await ensureWatchtowerDb(projectPath);
...
async function readWatchtowerReviewCards(projectPath, { limit = 40 } = {}) {
  const dbFile = await ensureWatchtowerDb(projectPath);
  const rows = await runSqlite(
    dbFile,
...
function watchtowerGuardrailHeading(line) {
  return /^(?:#{1,6}\s*)?(?:\*\*)?(?:hard stops?|guardrails?|safety constraints?|safety rules?)(?:\*\*)?:?\s*$/iu.test(
    String(line || "").trim(),
  );
}
...
function watchtowerGuardrailLine(line) {
  ...
  return (
    ...
    /\bno\s+(?:paid|spend|credentials?|auth|billing|human|external)\b/iu.test(value) ||
    /\b(?:paid services?|paid api|remote gpu|credentials?|mfa|billing|account decisions?|external approval|human decision)\b.{0,80}\b(?:hard stops?|blocked|forbidden|prohibited)\b/iu.test(value)
  );
}
...
function buildWatchtowerFeedEventFromDelegateEvent(projectPath, event, context) {
  ...
  const sourceRef = `delegate-event:${event.runId || "unknown"}:${event.id}`;
  return {
...
function buildSyntheticWatchtowerEvent(projectPath, kind, payload = {}) {
...
async function insertWatchtowerEventWithCard(projectPath, event) {
  ...
  await insertWatchtowerEvent(projectPath, event, cards);
  return { event, cards };
}

async function scanWatchtowerDelegateEvents(projectPath, context) {
  const runsDir = delegatePaths(projectPath).runsDir;
  let files = [];
  try {
  ...
  let eventCount = 0;
  let cardCount = 0;
  for (const file of files.filter((value) => value.endsWith(".jsonl")).sort()) {
    ...
    for (const event of page.events) {
      const feedEvent = buildWatchtowerFeedEventFromDelegateEvent(projectPath, event, context);
      const result = await insertWatchtowerEventWithCard(projectPath, feedEvent);
      eventCount += 1;
...
async function runWatchtowerScan(projectPath) {
  ...
  const [git, orp, status] = await Promise.all([
    ...
    readDelegateStatus(projectPath).catch(() => null),
  ]);
  const context = {
    git,
    orp,
  ...
  const delegateResult = await scanWatchtowerDelegateEvents(projectPath, context);
...
async function evaluateDelegateReviewGuard(projectPath, { delegateDecision = {} } = {}) {
  const scan = await runWatchtowerScan(projectPath);
  ...
  const allCards = await readWatchtowerReviewCards(projectPath, { limit: 80 });
...
function normalizeDelegateStatus(payload = {}) {
  ...
  const activeRequestId = pickString(payload.activeRequestId, payload.active_request_id) || null;
  ...
  return {
    ...
    activeRequestId,
    activeStep: normalizedActiveStep || (state === "running" && activeRequestId ? stepCount + 1 : null),
...
async function readDelegateCatalogStatus(projectPath) {
  try {
    return delegateStatusForProjectCatalog(
      await readDelegateStatus(projectPath, { reconcile: false }),
    );
  } catch (error) {
    console.warn(`[clawdad-server] ignoring delegate status for ${projectPath}: ${error.message}`);
    return null;
  }
}
...
async function stopDelegateRunFromDecision({ projectPath, config, status, decision, step, logRunEvent }) {
...
async function runDelegateLoop(
  ...
  try {
    ...
    for (let stepIndex = Math.max(0, Number.parseInt(String(latestStatus.stepCount || "0"), 10) || 0); ; stepIndex += 1) {
      ...
      const reviewGuard = await evaluateDelegateReviewGuard(projectPath, { delegateDecision: decision }).catch((error) => ({
        ok: false,
        hardStop: false,
      ...
      if (reviewGuard.hardStop || reviewGuard.pauseRecommended) {
      ...
      if (reviewGuard.repairRecommended) {
        latestStatus = await writeDelegateStatus(projectPath, {
          ...latestStatus,
          nextAction: reviewGuard.nextAction || latestStatus.nextAction,
          stopReason: null,
          error: "",
        });
        await logRunEvent("watchtower_hygiene_repair_queued", {
          title: "Watchtower hygiene repair queued",
          step: stepIndex + 1,
...
function currentWatchtowerReviewCards(cards = [], scan = {}) {
  const currentRunId = pickString(scan.runId);
  return cards.filter((card) => {
    const cardRunId = pickString(card.runId);
    return !cardRunId || !currentRunId || cardRunId === currentRunId;
  });
}
...
async function runWatchtower(argv) {
  ...
  const scanOnce = async () => {
    const scan = await runWatchtowerScan(resolved.projectPath);
    const allCards = await readWatchtowerReviewCards(resolved.projectPath, { limit: 80 });
    const cards = currentWatchtowerReviewCards(allCards, scan).slice(0, 12);
...
async function runFeed(argv) {
  ...
  if (mode === "review") {
    const cards = await readWatchtowerReviewCards(resolved.projectPath, { limit });
    if (rawOptions.json) {
  ...
  if (mode === "search") {
    ...
    const events = await searchWatchtowerFeed(resolved.projectPath, query, { limit });
    if (rawOptions.json) {
      console.log(JSON.stringify({
        ok: true,
        mode,
  ...
  const events = await readWatchtowerTail(resolved.projectPath, { limit });
  if (rawOptions.json) {
    console.log(JSON.stringify({
      ok: true,
      mode,
...
async function handleDelegateFeedGet(_req, res, options, url, actor) {
  ...
  try {
    ...
    const events = mode === "tail"
      ? await readWatchtowerTail(resolved.projectPath, { limit })
      : mode === "search"
        ? await searchWatchtowerFeed(resolved.projectPath, query, { limit })
        : [];
    const cards = mode === "review"
      ? await readWatchtowerReviewCards(resolved.projectPath, { limit })
      : await readWatchtowerReviewCards(resolved.projectPath, { limit: 12 });
    json(res, 200, {
      ok: true,
      actor,
      mode,
