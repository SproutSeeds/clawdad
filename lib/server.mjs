...
async function reconcileDelegateStatusFromCompletedMailbox(projectPath, status = {}, laneId = defaultDelegateLaneId) {
  ...
  const statusRequestIds = uniqueList([
    pickString(status.activeRequestId),
    pickString(status.lastRequestId),
  ]);
  if (mailboxRequestId && statusRequestIds.length > 0 && !statusRequestIds.includes(mailboxRequestId)) {
    return status;
  }
