  ...
  try {
    ...
    const unexpectedServeRoutes =
      expectedLiveRoutes.size > 0
        ? serveRoutes.filter((routeName) => !expectedLiveRoutes.has(routeName))
        : serveRoutes.slice(1);
    const missingLiveRoutes =
      expectedLiveRoutes.size > 0
        ? [...expectedLiveRoutes].filter((routeName) => !serveRoutes.includes(routeName))
        : [];
    if (expectedLiveRoutes.size > 0 && unexpectedServeRoutes.length === 0 && missingLiveRoutes.length === 0) {
      checks.push(
        formatCheck(
          "pass",
          "Serve surface",
          `configured routes match live app set: ${serveRoutes.join(", ")}`,
        ),
      );
    ...
    } else if (serveRoutes.length <= 1) {
      checks.push(
        formatCheck(
          "pass",
          "Serve surface",
          serveRoutes.length === 0 ? "no extra web routes" : `single web route: ${serveRoutes[0]}`,
        ),
      );
    } else {
      checks.push(
        formatCheck(
          "warn",
          "Serve surface",
          `multiple web routes configured: ${serveRoutes.join(", ")}`,
        ),
      );
    }
