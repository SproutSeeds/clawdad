...
async function runServe(argv) {
  ...
  assistantRuntime.appServer = new AssistantAppServer({root:path.join(assistantRuntime.root,'AppServer'),
    mayDispatch:job=>!assistantRuntime.state?.paused&&assistantRuntime.state?.jobs.find(j=>j.id===job.id)?.status!=='cancelled',
  ...
  await new Promise((resolve, reject) => {
    ...
    server.once("close", () => { weeklyUsage.stop(); speechRecovery.stop(); void assistantRuntime.close(); });
