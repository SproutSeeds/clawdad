...
Usage:
  ...
  clawdad delegate-run [project] [--lane <laneId>] [--json]
...
async function evaluateDelegateReviewGuard(projectPath, { delegateDecision = {}, laneId = defaultDelegateLaneId } = {}) {
  const normalizedLaneId = normalizeDelegateLaneId(laneId);
...
async function readDelegateStatus(projectPath, { reconcile = false, laneId = defaultDelegateLaneId } = {}) {
  ...
  if (status.state === "running" && delegateRunJobs.has(jobKey)) {
...
async function activeDelegateLaneSummaries(projectPath, { excludeLaneId = "" } = {}) {
  ...
  return lanes.filter((lane) => {
    ...
    return ["planning", "running"].includes(String(status.state || "").trim().toLowerCase()) && Boolean(status.live);
  });
}
...
async function runTrackedSessionDispatchWait(
  ...
  sessionId,
  message,
  { permissionMode = "approve", model = "", onEvent = null, liveRunId = "", liveStep = null, laneId = defaultDelegateLaneId } = {},
) {
  const normalizedLaneId = normalizeDelegateLaneId(laneId);
...
async function waitForMailboxRequestStart(projectPath, previousRequestId = "", timeoutMs = 3000, laneId = null) {
  const startedAt = Date.now();
