...
async function handleTtsMessageCreate(req, res, options, actor) {
  ...
  if (!apiKey) {
    json(res, 409, {
      ...
      project: resolved.projectPath,
      error:
        "ElevenLabs API key is not configured. Set ELEVENLABS_API_KEY, CLAWDAD_ELEVENLABS_API_KEY, or add a Keychain item named clawdad-elevenlabs.",
    });
    return;
  }
  ...
  try {
    ...
    const statusCode =
      /too long|missing text|disabled|unsupported/iu.test(error.message)
        ? 400
        : /ElevenLabs|HTTP|API key/iu.test(error.message)
          ? 502
          : 500;
    json(res, statusCode, {
      ok: false,
