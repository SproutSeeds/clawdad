...
async function handleProjectSummaryGet(_req, res, options, url, actor) {
  ...
  try {
    const snapshots = await readProjectSummarySnapshots(resolved.projectPath);
    json(res, 200, {
      ok: true,
      actor,
...
async function handleProjectSummaryCreate(req, res, options, actor) {
  ...
  try {
    const result = await generateProjectSummarySnapshot(
      resolved.projectDetails,
      resolved.sessionDetails,
    );
    ...
    json(res, 200, {
      ...
      output: result.snapshot.summary,
    });
  } catch (error) {
    json(res, 500, {
      ok: false,
