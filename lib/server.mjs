...
function mailboxRunningAgeMs(status = {}) {
  const dispatchedAtMs = Date.parse(pickString(status.dispatched_at, status.dispatchedAt));
...
function staleMailboxStatusReason(status = {}) {
  ...
  const ageMs = mailboxRunningAgeMs(status);
  if (
    ...
    return `Dispatch has been running for about ${ageMinutes} minutes, beyond the ${limitMinutes} minute safety limit.`;
  }
...
Artifact handoff: If you create a deliverable file the user may need to download or share, save it under ${projectArtifactsDir(project.path)} using a clear filename. Create that folder if needed. Mention the saved filename in your response.
...
\`\`\`json
{"state":"continue|blocked|completed","stop_reason":"none|paid|needs_human|compute_limit","next_action":"short string","summary":"short string"}
\`\`\`
...
async function runTrackedSessionDispatchWait(
  ...
  sessionId,
  message,
  { permissionMode = "approve", model = "", onEvent = null } = {},
) {
  ...
  try {
    ...
    for (let stepIndex = Math.max(0, Number.parseInt(String(latestStatus.stepCount || "0"), 10) || 0); ; stepIndex += 1) {
      ...
      await logRunEvent("agent_response", {
        ...
        text: dispatchResult.responseText,
      });
...
async function pauseDelegateRun(projectDetails) {
  ...
  await appendDelegateRunEvent(projectDetails.path, nextStatus.runId, "pause_requested", {
    title: isActive ? "Pause requested" : "Delegate paused",
    text: isActive
...
async function projectCatalogFromStateProjects(stateProjects = {}) {
  ...
  return disambiguateProjectDisplayNames(
    projectEntries
      .filter(Boolean)
      .map(([projectPath]) => projectSummaryFromTabs(projectPath, [], stateProjects))
