...
async function handleProjects(_req, res, options, actor) {
  try {
    let cachedProjects = await loadProjectCatalogCached();
    const importedCount = await autoRegisterImportableCodexSessionsForProjects(cachedProjects);
    if (importedCount > 0) {
      cachedProjects = await loadProjectCatalogCached({ allowStale: false });
    }
    ...
    json(res, 200, {
      ...
      autoImportedSessionCount: importedCount,
      projects,
    });
  } catch (error) {
    json(res, 500, {
...
async function autoRegisterImportableCodexSessionsForProject(projectDetails = {}, {
  ...
  let importable = [];
  try {
    importable = await listImportableCodexSessionsForProject(projectDetails, { limit });
  } catch (error) {
