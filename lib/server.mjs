...
function dispatchScheduleModeFromPayload(payload = {}) {
  ...
  if (["interject", "interrupt", "steer"].includes(mode)) {
    return "interject";
  }
  return "linear";
}
...
async function startQueuedDispatchWorker(item) {
  ...
  const child = spawn(process.execPath, [dispatchQueueWorkerPath, "--item-file", itemFile], {
    cwd: clawdadRoot,
    detached: true,
    stdio: "ignore",
    env: {
  ...
  child.once("error", (error) => {
    console.warn(`[clawdad-server] failed to start queued dispatch worker: ${error.message}`);
  });
  child.unref();
  return {
    itemFile,
...
async function readProviderHistory(projectPath, session, { tailBytes = 0 } = {}) {
...
async function readSessionHistoryPage(projectPath, session, { cursor = 0, limit = 20, providerTailBytes = 0 } = {}) {
...
function classifySessionDoctorIssue(type, details = {}) {
  ...
  const active = Boolean(
    details.active ||
      ...
      ["project_missing", "project_inaccessible", "sandbox_host_access", "active_session_quarantined", "active_session_failed", "stale_failed_mailbox", "busy_session_status", "delegate_supervisor_not_live", "delegate_dispatch_stalled"].includes(type),
  );
...
function sessionTerminalCommandParts(session = {}) {
...
function sessionTerminalShellCommand(projectPath, session = {}) {
...
async function launchConfiguredTerminal(projectPath, session, shellCommand) {
  ...
  return runExec(launcher, [], {
    input: `${JSON.stringify({
      projectPath,
      provider: commandParts.provider,
    ...
    env: {
      ...
      CLAWDAD_TERMINAL_PROVIDER: commandParts.provider,
      CLAWDAD_TERMINAL_SESSION_ID: pickString(session.sessionId),
      CLAWDAD_TERMINAL_SESSION_LAUNCH_MODE: commandParts.launchMode || "resume",
...
async function launchSessionTerminal(projectPath, session) {
  const shellCommand = sessionTerminalShellCommand(projectPath, session);
  const configuredResult = await launchConfiguredTerminal(projectPath, session, shellCommand);
  if (configuredResult) {
    return {
      ...configuredResult,
      launcher: "configured",
  ...
  if (mailboxBusy) {
    if (scheduleMode === "linear") {
      json(res, 409, {
        ...
        error: "This session is busy. Switch the composer mode to Queue or Interject to send another message.",
        reason: "session_busy",
      });
      return;
    }
    ...
    const activeMailboxSessionId = pickString(baselineStatus.session_id, baselineStatus.sessionId);
    ...
    const canInterject = scheduleMode === "interject" && provider === "codex" && interjectTargetsActiveSession;
    const effectiveScheduleMode = canInterject ? "interject" : "queue";
    ...
    json(res, 202, {
      ...
      requestState: effectiveScheduleMode === "interject" ? "interjected" : "queued",
      handoffPending: false,
      mailboxStatus: baselineStatus,
      queued: effectiveScheduleMode === "queue",
      interjected: effectiveScheduleMode === "interject",
      dispatchMode: effectiveScheduleMode,
      interjectionFallbackReason: scheduleMode === "interject" && !canInterject
        ? provider !== "codex"
  ...
  if (requestId) {
    ...
    if (historyItem) {
      ...
      const matchedSession =
        Array.isArray(matchedProject?.sessions)
          ? matchedProject.sessions.find(
              (session) => session.sessionId === historySessionId || session.slug === historySessionId,
            )
          : null;
...
function classifyTtsError(error = null) {
  ...
  if (/doc reader.*not configured|local speech.*not configured/iu.test(message)) {
    return {
      ...
      error: "Local text-to-speech is not configured with a Doc Reader service URL.",
      retryAfterMs: ttsUnavailableRetryMs,
      unavailable: true,
      statusCode: 409,
    };
  ...
  if (/doc reader|local speech|fetch failed|econnrefused|econnreset|enotfound/iu.test(message)) {
    return {
      ...
      error: "Local text-to-speech service is unavailable. Check Doc Reader's 4090/Mac speech services and try again.",
      retryAfterMs: ttsUnavailableRetryMs,
      unavailable: true,
      statusCode: 503,
    };
...
async function currentTtsAvailabilityStatus(config = {}) {
  const ttsConfig = resolveTtsRuntimeConfig({ config });
  ...
  if (!["doc-reader", "openai"].includes(provider)) {
    return {
      enabled: true,
      provider,
      available: false,
  ...
  if (provider === "doc-reader") {
    const library = await checkDocReaderLibraryService(ttsConfig.libraryUrl, {
      engine: pickString(ttsConfig.engine, ttsConfig.modelId, "kokoro"),
    });
    if (library.available) {
      return {
        enabled: true,
        provider,
        label: library.degraded ? "Doc Reader local speech fallback" : "Doc Reader local speech",
        available: true,
        ...
        engine: pickString(ttsConfig.engine, ttsConfig.modelId, "kokoro"),
        outputFormat: pickString(ttsConfig.outputFormat, "wav"),
    ...
    return {
      enabled: true,
      provider,
      label: "Doc Reader local speech",
      available: false,
      degraded: false,
      error: library.error || "Doc Reader local speech is unavailable. Start Doc Reader and check its 4090/Mac speech services.",
      errorCode: "local_service_unavailable",
      retryAfterMs: 0,
      unavailableUntil: null,
      updatedAt: ttsAvailabilityState.updatedAt || null,
  ...
  return {
    enabled: true,
    provider,
    label: provider === "openai" ? "OpenAI speech" : "Text-to-speech",
    available: true,
    error: "",
    errorCode: "",
    retryAfterMs: 0,
...
async function ttsStatusPayload(config = {}) {
  return currentTtsAvailabilityStatus(config);
}
...
async function resolveSessionTerminalLogTarget({
  ...
  const [historyItem, mailboxStatus] = await Promise.all([
...
async function handleTtsMessageCreate(req, res, options, actor) {
  ...
  if (!ttsConfig.enabled) {
    ...
    const ttsStatus = await ttsStatusPayload({ tts: ttsConfig });
    json(res, 403, {
      ok: false,
      actor,
      project: resolved.projectPath,
  ...
  const unavailable = await ttsUnavailableResponsePayload({ tts: ttsConfig });
  if (unavailable) {
    json(res, ttsUnavailableStatusCode(unavailable.errorCode), {
      ok: false,
      actor,
      project: resolved.projectPath,
      ...
      unavailableUntil: unavailable.unavailableUntil,
      ttsStatus: unavailable.status,
    });
    return;
  }
  ...
  if (ttsConfig.provider === "openai") {
    try {
      apiKey = await resolveOpenAiApiKey({ projectPath: resolved.projectPath });
    } catch (_error) {
      apiKey = "";
    }
    if (!apiKey) {
      const classified = markTtsUnavailableForError(new Error(
        "OpenAI API key is not configured. Set OPENAI_API_KEY, CLAWDAD_OPENAI_API_KEY, add a Keychain item named clawdad-openai, or store it in ORP secrets.",
      ));
      const ttsStatus = await ttsStatusPayload({ tts: ttsConfig });
      json(res, 409, {
        ok: false,
...
async function handleSttTranscribe(req, res, options, actor) {
  ...
  if (!["doc-reader", "openai"].includes(sttConfig.provider)) {
    json(res, 400, {
      ok: false,
      actor,
      error: `unsupported speech-to-text provider '${sttConfig.provider}'`,
    });
    return;
  }
  ...
  if (sttConfig.provider === "openai") {
    try {
      apiKey = await resolveOpenAiApiKey({ projectPath: resolvedProjectPath });
    } catch (_error) {
      apiKey = "";
    }
    if (!apiKey) {
...
async function handleSessionTerminalOpen(req, res, options, actor) {
  ...
  json(res, 200, {
    ...
    provider: matchedSession.provider || "codex",
    launcher: launchResult.launcher || null,
    launchMode: sessionTerminalCommandParts(matchedSession).launchMode || "resume",
  });
}
