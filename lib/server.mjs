...
function historyEntriesLikelyDuplicate(left, right) {
  ...
  const leftResponse = normalizedHistoryText(left?.response);
  const rightResponse = normalizedHistoryText(right?.response);

  if (!leftMessage || !rightMessage || !leftResponse || !rightResponse) {
    return false;
  }
  ...
  const leftTimestamp = historyItemTimestampMs(left);
  const rightTimestamp = historyItemTimestampMs(right);
  if (leftTimestamp > 0 && rightTimestamp > 0) {
    return Math.abs(leftTimestamp - rightTimestamp) <= 90_000;
  }
...
async function readSessionHistoryPage(projectPath, session, { cursor = 0, limit = 20 } = {}) {
  ...
  if (providerItems.length > 0) {
    if (mirroredItems.length === 0) {
      ...
      const distinctProviderItems = providerItems.filter(
        (providerEntry) =>
          !mirroredItems.some((mirroredEntry) =>
            historyEntriesLikelyDuplicate(providerEntry, mirroredEntry),
          ),
      );

      combinedItems = [...mirroredItems, ...distinctProviderItems];
    }
  }
...
async function readProjectSummaryStatus(projectPath, { reconcile = true } = {}) {
  const { statusFile } = projectSummaryPaths(projectPath);
...
function extractClaudeSummaryText(output) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      if (typeof result?.result === "string" && result.result.trim()) {
        return trimTrailingNewlines(result.result);
      }
    } else if (typeof parsed?.result === "string" && parsed.result.trim()) {
      return trimTrailingNewlines(parsed.result);
    }
  } catch (_error) {
  ...
  for (const line of lines) {
    try {
      ...
      if (parsed?.type === "result" && typeof parsed.result === "string" && parsed.result.trim()) {
        resultEvents.push(parsed.result);
      }
    } catch (_error) {
      // Ignore mixed non-JSON lines.
    }
  }

  if (resultEvents.length > 0) {
    return trimTrailingNewlines(resultEvents[resultEvents.length - 1]);
  }
...
async function runClaudeProjectSummary(projectPath, prompt) {
  ...
  return extractClaudeSummaryText(result.stdout);
}
...
async function generateProjectSummarySnapshot(project, session) {
  ...
  const provider = String(session?.provider || "claude").trim().toLowerCase();
  let summaryText = "";
