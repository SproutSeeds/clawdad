...
async function runServe(argv) {
  ...
  const assistantRuntime = new AssistantRuntime({notificationIdentity:()=>readHostNotificationIdentity(
  ...
  const codexAccounts=new CodexAccounts({usage:weeklyUsage,inspectConsumers:()=>inspectAccountConsumers(assistantRuntime),
    readWork:async()=>{
    ...
    authorizations:process.platform==='darwin'?new CodexAccountAuthorizations({root:codexAccountsRoot(),binary:defaultCodexBinary}):null});
  ...
  accountLegacyWork=new CodexAccountLegacyWork({root:path.join(codexAccountsRoot(),'ProjectWork'),accounts:codexAccounts,
  ...
  if(accountSwitchRuntime)accountShellLaunches=new CodexAccountShellLaunches({root:path.join(codexAccountsRoot(),'ShellLaunches'),accounts:codexAccounts,
