  ...
  return trimTrailingNewlines(
    [
      ...
      status?.computeBudget?.remainingPercent !== undefined
        ...
        : "",
      "Autonomy: continue local/free work until semantic completion, explicit pause, compute reserve, or hard stop.",
    ]
      .filter(Boolean)
      .join("\n"),
  );
...
async function syncDelegateCodexGoalStatus(projectPath, status, nextGoalStatus, {
  ...
  try {
    ...
    return normalizeDelegateCodexGoal({
      ...localGoal,
      synced: false,
      error: error || syncError.message || "Codex goal sync failed",
      updatedAt: new Date().toISOString(),
    }) || localGoal;
  }
}
