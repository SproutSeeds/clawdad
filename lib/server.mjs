...
async function scanWatchtowerDelegateEvents(projectPath, context, laneId = defaultDelegateLaneId) {
  ...
  for (const file of files.filter((value) => value.endsWith(".jsonl"))) {
    ...
    runFiles.push({ file, filePath, signature });
  }
  ...
  for (const entry of runFiles) {
    const runId = entry.file.replace(/\.jsonl$/u, "");
    const stateKey = watchtowerDelegateRunScanStateKey(laneId, runId);
    const previousSignature = await readWatchtowerScanState(projectPath, stateKey).catch(() => "");
    const isCurrentRun = currentRunId && safeDelegateRunId(runId) === currentRunId;
    if (entry.signature && previousSignature === entry.signature && !isCurrentRun) {
      skippedRunCount += 1;
      continue;
    }
    if (!previousSignature && !isCurrentRun) {
      const alreadyIndexed = await watchtowerDelegateRunHasIndexedEvents(projectPath, laneId, runId).catch(() => false);
      if (alreadyIndexed) {
        if (entry.signature) {
          await writeWatchtowerScanState(projectPath, stateKey, entry.signature).catch(() => {});
        }
        skippedRunCount += 1;
        continue;
      }
