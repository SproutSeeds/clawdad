...
async function withStateLock(work) {
  ...
  while (true) {
    try {
      ...
      await writeAtomicTextFile(stateLockOwnerPath(), `${process.pid} ${Math.floor(Date.now() / 1000)}\n`);
      break;
    } catch (error) {
...
function projectSessionAutoImportReady(projectDetails = {}, { force = false, now = Date.now() } = {}) {
  const projectPath = pickString(projectDetails?.path);
  if (!projectPath) {
    return false;
  }
  if (!force && !projectNeedsCodexSessionAutoImport(projectDetails)) {
    return false;
  }
...
async function autoRegisterImportableCodexSessionsForProject(projectDetails = {}, {
  ...
  const now = Date.now();
  if (!projectSessionAutoImportReady(projectDetails, { force, now })) {
    return 0;
  }
