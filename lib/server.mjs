...
async function runServe(argv) {
  ...
  const codexAccounts=new CodexAccounts({usage:weeklyUsage,inspectConsumers:()=>inspectAccountConsumers(assistantRuntime),
    readWork:()=>readAccountWork(assistantRuntime),
  ...
  const handleRequest = async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/v1/dispatch") {
      try{await codexAccounts.assertAdmission();}catch(error){json(res,409,{ok:false,error:error.message,reasonCode:'account_switch_pending'});return;}
  ...
  await new Promise((resolve, reject) => {
    ...
    server.once("close", () => { weeklyUsage.stop(); speechRecovery.stop(); void codexAccounts.stop(); void assistantRuntime.close(); });
