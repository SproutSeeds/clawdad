...
import {CodexWeeklyUsageMonitor, readCodexAccountUsage} from './codex-weekly-usage.mjs';
import {CodexAppAccounts as CodexAccounts,codexAccountsRoot} from './codex-app-accounts.mjs';
...
import {readAccountWork} from './codex-app-account-work.mjs';
import {ensureAccountSharedRuntime} from './codex-account-runtime-routing.mjs';
import {connectCodexAppAccounts} from './codex-app-account-runtime.mjs';
import {CodexAppAccountStatus} from './codex-app-account-status.mjs';
import https from "node:https";
import os from "node:os";
import { AssistantRuntime, assistantHttp } from "./assistant-runtime.mjs";
import { ResearchSupervisor } from './research-supervisor.mjs';
import { ResearchBudget } from './research-budget.mjs';
import { ResearchReviewRunner } from './research-evidence.mjs';
import {AgentAccessSettings} from './agent-access-settings.mjs';
import {CodexSharedTurnRunner} from './codex-shared-turn-runner.mjs';
import {CodexAuxiliaryTasks} from './codex-auxiliary-tasks.mjs';
import {indexedCodexTranscriptPath,liveCodexProjectBinding} from './codex-transcript-location.mjs';
import {CodexSharedClient} from './codex-thread-control.mjs';
import path from "node:path";
import readline from "node:readline";
import { pipeline as fileLibraryPipeline } from "node:stream/promises";
import { LocalFileLibrary, runFilesCommand } from "./local-file-library.mjs";
import { LocalImageInbox } from "./local-image-inbox.mjs";
import { voiceSettings, runtimeWithVoice, readVoicePreferences, localVoiceServiceUrl } from "./local-voices.mjs";
...
import {LocalSpeechRecovery} from "./local-speech-recovery.mjs";
import {AssistantAppServer} from './assistant-app-server.mjs';
...
import {AssistantModelSettings} from './assistant-model-settings.mjs';
import {
...
const privateRuntimeRoots = serverRuntimeRoots();
const clawdadBin =
...
const babyMascotAssetPath = [
  path.join(clawdadRoot, "assets", "clawdad-baby-mascot.png"),
  path.join(
    clawdadRoot,
    "apps",
    "ios",
...
}) || path.join(clawdadRoot, "assets", "clawdad-mascot-app.png");
...
let codexAccountRuntimeControls = null;
let nativeCodexAuxiliaryTasks = null;
...
const runtimeHomeMutationCompatible = (() => {
  if (boolFromUnknown(process.env.CLAWDAD_ALLOW_MIXED_RUNTIME_HOMES, false)) {
    return true;
  }
  const explicitCodexHome = pickString(process.env.CLAWDAD_CODEX_HOME);
  if (!explicitCodexHome) {
    return true;
  }
  const userCodexHome = path.join(os.homedir(), ".codex");
  if (path.resolve(explicitCodexHome) === path.resolve(userCodexHome)) {
    return true;
  }
  const explicitClawdadHome = pickString(process.env.CLAWDAD_HOME);
  if (!explicitClawdadHome) {
    return false;
  }
  const userClawdadHome = path.join(os.homedir(), ".clawdad");
  return path.resolve(explicitClawdadHome) !== path.resolve(userClawdadHome);
})();
let runtimeHomeMutationWarningEmitted = false;

function allowAutomaticRuntimeStateMutation(context = "automatic state mutation") {
  if (runtimeHomeMutationCompatible) {
    return true;
  }
  if (!runtimeHomeMutationWarningEmitted) {
    runtimeHomeMutationWarningEmitted = true;
    console.warn(
      `[clawdad-server] ${context} disabled because CLAWDAD_CODEX_HOME points at a non-default Codex home without a separate isolated CLAWDAD_HOME. Set both homes to isolated paths or CLAWDAD_ALLOW_MIXED_RUNTIME_HOMES=1.`,
    );
  }
  return false;
}
...
const queuedDispatchResumeDisabled = ["1", "true", "yes", "on"].includes(
  String(process.env.CLAWDAD_DISABLE_QUEUED_DISPATCH_RESUME || "").trim().toLowerCase(),
);
...
function scheduleQueuedDispatchPumpResume() {
  if (
    queuedDispatchResumeDisabled ||
    !allowAutomaticRuntimeStateMutation("queued dispatch resume")
  ) {
    return;
  }
...
function normalizeHistoryAudioManifest(manifest = {}, projectPath = "") {
  ...
  return {
    ...
    engine: pickString(manifest.engine) || null,
    ...
    parts: parts
      .map((part) => {
        ...
        return {
          ...
          textHash: pickString(part?.textHash) || null,
          audioHash: pickString(part?.audioHash) || null,
...
async function findProviderTranscriptPath(provider, sessionId) {
  ...
  const indexedPath=normalizedProvider==='codex'?await indexedCodexTranscriptPath(defaultCodexHome,normalizedSessionId):'';
  const filePath = indexedPath || await findTranscriptFileRecursive(rootDir, (name) => {
...
async function validateCodexSessionProjectBinding(projectPath, sessionId) {
  ...
  const filePath = await indexedCodexTranscriptPath(defaultCodexHome,normalizedSessionId) || await findTranscriptFileRecursive(rootDir, (name) => (
    name.endsWith(".jsonl") && name.includes(normalizedSessionId)
  ));
  if (!filePath) {
    if(codexSharedRuntimeState?.ready&&codexSharedRuntimeState.mode==='shared'){
      const client=new CodexSharedClient({socketPath:codexAppServerSocket});
      try{const live=await liveCodexProjectBinding(client,normalizedSessionId,normalizedProjectPath);if(live)return live;}
...
async function runCodexProjectSummary(projectPath, prompt) {
  if(process.env.CLAWDAD_NATIVE_RUNTIME_VERSION){
    if(!nativeCodexAuxiliaryTasks)throw Error('The subscription app server is not ready for project summaries.');
    return nativeCodexAuxiliaryTasks.run('project_summary',projectPath,prompt);
  }
...
async function runCodexDelegatePlan(projectPath, prompt) {
  if(process.env.CLAWDAD_NATIVE_RUNTIME_VERSION){
    if(!nativeCodexAuxiliaryTasks)throw Error('The subscription app server is not ready for delegate plans.');
    return nativeCodexAuxiliaryTasks.run('delegate_plan',projectPath,prompt);
  }
...
async function runCodexDelegateRunSummary(projectPath, prompt) {
  if(process.env.CLAWDAD_NATIVE_RUNTIME_VERSION){
    if(!nativeCodexAuxiliaryTasks)throw Error('The subscription app server is not ready for run summaries.');
    return nativeCodexAuxiliaryTasks.run('run_summary',projectPath,prompt);
  }
...
function artifactHandoffPrompt(project) {
  ...
  const command = `${quote(process.execPath)} ${quote(fileURLToPath(import.meta.url))} files add`;
  return `Artifact handoff: Save final requested deliverables under ${path.join(project.path, ".clawdad", "deliverables")} using clear filenames. Register each final file in the local ClawDad Files library with ${command} /absolute/path/to/file --project ${quote(project.path)}. Mention its filename in your response. Register only finished requested deliverables; ordinary source edits and temporary files stay outside Files. The Mac keeps the canonical snapshot and paired phones download on demand.`;
}
...
async function ensureScratchpadProject(workspace = {}) {
  if (!allowAutomaticRuntimeStateMutation("scratchpad bootstrap")) {
    return false;
  }
...
const sessionDoctorRepairableIssueTypes = new Set([
  ...
  "codex_session_false_quarantine",
...
async function restoreFalseQuarantinedCodexSession(
  projectPath,
  sessionId,
  validation = {},
) {
  const statePayload = await readSessionDoctorStatePayload();
  const projectState = statePayload.projects?.[projectPath] || {};
  const aliases = normalizeProjectSessionAliases(projectState.session_aliases);
  const replacementSessionId = pickString(aliases[sessionId]);
  const replacementState = replacementSessionId
    ? projectState.sessions?.[replacementSessionId] || {}
    : {};
  const replacementHistory = replacementSessionId
    ? await readMirroredSessionHistory(projectPath, replacementSessionId).catch(() => [])
    : [];
  const replacementIsEmptyPlaceholder = Boolean(
    replacementSessionId &&
      pickString(replacementState.provider, "codex").toLowerCase() === "codex" &&
      pickString(replacementState.provider_session_seeded, "false") !== "true" &&
      (Number.parseInt(String(replacementState.dispatch_count || "0"), 10) || 0) === 0 &&
      !pickString(replacementState.last_dispatch) &&
      !pickString(replacementState.last_response) &&
      replacementHistory.length === 0
  );
  const restoredAt = new Date().toISOString();

  const updatedProject = await updateSessionDoctorStateProject(projectPath, (currentProject) => {
    const currentSessions =
      currentProject.sessions && typeof currentProject.sessions === "object"
        ? currentProject.sessions
        : {};
    ...
    const {
      ...
      quarantine_detail: _quarantineDetail,
    ...
    const nextSessions = {
      ...
      [sessionId]: {
        ...
        provider_transcript_path: pickString(
          validation.filePath,
          restoredSession.provider_transcript_path,
        ) || null,
        provider_session_source: pickString(
          validation.source,
          restoredSession.provider_session_source,
        ) || null,
        status: sessionDoctorState(restoredSession.status) === "failed"
          ? "idle"
          : pickString(restoredSession.status, "idle"),
      },
    };
    const nextQuarantined = {
      ...(currentProject.quarantined_sessions || {}),
    };
    delete nextQuarantined[sessionId];
    const nextAliases = normalizeProjectSessionAliases(currentProject.session_aliases);
    delete nextAliases[sessionId];

    let nextActiveSessionId = pickString(currentProject.active_session_id);
    if (replacementIsEmptyPlaceholder && replacementSessionId) {
      const placeholder =
        nextSessions[replacementSessionId] && typeof nextSessions[replacementSessionId] === "object"
          ? nextSessions[replacementSessionId]
          : {};
      const detail = `Superseded by restored Codex session ${sessionId}.`;
      nextSessions[replacementSessionId] = {
        ...placeholder,
        status: "failed",
        quarantined: "true",
        quarantine_reason: "false_quarantine_placeholder",
        quarantine_detail: detail,
        quarantined_at: restoredAt,
      };
      nextQuarantined[replacementSessionId] = {
        ...placeholder,
        status: "failed",
        reason: "false_quarantine_placeholder",
        detail,
        quarantined_at: restoredAt,
      };
      for (const [alias, target] of Object.entries(nextAliases)) {
        if (target === replacementSessionId) {
          nextAliases[alias] = sessionId;
        }
      }
      nextAliases[replacementSessionId] = sessionId;
      if (nextActiveSessionId === replacementSessionId) {
    ...
    return {
      ...currentProject,
      status: nextActiveSessionId === sessionId
        ? pickString(nextSessions[sessionId]?.status, "idle")
        : pickString(currentProject.status, "idle"),
      active_session_id: nextActiveSessionId || sessionId,
      sessions: nextSessions,
      quarantined_sessions: nextQuarantined,
      session_aliases: nextAliases,
    };
  });
  ...
  return {
    ...
    sessionId,
    replacementSessionId: replacementSessionId || null,
    placeholderRetired: replacementIsEmptyPlaceholder,
  };
}

const autoRecoverableCodexBindingReasons = new Set([
  "missing_transcript",
  "session_id_mismatch",
  "cwd_mismatch",
  "unsupported_source",
...
function codexAuthenticationRecovery(text = "") {
  const detail = String(text || "").trim();
  if (!detail) {
    return null;
  }
  if (!(
    /refresh token was already used/iu.test(detail) ||
    /access token could not be refreshed/iu.test(detail) ||
    /(?:authentication|authorization|auth) (?:failed|required|expired)/iu.test(detail) ||
    /(?:please|must) (?:log|sign) (?:out and )?(?:log|sign) in again/iu.test(detail) ||
    /(?:invalid|expired) (?:access|refresh) token/iu.test(detail)
  )) {
    return null;
  }
  return {
    kind: "codex_reauthentication",
    action: "sign_in_again",
    message: "Codex needs a fresh ChatGPT sign-in on this computer.",
  };
}

async function recoverInvalidActiveCodexSession(
  projectPath,
  projectDetails,
  session,
  validation,
  ...
  if (!allowAutomaticRuntimeStateMutation("stale Codex session recovery")) {
    const error = new Error(
      "ClawDad refused to rewrite the session registry because the configured Codex and ClawDad homes are not isolated together.",
    );
    error.code = "CLAWDAD_RUNTIME_HOME_MISMATCH";
    throw error;
  }
  const sessionId = pickString(session?.sessionId);
  const activeSessionId = pickString(
    projectDetails?.activeSessionId,
    projectDetails?.activeSession?.sessionId,
  );
  const provider = pickString(session?.provider, "codex").toLowerCase();
  if (
    !sessionId ||
    ...
    provider !== "codex" ||
    !sessionProviderSessionSeeded(session) ||
    validation?.ok !== false ||
    !autoRecoverableCodexBindingReasons.has(pickString(validation?.reason))
  ) {
    return null;
  }
  ...
  await quarantineSessionDoctorSession(
    ...
    pickString(validation?.message, "The active Codex session binding is no longer usable."),
  );
  const registration = await registerProjectSessionLocally(projectPath, {
    slug: pickString(session?.slug, session?.title, path.basename(projectPath)),
    provider: "codex",
  ...
  await updateSessionDoctorStateProject(projectPath, (currentProject) => ({
    ...currentProject,
    session_aliases: {
      ...normalizeProjectSessionAliases(currentProject.session_aliases),
      [sessionId]: registration.sessionId,
    },
  }));
  invalidateProjectCatalogCache();
  const refreshedProjects = await loadProjectCatalogCached({ allowStale: false });
  const refreshedProject = refreshedProjects.find((entry) => entry.path === projectPath) || null;
  const refreshedSession = findProjectSession(refreshedProject, registration.sessionId) ||
    refreshedProject?.activeSession ||
    null;
  return {
    kind: "stale_active_session",
    source,
    reason: pickString(validation?.reason, "invalid_session_binding"),
    previousSessionId: sessionId,
    sessionId: registration.sessionId,
    message: "ClawDad replaced a stale local thread binding with a fresh Codex thread.",
    projectDetails: refreshedProject,
...
async function inspectSessionDoctorProject(projectPath, stateProjects = {}, { repair = false } = {}) {
  ...
  for (const [sessionId, sessionState] of Object.entries(sessions)) {
    ...
    if (quarantined) {
      const quarantineRecord = stateEntry.quarantined_sessions?.[sessionId] || {};
      const quarantineReason = pickString(
        sessionState?.quarantine_reason,
        quarantineRecord?.reason,
      );
      const provider = pickString(sessionState?.provider).toLowerCase();
      const providerSeeded = pickString(
        sessionState?.provider_session_seeded,
        "true",
      ) === "true";
      if (
        provider === "codex" &&
        providerSeeded &&
        quarantineReason === "codex_session_not_found_for_project"
      ) {
        const binding = await validateCodexSessionProjectBinding(projectPath, sessionId);
        if (binding.ok) {
          const aliasTarget = pickString(
            normalizeProjectSessionAliases(stateEntry.session_aliases)[sessionId],
          );
          const effectivelyActive =
            sessionId === activeSessionId ||
            Boolean(aliasTarget && aliasTarget === activeSessionId);
          const issue = addIssue(
            "codex_session_false_quarantine",
            `Quarantined Codex session ${sessionId} has a valid saved transcript for this project and can be restored safely.`,
            {
              sessionId,
              cwd: binding.cwd || null,
              active: effectivelyActive,
              replacementSessionId: aliasTarget || null,
            },
          );
          if (repair) {
            const restored = await restoreFalseQuarantinedCodexSession(
              projectPath,
              sessionId,
              binding,
            );
            addRepair(
              "codex_session_false_quarantine_restored",
              restored.placeholderRetired
                ? `Restored ${sessionId} and retired empty replacement ${restored.replacementSessionId}.`
                : `Restored ${sessionId} from an incorrect quarantine.`,
              {
                sessionId,
                replacementSessionId: restored.replacementSessionId,
                placeholderRetired: restored.placeholderRetired,
              },
            );
            updateReportedSession(sessionId, {
              status: "idle",
              active: effectivelyActive,
              quarantined: false,
            });
            projectReport.quarantinedSessions = projectReport.quarantinedSessions.filter(
              (candidate) => candidate !== sessionId,
            );
            if (
              restored.placeholderRetired &&
              restored.replacementSessionId &&
              !projectReport.quarantinedSessions.includes(restored.replacementSessionId)
            ) {
              projectReport.quarantinedSessions.push(restored.replacementSessionId);
            }
            markResolved(issue);
          }
        }
...
async function runSharedCodexCli(argv = []) {
  ...
  const runtime = await ensureAccountCodexSharedRuntime({
    mode: requestedMode,
...
async function runCodexRuntime(argv = []) {
  ...
  if (["ensure", "start"].includes(subcommand)) {
    result = await ensureAccountCodexSharedRuntime(runtimeOptions);
...
async function maybeServeApp(req, res, url) {
  ...
  const appAssets = {
    ...
    "/files-library.js": path.join(webAppRoot, "files-library.js"),
    "/files-library.css": path.join(webAppRoot, "files-library.css"),
    "/assistant.js": path.join(webAppRoot, "assistant.js"),
    "/assistant-settings.js": path.join(webAppRoot, "assistant-settings.js"),
    "/agent-access-settings.js": path.join(webAppRoot, "agent-access-settings.js"),
    "/research-supervisor.js": path.join(webAppRoot, "research-supervisor.js"),
    "/research-budget.js": path.join(webAppRoot, "research-budget.js"),
    "/weekly-usage.js": path.join(webAppRoot, "weekly-usage.js"),
    "/codex-accounts.js": path.join(webAppRoot, "codex-accounts.js"),
    "/main-terminal-workspace.js": path.join(webAppRoot, "main-terminal-workspace.js"),
    "/main-terminal-workspace-state.mjs": path.join(webAppRoot, "main-terminal-workspace-state.mjs"),
    "/main-terminal-workspace.css": path.join(webAppRoot, "main-terminal-workspace.css"),
    "/assistant.css": path.join(webAppRoot, "assistant.css"),
    "/assistant-audio-worklet.js": path.join(webAppRoot, "assistant-audio-worklet.js"),
    "/speech-output.js": path.join(webAppRoot, "speech-output.js"),
    "/speech-output-dsp.js": path.join(webAppRoot, "speech-output-dsp.js"),
    "/speech-output-worklet.js": path.join(webAppRoot, "speech-output-worklet.js"),
    ...
    "/assets/clawdad-baby-mascot.png": babyMascotAssetPath,
...
async function launchWindowsTerminal(projectPath, session, commandParts) {
  const title = pickString(session?.title, session?.slug, "ClawDad Codex").slice(0, 80);
  const args = [
    "-w",
    "0",
    "new-tab",
  ...
  const result = await runExec("wt.exe", args, { timeoutMs: 7_000 });
  if (result.ok) {
    return result;
  }
  const missing = /ENOENT|not recognized|cannot find/iu.test(
    `${result.stderr || ""}\n${result.stdout || ""}`,
  );
  return {
    ...result,
    stderr: missing
      ? "Windows Terminal (wt.exe) is unavailable. Install Windows Terminal, then try again."
      : result.stderr,
  };
}
...
async function launchSessionTerminal(projectPath, session) {
  ...
  if (os.platform() === "win32") {
    const result = await launchWindowsTerminal(
      projectPath,
      session,
      commandParts,
    );
...
async function handleDispatch(req, res, options, actor) {
  ...
  if (typeof payload.sessionId === "string" && payload.sessionId.trim() !== "") {
    ...
    if (!matchedSession?.sessionId) {
      const recoveryState = await readSessionDoctorStatePayload();
      const projectState = recoveryState.projects?.[resolvedProjectPath] || {};
      const recoveredSessionId = pickString(
        normalizeProjectSessionAliases(projectState.session_aliases)[requestedSessionId],
      );
      if (recoveredSessionId) {
        matchedSession = findProjectSession(matchedProject, recoveredSessionId);
        if (!matchedSession?.sessionId) {
          matchedProject = projectSummaryFromTabs(
            resolvedProjectPath,
            [],
            recoveryState.projects,
          );
          matchedSession = findProjectSession(matchedProject, recoveredSessionId);
        }
      }
    ...
    if (matchedSession.sessionId !== requestedSessionId) {
      sessionRecovery = {
        kind: "stale_active_session",
        source: "dispatch",
        reason: "recovered_session_alias",
        previousSessionId: requestedSessionId,
        sessionId: matchedSession.sessionId,
        message: "ClawDad replaced a stale local thread binding with a fresh Codex thread.",
        projectDetails: matchedProject,
  ...
  let permissionMode =
    typeof payload.permissionMode === "string" && payload.permissionMode.trim() !== ""
      ...
      : options.agentAccess?'host':defaultRemotePermissionMode(matchedSession?.provider);
  if(permissionMode==='host'&&options.agentAccess){
    if(String(matchedSession?.provider||'').toLowerCase()==='codex'){
      const policy=await options.agentAccess.resolve();
      permissionMode={full:'full',workspace:'approve','read-only':'plan'}[policy.mode];
    }else permissionMode=defaultRemotePermissionMode(matchedSession?.provider);
  }
  ...
  let sessionValidation = await validateDispatchSessionBinding(resolvedProjectPath, matchedSession || {});
  if (!sessionValidation.ok) {
    try {
      sessionRecovery = await recoverInvalidActiveCodexSession(
        resolvedProjectPath,
        matchedProject,
        matchedSession,
        sessionValidation,
      );
      if (sessionRecovery?.session?.sessionId) {
        matchedProject = sessionRecovery.projectDetails;
        matchedSession = sessionRecovery.session;
        sessionValidation = await validateDispatchSessionBinding(
          resolvedProjectPath,
          matchedSession,
        );
      }
      ...
      json(res, 500, {
        ...
        error: `ClawDad could not replace the stale thread safely: ${error.message}`,
  ...
  // Deliverable handoffs now register local Files snapshots. Cloud sharing is
  // a separate explicit action, never an automatic side effect of an export.
  ...
  let accountWork;
  if(options.accountWork&&String(matchedSession?.provider||'').toLowerCase()==='codex'){
    try{accountWork=await options.accountWork.reserve({id:dispatchRequestId,projectPath:resolvedProjectPath,sessionId:matchedSession.sessionId,
      fingerprint:crypto.createHash('sha256').update(JSON.stringify({project:resolvedProjectPath,sessionId:matchedSession.sessionId,message:dispatchMessage,
        permissionMode,model:requestedModel,reasoningEffort,attachmentManifest:savedAttachments.manifestPath,scheduleMode})).digest('hex')});}
    catch(error){json(res,409,{ok:false,error:error.message,reasonCode:'account_switch_pending'});return;}
  }
  try {
    dispatchAdmission = await acquireDispatchAdmissionLock(resolvedProjectPath);
  } catch (error) {
    if(accountWork)await options.accountWork.finishAttempt(accountWork);
    json(res, 503, {
      ok: false,
      project,
      wait,
  ...
  if (mailboxBusy || hasQueuedBacklog) {
    ...
    if(accountWork)await options.accountWork.prepare(accountWork);
  ...
  const startResult = await startClawdadDetached(args, {
    env: {
      ...
      CLAWDAD_MANUAL_REQUEST_TEXT: dispatchMessage,
    },
  });
  if (!startResult.ok) {
    await dispatchAdmission.release();
  ...
  if (wait) {
    ...
    json(res, completed ? 200 : 500, {
      ...
      mailboxStatus,
      attachments: savedAttachments.attachments,
      recovery: completed ? null : codexAuthenticationRecovery(responseText),
      sessionRecovery,
    });
    return;
  }
...
async function handleRead(_req, res, options, url, actor) {
  ...
  if (requestId) {
    ...
    if (historyItem) {
      ...
      json(res, 200, {
        ...
        historyItem,
        source: "history",
        recovery: normalizeHistoryStatus(historyItem.status) === "failed"
          ? codexAuthenticationRecovery(output)
          : null,
      });
      return;
    }
    ...
    if (
      ...
      json(res, 200, {
        ...
        historyItem: null,
        source: "mailbox",
        recovery: mailboxLifecycle === "failed"
          ? codexAuthenticationRecovery(output)
          : null,
      });
      return;
  ...
  json(res, 200, {
    ...
    stderr: "",
    mailboxStatus,
    recovery: lifecycle === "failed" ? codexAuthenticationRecovery(output) : null,
  });
}
...
async function recentWorkspaceCodexSessions(workspace = {}, { limit = 20, refresh = false } = {}) {
  ...
  try {
    ...
    try {
      nativeRecentSessions = await recentWorkspaceCodexSessions(workspace, {
        ...
        refresh: url?.searchParams?.get("refreshRecent") === "1",
      });
    } catch (error) {
...
async function handleArtifactsGet(req, res, options, url, actor) {
  ...
  try {
    ...
    const dumpy = dumpyStateForClient(resolved.projectPath, resolved.projectDetails,
      await readProjectDumpyState(resolved.projectPath));
    json(res, 200, {
      ok: true,
      actor,
      project: resolved.projectPath,
...
function markTtsUnavailableForError(error = null) {
  ...
  // A transient local chunk failure is not a billing/quota circuit breaker.
  // Readiness is checked on the next request; clients apply bounded backoff.
  if (!classified.unavailable || classified.errorCode === "local_service_unavailable") {
    return classified;
  }
...
function normalizeDocReaderEngineHealth(payload = {}, engine = "kokoro") {
  ...
  return {
    enabled,
    loaded,
    available: Boolean(enabled && (engineHealth.available ?? loaded)),
...
function ttsGeneratingAudioForClient(projectPath, audioId, source = {}, manifest = {}) {
  return {
    ...
    cachedAt: pickString(manifest.updatedAt, manifest.createdAt) || null,
    source,
    parts: (manifest.parts || []).map(part => ({...part, textHash: part.textHash || null,
      audioHash: part.audioHash || null, url: ttsAudioUrl(projectPath, audioId, part.fileName)})),
  };
}
...
async function startTtsPrepareJob({ projectPath, text, source, config, apiKey, poll = false, retry = false }) {
  ...
  if (await ttsManifestIsReady(projectPath, existingManifest)) {
    return {
      statusCode: 200,
      cached: true,
  ...
  if (existingManifest?.state === "failed" && poll && !retry) {
...
function historyAudioManifestMatchesText(manifest = {}, text = "", config = {}) {
  if (manifest.provider !== config.provider || manifest.voiceId !== config.voiceId ||
      manifest.modelId !== config.modelId || Number(manifest.speed || 1) !== Number(config.speed || 1)) return false;
  const normalizedText = normalizeTtsText(text);
  if (!normalizedText) {
    return false;
  }
...
async function handleTtsStatusGet(_req, res, options, actor) {
  let ttsStatus;
  try {
    const configured = resolveTtsRuntimeConfig({config: options.config});
    const runtimeConfig = configured.enabled ? await runtimeWithVoice(options, configured) : configured;
    ttsStatus = await ttsStatusPayload(options.config, {runtimeConfig});
  } catch {
    // A saved voice needs the local catalog for validation. During login that
    // service may not have started yet; speech availability is independent of
    // the workspace, pairing and Terminal control service.
    ttsStatus = {
      enabled: true, provider: 'doc-reader', label: 'ClawDad local speech', available: false,
      error: 'The local speech service is unavailable. ClawDad will check again automatically.',
      errorCode: 'local_service_unavailable', retryAfterMs: 5000, unavailableUntil: null,
    };
  }
  json(res, 200, {
    ok: true,
...
async function handleVoiceSettings(req, res, options, actor) {
  try {
    const payload = req.method === "POST" ? await parseDispatchPayload(req, options.bodyLimitBytes) : null;
    if (req.method === "POST" && !payload?.selection) throw new Error("Choose a model and voice to save.");
    const settings = await voiceSettings(options, resolveTtsRuntimeConfig({config: options.config}), payload?.selection);
    json(res, 200, {ok: true, actor, ...settings});
  } catch (error) {
    json(res, req.method === "POST" ? 400 : 503, {ok: false, error: error.message});
  }
}
...
async function handleTtsMessageCreate(req, res, options, actor) {
  ...
  let ttsConfig;
  try {
    // Freeze this request's voice, including every subsequent preparation poll.
    ttsConfig = await runtimeWithVoice(options, ttsRuntimeConfigForRequest(
      resolveTtsRuntimeConfig({ config: options.config }), payload), payload.voiceSelection);
  } catch (error) {
    json(res, 400, {ok: false, error: error.message});
    return;
  }
  ...
  // A service restart cannot make a complete, verified local recording
  // unavailable. Voice selection still resolves before this cache lookup.
  if (ttsConfig.preferDirectAudio) {
    const cached = await readTtsManifest(resolved.projectPath, ttsMessageAudioId(ttsConfig, ttsMessage.text));
    if (await ttsManifestIsReady(resolved.projectPath, cached)) {
      json(res, 200, {ok: true, actor, project: resolved.projectPath, cached: true,
        audio: ttsManifestForClient(resolved.projectPath, cached)});
      return;
    }
  }
    ...
    if (asyncPrepare) {
      ...
      json(res, prepared.statusCode, {
        ...
        audio: {...prepared.audio,
          ...(payload.source === 'remote-assist' && ttsGenerationConfig.preferDirectAudio
            ? {fallbackChunks: splitTtsPlaybackText(normalizeTtsText(ttsMessage.text), {chunkChars: ttsGenerationConfig.chunkChars})} : {})},
...
async function handleTtsAudioGet(req, res, options, url, actor) {
  ...
  try {
    ...
    if (!manifest) {
      throw new Error("audio was not found");
    }
    if (!["generating", "ready"].includes(manifest.state)) {
      throw new Error("Audio generation did not finish on this Mac.");
    }
    ...
    if (!part) {
      if (manifest.state === "generating") {
        json(res, 409, {ok: false, actor, error: "Audio is still preparing.", errorCode: "audio_not_ready", retryAfterMs: 750});
        return;
      }
      throw new Error("audio part was not found");
    }
    // A part is published only after its file has been atomically completed.
    // Later parts may still be generating when the phone requests this one.
    const filePath = ttsAudioFilePath(resolved.projectPath, audioId, partName);
    const info = await stat(filePath);
    if (!info.isFile() || info.size === 0 || (typeof part.bytes === "number" && info.size !== part.bytes)) {
      throw new Error("The completed audio file is unavailable or incomplete.");
    }
    await sendRangeFile(req, res, filePath, {
...
async function handleSttTranscribe(req, res, options, actor) {
  ...
  try {
    ...
    json(res, 200, {
      ...
      ...(Number.isFinite(result.generationSeconds) ? { generationSeconds: result.generationSeconds } : {}),
    });
  } catch (error) {
...
async function autoRegisterImportableCodexSessionsForProject(projectDetails = {}, {
  ...
  if (!allowAutomaticRuntimeStateMutation("Codex session auto-import")) {
    return 0;
  }
  const projectPath = pickString(projectDetails?.path);
...
function scheduleActiveDelegateSupervisorResume() {
  if (
    delegateSupervisorResumeDisabled ||
    !allowAutomaticRuntimeStateMutation("delegate supervisor resume")
  ) {
    return;
  }
  const run = () => {
...
async function handleCodexModelsGet(_req, res, options, url, actor) {
  ...
  try {
    ...
    const recovery = codexAuthenticationRecovery(error.message);
    json(res, recovery ? 401 : 500, {
      ok: false,
      actor,
      project: projectPath,
...
async function ensureAccountCodexSharedRuntime(options){
  if(process.platform!=='darwin'||!process.env.CLAWDAD_NATIVE_RUNTIME_VERSION)return ensureCodexSharedRuntime(options);
  const accounts=codexAccountRuntimeControls ||= new CodexAccounts({authorizations:process.platform==='darwin'
    ?new CodexAccountAuthorizations({root:codexAccountsRoot(),binary:defaultCodexBinary}):null});
  return ensureAccountSharedRuntime(options,{accounts});
}
...
async function ensureCodexSharedRuntimeForServer() {
  ...
  const pending = ensureAccountCodexSharedRuntime({
    mode: codexAppServerRequestedMode,
    socketPath: codexAppServerSocket,
    codexBinary: defaultCodexBinary,
    env: process.env,
    cwd: process.env.CLAWDAD_NATIVE_RUNTIME_VERSION ? os.homedir() : clawdadRoot,
    checkLaunchPaths: process.platform === 'darwin' && !!process.env.CLAWDAD_NATIVE_RUNTIME_VERSION,
  })
    .then((runtime) => {
...
async function runServe(argv) {
  ...
  if (codexAppServerRequestedMode !== "isolated" && !codexSharedRuntimeState) {
    codexSharedRuntimeState = {
      mode: "shared",
      requestedMode: codexAppServerRequestedMode,
      ready: false,
      state: "starting",
  ...
  const speechRecovery = new LocalSpeechRecovery({config: async () => {
    const configured = resolveTtsRuntimeConfig({config: options.config});
    const preferences = await readVoicePreferences(options, configured);
    return {...configured, baseUrl: localVoiceServiceUrl(configured), engine: preferences.selection.engine, voiceId: preferences.selection.voice};
  }});
  const {readHostNotificationIdentity}=await import('./notification-outbox-delivery.mjs');
  const assistantRuntime = new AssistantRuntime({root:privateRuntimeRoots.assistant,notificationIdentity:()=>readHostNotificationIdentity(
    pickString(options.config?.cloudHostConfigPath,options.config?.cloudConfigPath))});
  assistantRuntime.modelSettings = new AssistantModelSettings({file:path.join(assistantRuntime.root,'model-settings.json'),
    legacyMain:async()=>{await assistantRuntime.load();return assistantRuntime.state.coordinator;},
    readCatalog:async()=>{
      await assistantRuntime.coordinator.prepare();
      return readCodexModelCatalog({codexBinary:assistantRuntime.coordinator.codexPath,projectPath:assistantRuntime.root,requireAuthenticated:true,
        accountLaunch:await codexAccounts.selectedLaunch()});
    }});
  assistantRuntime.appServer = new AssistantAppServer({root:path.join(assistantRuntime.root,'AppServer'),
    mayDispatch:async job=>!assistantRuntime.state?.paused&&assistantRuntime.state?.jobs.find(j=>j.id===job.id)?.status!=='cancelled'
      &&(!assistantRuntime.accounts||(await assistantRuntime.accounts.deliveryAdmission(job)).allowed),
    workspaces:()=>workspacePayload(options),onJob:job=>assistantRuntime.recordAppServerJob(job),
    onCreated:(thread,cwd)=>registerImportedSessionLocally(cwd,{sessionId:thread.id,title:thread.name||'Assistant thread',transcriptPath:thread.path,cwd}, {}, {makeActive:false})});
  const nativeAppAccounts=process.platform==='darwin'&&!!process.env.CLAWDAD_NATIVE_RUNTIME_VERSION;
  const activeAppAccount=nativeAppAccounts?new CodexAppAccountStatus({socketPath:codexAppServerSocket}):null;
  const weeklyUsage = new CodexWeeklyUsageMonitor({statePath: path.join(clawdadHome, 'codex-weekly-usage.json'),
    read: async () => {
      if(!activeAppAccount)return readCodexAccountUsage({binary: defaultCodexBinary,accountLaunch:await codexAccounts.selectedLaunch()});
      const current=await activeAppAccount.read({fresh:true});
      if(!current.usage)throw Error(current.message||'The running ClawDad subscription could not be verified.');
      return current.usage;
    }});
  let accountLegacyWork;
  const codexAccounts=new CodexAccounts({root:privateRuntimeRoots.accounts,usage:weeklyUsage,requireSelection:nativeAppAccounts,
    readActive:activeAppAccount?()=>activeAppAccount.read():null,
    readWork:async()=>{
      const native=await readAccountWork(assistantRuntime),legacy=accountLegacyWork?await accountLegacyWork.snapshot():{complete:true,jobs:[]};
      return {complete:native.complete&&legacy.complete,jobs:[...native.jobs,...legacy.jobs]};
    },
    authorizations:process.platform==='darwin'?new CodexAccountAuthorizations({root:privateRuntimeRoots.accounts,binary:defaultCodexBinary}):null});
  codexAccountRuntimeControls=codexAccounts;
  assistantRuntime.accounts=codexAccounts;
  assistantRuntime.coordinator.resolveAccountLaunch=()=>codexAccounts.selectedLaunch();
  assistantRuntime.accessSettings=new AgentAccessSettings({file:path.join(assistantRuntime.root,'agent-access.json')});
  if(nativeAppAccounts){
    assistantRuntime.coordinator.turnRunner=new CodexSharedTurnRunner({root:assistantRuntime.root,
      resolveAccountLaunch:()=>codexAccounts.selectedLaunch(),accessSettings:assistantRuntime.accessSettings});
    nativeCodexAuxiliaryTasks=new CodexAuxiliaryTasks({root:path.join(assistantRuntime.root,'Auxiliary'),
      runner:assistantRuntime.coordinator.turnRunner,accounts:codexAccounts,
      resolveModel:()=>assistantRuntime.modelSettings.resolve('research'),timeoutMs:projectSummaryTimeoutMs});
    await nativeCodexAuxiliaryTasks.load();
  }
  assistantRuntime.appServer.accessSettings=assistantRuntime.accessSettings;
  assistantRuntime.appServer.accountControls=codexAccounts;
  accountLegacyWork=new CodexAccountLegacyWork({root:path.join(codexAccounts.root,'ProjectWork'),accounts:codexAccounts,
    readProjects:()=>loadProjectCatalogCached({allowStale:false}),
    inspectReceipt:async job=>{
      const item=await readHistoryEntryByRequestId(job.projectPath,job.id,job.sessionId);
      if(!item||pickString(item.requestId)!==job.id||pickString(item.sessionId)!==job.sessionId)return null;
      return {id:job.id,projectPath:job.projectPath,sessionId:job.sessionId,status:item.status};
    }});
  const accountSwitchRuntime=process.platform==='darwin'&&process.env.CLAWDAD_NATIVE_RUNTIME_VERSION
    ?connectCodexAppAccounts({accounts:codexAccounts,runtime:assistantRuntime,binary:defaultCodexBinary,
      socketPath:codexAppServerSocket,canonicalHome:path.resolve(process.env.CODEX_HOME||path.join(os.homedir(),'.codex'))}):null;
  const researchRoot = path.join(assistantRuntime.root,'Research');
  assistantRuntime.research = new ResearchSupervisor({root:researchRoot,runtime:assistantRuntime,
    modelSettings:assistantRuntime.modelSettings,
    budget:new ResearchBudget({file:path.join(researchRoot,'account-budget.json'),usage:weeklyUsage}),
    reviewer:new ResearchReviewRunner({root:path.join(researchRoot,'Reviews'),binary:defaultCodexBinary,
      turnRunner:assistantRuntime.coordinator.turnRunner,
      resolveAccountLaunch:()=>codexAccounts.selectedLaunch(),
      resolveBinary:async()=>{await assistantRuntime.coordinator.prepare();return assistantRuntime.coordinator.codexPath;}})});
  codexAccounts.onActivated=async selection=>{
    activeAppAccount?.invalidate();
    await assistantRuntime.research.accountActivated(selection);
  };
  const handleRequest = async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/healthz") {
      json(res, 200, {
        ...
        speechRecovery: speechRecovery.snapshot(),
        version,
    ...
    if (url.pathname === '/v1/codex/weekly-usage' && req.method === 'GET') {
      try { json(res, 200, await weeklyUsage.snapshot()); }
      catch { json(res, 200, {status: 'unavailable', message: 'The weekly allowance checkpoint needs repair.', alerts: []}); }
      return;
    }
    if (url.pathname === '/v1/codex/weekly-usage/outbox' && req.method === 'GET') {
      json(res, 200, {events: await weeklyUsage.outbox().catch(() => [])}); return;
    }
    if (url.pathname === '/v1/codex/weekly-usage/delivered' && req.method === 'POST') {
      try { const body = JSON.parse(await readBody(req, 2048)); await weeklyUsage.delivered(body.id); json(res, 200, {ok: true}); }
      catch { json(res, 503, {ok: false}); } return;
    }

    if (await assistantHttp(req, res, url, assistantRuntime, {
      json,
      readBody: async (request) => JSON.parse(await readBody(request, 2 * 1024 * 1024)),
    })) return;
    ...
    if (url.pathname.startsWith("/v1/files/")) {
      ...
      try {
        if (req.method === "GET" && url.pathname === "/v1/files/library") {
          json(res, 200, { ok: true, ...await library.list({ query: url.searchParams.get("query") || "", project: url.searchParams.get("project") || "", format: url.searchParams.get("format") || "", category: url.searchParams.get("category") || "documents", archived: url.searchParams.get("archived") === "true", pinned: url.searchParams.get("pinned") === "true", cursor: url.searchParams.get("cursor"), limit: url.searchParams.get("limit") }) });
        } else if (req.method === "POST" && ["/v1/files/image-upload", "/v1/files/image-resolve"].includes(url.pathname)) {
          const payload = JSON.parse(await readBody(req, 256 * 1024));
          const inbox = new LocalImageInbox(library);
          const result = url.pathname.endsWith("image-upload") ? await inbox.request(payload) : await inbox.resolve(payload);
          json(res, 200, { ok: true, ...result });
        } else if (req.method === "GET" && url.pathname === "/v1/files/chunk") {
          json(res, 200, { ok: true, ...await library.chunk({ id: url.searchParams.get("id"), versionId: url.searchParams.get("versionId"), offset: Number(url.searchParams.get("offset") || 0) }) });
        } else if (req.method === "GET" && url.pathname === "/v1/files/instructions") {
          const quote = (value) => `'${String(value).replaceAll("'", "'\\''")}'`;
          const command = `${quote(process.execPath)} ${quote(fileURLToPath(import.meta.url))} files add`;
          json(res, 200, { ok: true, instructions: `Save the final requested deliverable in its project folder, then register it in my local ClawDad Files library by running ${command} '/absolute/path/to/file' --project '/absolute/project/path'. Register only finished deliverables I asked for. Ordinary source changes and temporary files stay outside Files. Reusing the same filename groups revisions; exports with the same base filename group their formats. Tell me the saved filename when done.` });
        } else if (req.method === "GET" && url.pathname === "/v1/files/legacy") {
          const resolved = await resolveProjectForArtifacts(url.searchParams.get("project") || "", options.defaultProject);
          if (!resolved.projectPath || !resolved.projectDetails) throw new Error("Choose a tracked project first.");
          json(res, 200, { ok: true, artifacts: await listProjectArtifacts(resolved.projectPath) });
        } else if (req.method === "POST" && url.pathname === "/v1/files/import") {
          const payload = JSON.parse(await readBody(req, 16 * 1024));
          const resolved = await resolveProjectForArtifacts(payload.project || "", options.defaultProject);
          if (!resolved.projectPath || !resolved.projectDetails) throw new Error("Choose a tracked project first.");
          const artifact = artifactPathFor(resolved.projectPath, payload.relativePath);
          const sourcePath = await realpath(artifact.absolutePath);
          if (!pathInsideRoot(await realpath(artifact.artifactRoot), sourcePath) || artifactIsHidden(artifact.relativePath)) throw new Error("Choose a visible deliverable inside this project's artifact folder.");
          json(res, 200, { ok: true, item: await library.add({ sourcePath, project: resolved.projectPath }) });
        } else if (req.method === "GET" && ["/v1/files/download", "/v1/files/preview"].includes(url.pathname)) {
          const { handle, version } = await library.resolve(url.searchParams.get("id"), url.searchParams.get("versionId"));
          try {
            const preview = url.pathname.endsWith("/preview");
            const previewType = ["application/pdf", "image/png", "image/jpeg", "image/webp"].includes(version.mimeType) ? version.mimeType : "text/plain; charset=utf-8";
            res.writeHead(200, { "content-type": preview ? previewType : version.mimeType, "content-length": version.size, "content-disposition": contentDispositionAttachment(version.fileName).replace(/^attachment/u, preview ? "inline" : "attachment"), "cache-control": "private, no-store", "x-content-type-options": "nosniff", "content-security-policy": "default-src 'none'; frame-ancestors 'self'; sandbox" });
            await fileLibraryPipeline(handle.createReadStream({ autoClose: false }), res);
          } finally { await handle.close(); }
        } else if (req.method === "POST" && ["/v1/files/add", "/v1/files/update"].includes(url.pathname)) {
          const payload = JSON.parse(await readBody(req, 16 * 1024));
          const item = url.pathname.endsWith("/add") ? await library.add(payload) : await library.update(payload);
          json(res, 200, { ok: true, item });
        } else { json(res, 404, { ok: false, error: "Unknown Files action." }); }
      } catch (error) {
        if (res.headersSent) res.destroy(error);
        else json(res, 400, { ok: false, error: error.message || "Files could not complete this action." });
      }
      return;
    }
    ...
    if (["GET", "POST"].includes(req.method) && url.pathname === "/v1/tts/voices") {
      await handleVoiceSettings(req, res, options, auth.actor);
      return;
    }
    ...
    if (req.method === "POST" && url.pathname === "/v1/dispatch") {
      await handleDispatch(req, res, {...options,agentAccess:assistantRuntime.accessSettings,accountWork:accountSwitchRuntime?accountLegacyWork:null}, auth.actor);
      return;
    }
  ...
  // Node's HTTP callback does not await an async handler. Contain an unexpected
  // request failure here instead of letting its rejection terminate the host.
  const server = http.createServer((req, res) => {
    void handleRequest(req, res).catch(() => {
      console.error('[clawdad-server] request failed; local service remains available');
      if (res.destroyed || res.writableEnded) return;
      if (res.headersSent) res.destroy();
      else json(res, 500, {ok: false, code: 'request_failed', error: 'ClawDad could not finish this request.'});
    });
  });
  ...
  await new Promise((resolve, reject) => {
    ...
    server.once("close", () => { weeklyUsage.stop(); speechRecovery.stop(); void codexAccounts.stop().finally(()=>accountSwitchRuntime?.close()); void assistantRuntime.close(); });
  ...
  codexAccounts.start();
  if (process.platform === 'darwin' && process.env.CLAWDAD_NATIVE_RUNTIME_VERSION) { weeklyUsage.start(); assistantRuntime.research.start(); speechRecovery.start(); assistantRuntime.appServer.start(); }
  scheduleActiveDelegateSupervisorResume();
  if (codexAppServerRequestedMode !== "isolated") {
    // The HTTP service is the control plane for setup, pairing, workspace
    // browsing, and controller-only Macs. Bring it online before probing or
    // launching Codex so a missing, logged-out, or slow Codex installation can
    // be repaired from the app instead of making ClawDad look unavailable.
