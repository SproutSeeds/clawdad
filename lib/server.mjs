...
import {inspectAccountConsumers,readAccountWork} from './codex-account-consumers.mjs';
import {ensureAccountSharedRuntime} from './codex-account-runtime-routing.mjs';
import {connectCodexAccountSwitchRuntime} from './codex-account-switch-runtime.mjs';
...
import {CodexAccountShellLaunches} from './codex-account-shell-launch.mjs';
import {
  createTtsAudioId,
...
async function runServe(argv) {
  ...
  const weeklyUsage = new CodexWeeklyUsageMonitor({statePath: path.join(clawdadHome, 'codex-weekly-usage.json'),
    read: async () => readCodexAccountUsage({binary: defaultCodexBinary,accountLaunch:await codexAccounts.selectedLaunch()})});
  let accountLegacyWork,accountShellLaunches;
  const codexAccounts=new CodexAccounts({root:privateRuntimeRoots.accounts,usage:weeklyUsage,inspectConsumers:()=>inspectAccountConsumers(assistantRuntime),
    readWork:async()=>{
      ...
      const shells=accountShellLaunches?await accountShellLaunches.snapshot():{complete:true,jobs:[]};
      return {complete:native.complete&&legacy.complete&&shells.complete,jobs:[...native.jobs,...legacy.jobs,...shells.jobs]};
    },
  ...
  const accountSwitchRuntime=process.platform==='darwin'&&process.env.CLAWDAD_NATIVE_RUNTIME_VERSION
    ?connectCodexAccountSwitchRuntime({accounts:codexAccounts,runtime:assistantRuntime,binary:defaultCodexBinary,
      socketPath:codexAppServerSocket,canonicalHome:path.resolve(process.env.CODEX_HOME||path.join(os.homedir(),'.codex'))}):null;
  if(accountSwitchRuntime)accountShellLaunches=new CodexAccountShellLaunches({root:path.join(codexAccounts.root,'ShellLaunches'),accounts:codexAccounts,
    readNative:()=>assistantRuntime.accountConsumerInventory()});
