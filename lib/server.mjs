    ...
    for (let stepIndex = Math.max(0, Number.parseInt(String(latestStatus.stepCount || "0"), 10) || 0); ; stepIndex += 1) {
      ...
      if (watchtowerReviewMode !== "off") {
        ...
        if (
          ...
          reviewGuard.ok &&
          (reviewGuard.hardStop || reviewGuard.pauseRecommended || reviewGuard.repairRecommended)
        ) {
...
function delegateStatusIsRunningLike(status = {}) {
  const state = String(status?.state || "").trim().toLowerCase();
  return state === "running" || state === "planning" || Boolean(status?.activeRequestId);
}
...
async function runDelegateSupervisorTick(resolved, laneId, options = {}) {
  const [config, brief, status, supervisor] = await Promise.all([
  ...
  while (true) {
    ...
    if (requestId && stallGuard?.runId) {
      const stall = await evaluateDelegateDispatchStall(projectPath, status, {
        ...stallGuard,
        laneId: normalizedLaneId || stallGuard.laneId || defaultDelegateLaneId,
      });
      if (stall.stalled) {
        ...
        }, normalizedLaneId || stallGuard.laneId || defaultDelegateLaneId).catch(() => {});
        return await repairStaleMailboxStatus(projectPath, status, reason, normalizedLaneId);
      }
    }
...
const sessionDoctorBusyStates = new Set(["running", "queued", "dispatched", "planning"]);
...
function sessionDoctorFallbackSessionId(projectState = {}, skippedSessionId = "") {
...
async function inspectSessionDoctorProject(projectPath, stateProjects = {}, { repair = false } = {}) {
  ...
  for (const laneId of laneIds) {
    ...
    if (status.state === "running" && !supervisorLive) {
      const issue = addIssue(
...
async function buildSessionDoctorReport({ selector = "", repair = false } = {}) {
  ...
  const issueCount = projects.reduce((sum, project) => sum + project.issues.length, 0);
