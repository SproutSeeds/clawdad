...
const actions = {inspect_tab:'terminal.inspect',insert_in_tab:'terminal.insert',send_to_tab:'terminal.send',focus_tab:'terminal.focus',move_tab:'terminal.move',close_tab:'terminal.close',resolve_close:'terminal.close.resolve'};
      ...
      case 'tools/call': {
        ...
        if(name==='workspace')value=await request('/v1/assistant/state');
          ...
          for(let i=0;i<25 && ['queued','running'].includes(value.job?.status);i++) {
            if(name==='send_to_tab'||name==='insert_in_tab')break;
