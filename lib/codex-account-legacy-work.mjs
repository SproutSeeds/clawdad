...
import {readLegacyAccountInventory,accountProjectStatus} from './codex-account-project-inventory.mjs';
    ...
    for(const name of names.filter(n=>n.endsWith('.json'))){
      try{
        ...
        if(job.effectPrepared&&!['completed','cancelled'].includes(job.status)){
          ...
          }else if(accountProjectStatus(observed.status)==='completed')job.status='completed';
