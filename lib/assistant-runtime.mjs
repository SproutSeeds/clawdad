...
export class AssistantRuntime {
    ...
    return this.transaction(async () => {
      ...
      const job = this.state.jobs.find((j,index) => nativeActions.has(j.action) && j.status === 'queued' && (!j.retryAt || j.retryAt <= this.clock()) &&
        !this.state.jobs.slice(0,index).some(earlier => active.has(earlier.status) && route(earlier) === route(j) &&
          !(['terminal.queue','terminal.insert','terminal.clear','terminal.replace'].includes(j.action) && ['submitted','working','agent_queued'].includes(earlier.status))));
      if (!job) return idle;
  ...
  consumeRecord(record, file, {coordinator}) {
    ...
    const jobs = this.state.jobs.filter(j => j.conversationPath === file && (active.has(j.status) || j.status === 'inserted' ||
    ...
    for (const job of jobs.filter(j => j.status === 'working' && (!j.turnId || !p.turn_id || j.turnId===p.turn_id))) {
      ...
      if (['terminal.send','terminal.queue','terminal.insert'].includes(job.action) && job.visibility !== 'diagnostic'
        ...
        const args={text};
        this.state.jobs.push({id:`update:${job.id}`,parentTaskId:job.id,action:'message',args,fingerprint:JSON.stringify({action:'message',args}),status:'queued',createdAt:now(),source:'task-update'});
      }
    }
    return true;
  }
