  ...
  async nativeResult({id, result, error, deferred = false}) {
    return this.transaction(async () => {
      ...
      if (deferred) {
        ...
        if(job.action==='terminal.key' && typeof result?.keySent==='boolean') job.result=result;
      }
      else {
        ...
        if (['terminal.clear','terminal.replace'].includes(job.action) && result?.draftVerified === true && (result?.sessionId || result?.agentInstanceId)) {
