...
export class AssistantRuntime {
  ...
  async command(request, {tool = false, supervisor = null, observationOnly = false} = {}) {
    ...
    if (request?.action?.startsWith('research.')) {
      if (tool && !['research.status','research.history'].includes(request.action)) throw new Error('Research autonomy is enabled or changed only through Cody’s explicit controls.');
      ...
      const research=await this.research.control(action,args,requestId);
  ...
  async cancelResearchWaiting(threadId) { return this.transaction(async()=>{
    for(const job of this.state.jobs)if(job.supervisor?.threadId===threadId&&job.status==='queued')job.status='cancelled';
    await this.save();
  }); }
  ...
  async close(){this.closed=true;await this.research?.close();this.coordinator.stop();await this.drainTask;}
