...
export class AssistantRuntime {
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      const idle={job:null,enabled:this.state.enabled,pendingBindings};
      ...
      return {job:{...job},enabled:this.state.enabled,pendingBindings};
    });
  }
  async nativePrepare({id, conversationPath, sessionId, tabTitle, priorTurnId, agentInstanceId, tty}) {
    ...
    return this.transaction(async () => {
      ...
      if (conversationPath || sessionId) {
        ...
        if (job.action==='terminal.queue' || !/^codex-process-[a-f0-9]{64}$/.test(agentInstanceId || '') || !/^\/dev\/tty[A-Za-z0-9]+$/.test(tty || '') || agentInstanceId!==job.args.agentInstanceId) throw new Error('Fresh input requires the exact inspected process and TTY');
  ...
  async close(){this.closed=true;await this.research?.close();await this.appServer?.close();this.coordinator.stop();await this.drainTask;}
