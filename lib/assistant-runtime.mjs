...
const accountReadOnlyActions=new Set(['terminal.inspect','terminal.observe','terminal.context','terminal.native.inspect',
...
export class AssistantRuntime {
  ...
  async accountConsumerInventory({waitMs=30000}={}){
    ...
    if(this.accountInventoryRequest===request)this.accountInventoryRequest=null;
    return {complete:false,consumers:[],reason:'The installed native worker did not return a fresh account-process inventory. Update/reconnect ClawDad on the Mac before switching.'};
  }
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      const idle={job:null,enabled:this.state.enabled,inventoryRequested,pendingBindings,accountInventoryRequest};
      ...
      return {job:{...job},enabled:this.state.enabled,inventoryRequested,pendingBindings,accountInventoryRequest};
    });
  }
