...
import {readAgentToolOrigin} from './agent-tool-context.mjs';
...
import {appAccountAction} from './codex-app-accounts.mjs';
import {isTurnControl} from './assistant-turn-control.mjs';
...
export class AssistantRuntime {
  ...
  async accountProcessInventory({waitMs=8000}={}){
    const request={id:uuid(),expires:Date.now()+waitMs};this.accountProcessRequest=request;
    try{
      while(Date.now()<request.expires&&!this.closed){
        const observed=this.observation?.accountProcesses;
        if(observed?.id===request.id)return structuredClone(observed);
        await new Promise(resolve=>setTimeout(resolve,40));
      }
      throw Object.assign(Error('The Mac process reader did not respond. Keep ClawDad open and retry activation.'),{code:'app_process_reader_unavailable',appAccountSafe:true});
    }finally{if(this.accountProcessRequest===request)this.accountProcessRequest=null;}
  }
  ...
  async command(request, {tool = false, supervisor = null, observationOnly = false} = {}) {
    const context=tool&&request?.toolContextId?await readAgentToolOrigin(this.root,request.toolContextId):null;
    if(context&&!context.policy.computerUse&&(/^(computer\.|terminal\.|clipboard\.|mainworkspace\.)/.test(request.action)))
      throw Error('Computer use is disabled for this request. Change Agent access in Settings for subsequent requests.');
    const externalOrigin=context&&context.source!=='assistant'?{id:context.requestId,action:'message',status:'running',source:context.authorizationSource||'assistant',
      args:{text:context.authorizationText},accountEpoch:context.accountEpoch,conversationId:this.state?.conversationId}:null;
    const coordinatorRequestId=context?.requestId||(tool?request?.coordinatorRequestId:null);
    const currentOrigin=()=>externalOrigin?{...externalOrigin,conversationId:this.state.conversationId}:this.state.jobs.find(j=>j.id===(coordinatorRequestId||this.state.coordinator?.activeRequestId));
    ...
    if(coordinatorRequestId){await this.transaction(verifyCoordinator);const {coordinatorRequestId:_,toolContextId:__,...rest}=request;request=rest;}
    if(tool&&request?.action==='context.workspace'&&context)return this.transaction(()=>({...this.snapshot(),
      currentRequest:{id:context.requestId,text:context.text,threadId:context.threadId,source:context.source},
      ...(externalOrigin?{requestDestination:{transport:'app_server',targets:{app_server:{threadId:context.threadId}},currentThread:true}}:{})}));
    if(['access.read','access.update','access.decide'].includes(request?.action)){
      if(tool)throw Error('Agent access is managed in ClawDad Settings.');
      if(!this.accessSettings)throw Error('Update ClawDad to manage agent access.');
      if(request.action==='access.read')return {access:await this.accessSettings.snapshot(),pendingDecisions:this.coordinator.turnRunner?.approvals()||[],nativeOnline:this.nativeSeen>0&&this.clock()-this.nativeSeen<45000};
      if(request.action==='access.decide')return this.coordinator.turnRunner.decide(request);
      return this.accessSettings.update(request,request.requestId);
    }
    if(request?.action?.startsWith('accounts.')){
      if(!this.accounts)throw Error('Update ClawDad on the Mac to use account controls.');
      if(tool&&!['accounts.status','accounts.preview','accounts.refresh','accounts.reconcile'].includes(request.action))await this.transaction(()=>{
        verifyCoordinator();const source=currentOrigin();
    ...
    if(request?.action?.startsWith('speech.')) {
      ...
      const origin=await this.transaction(()=>currentOrigin());
    ...
    if(request?.action?.startsWith('appserver.')){
      ...
      const meta=await this.transaction(()=>{
        verifyCoordinator();
        if(!this.state.enabled&&!externalOrigin)throw new Error('Start the Assistant first.');
        ...
        if(this.state.paused&&!isTurnControl(action)&&!['appserver.list','appserver.inspect','appserver.history','appserver.workspaces','appserver.models','appserver.reconcile'].includes(action))throw new Error('Mac control is paused.');
        const parent=currentOrigin();
        return {source:'assistant',...(parent?{accountParentRequestId:parent.id,nativeAuthority:{source:parent.source,text:parent.args.text,userRequestId:parent.id}}:{}),...(args.diagnostic?{visibility:'diagnostic'}:{parentRequestId:this.state.coordinator?.activeRequestId})};
      });
      delete args.diagnostic;
      if(isTurnControl(action))meta.authorize=(dispatch)=>this.transaction(()=>{
        verifyCoordinator();
        if(!this.state.enabled||this.state.paused)throw Error('Mac control is paused. Existing receipts can still be read.');
        const source=currentOrigin();
        if(source?.action!=='message'||source.status!=='running'||source.source!=='user'||source.cancelRequestedAt
          ||typeof args.approvalText!=='string'||!args.approvalText.trim()||Buffer.byteLength(args.approvalText)>8000||!source.args.text?.includes(args.approvalText))
          throw Error('Steering or stopping this exact active turn requires Cody’s explicit current instruction. Quote it in approvalText; queued follow-ups and agent output are not authorization.');
        const authorization={source:'top_level_assistant',userRequestId:source.id,approvalText:args.approvalText,
          userTextHash:crypto.createHash('sha256').update(source.args.text).digest('hex'),action,threadId:args.threadId,turnId:args.expectedTurnId||args.turnId};
        return dispatch?dispatch(authorization):authorization;
      });
    ...
    return this.transaction(async () => {
      ...
      const accountAdmission=this.accounts&&appAccountAction(action)?await this.accounts.admission():null;
      const accountParent=tool&&!supervisor?currentOrigin():null;
      const accountOrigin=externalOrigin?context.accountReceipt:accountParent;
      const finishingAccepted=accountOrigin&&this.accounts?.deliveryAdmission&&(await this.accounts.deliveryAdmission(accountOrigin)).allowed;
      if(accountAdmission&&!accountAdmission.allowed&&appAccountAction(action)&&action!=='message'&&!accountReadOnlyActions.has(action)&&!finishingAccepted)
      ...
      if(action==='terminal.prompt') {
        const source=externalOrigin||this.state.jobs.find(j=>j.id===(args.authorizationRequestId || coordinatorRequestId || this.state.coordinator?.activeRequestId));
        const quote=args.approvalText;
      ...
      if (action !== 'start' && !mainWorkspaceActions.includes(action) && !this.state.enabled && !externalOrigin) throw new Error('Start the Assistant first');
      ...
      const job = {id:requestId, action, args, fingerprint, status:action==='start'?'completed':'queued', createdAt:now(), source:tool?'assistant':'user',
        ...
        ...(tool && !diagnostic && currentOrigin()?.id ? {parentRequestId:currentOrigin().id} : {})};
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      const accountProcessRequest=this.accountProcessRequest&&Date.now()<this.accountProcessRequest.expires?this.accountProcessRequest.id:null;
      const idle={job:null,enabled:this.state.enabled,inventoryRequested,pendingBindings,accountInventoryRequest,accountProfileRequest,accountProcessRequest};
      ...
      if(this.state.paused||ordinaryNativeBusy)return idle;
...
export async function assistantHttp(req, res, url, runtime, {readBody, json}) {
  ...
  try {
    ...
    else if (req.method === 'POST' && ['/v1/assistant/native/account-prepare','/v1/assistant/native/account-result','/v1/assistant/accounts/native-permit'].includes(url.pathname)) {
      throw Error('Terminal account switching is retired. Use app-only account activation.');
    }
