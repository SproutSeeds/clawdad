...
const terminalTextActions = new Set(['terminal.send','terminal.queue','terminal.insert','terminal.clear','terminal.replace','terminal.append','terminal.native.type','terminal.key','terminal.images','terminal.pointer']);
...
const publicActions = new Set(['state', 'start', 'message', 'voice.timing', 'terminal.focus', 'cancel', 'pause','destination']);
...
export class AssistantRuntime {
  ...
  snapshot(knownHistoryRevision) {
    ...
    return {version:1, conversationMode:'background', imageAttachments:true, chatCapacity:assistantChatCapacity, enabled:this.state.enabled, paused:this.state.paused,
  ...
  async command(request, {tool = false, supervisor = null, observationOnly = false} = {}) {
    ...
    if(request?.action?.startsWith('appserver.')){
      ...
      const meta=await this.transaction(()=>{
        ...
        if(this.state.paused&&!['appserver.list','appserver.inspect','appserver.history','appserver.workspaces','appserver.reconcile'].includes(action))throw new Error('Mac control is paused.');
        return {source:'assistant',...(args.diagnostic?{visibility:'diagnostic'}:{parentRequestId:this.state.coordinator?.activeRequestId})};
      });
    ...
    if(request?.action==='mainworkspace.status'){
      ...
      return {mainWorkspace:await readMainWorkspace(this.root),catalog:this.observation?.catalog||recent,paused:this.state?.paused||false,...(request.jobId?{job:await this.job(request.jobId)}:{})};
    }
    if(supervisor){
    ...
    return this.transaction(async () => {
      ...
      if(mainWorkspaceActions.includes(action) && !['mainworkspace.inspect','mainworkspace.restore'].includes(action) && (!Number.isSafeInteger(args.expectedRevision)||args.expectedRevision<1))throw new Error('Refresh the saved workspace revision first');
      ...
      if(action==='message')job.destination=structuredClone(this.state.destination);
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      for (const binding of observation.bindings || []) {
        const job=this.state.jobs.find(j=>j.id===binding.id && j.preparedAt && !j.conversationPath && !j.bindingEnded);
        ...
        if (binding.processChanged === true) {
          job.bindingEnded=true;job.status='attention';job.error='The original Codex process ended or changed before its conversation was identified. Inspect the original receipt; input will not be repeated.';bindingsChanged=true;
      ...
      this.schedule();
      const pendingBindings=this.state.jobs.filter(j=>j.agentInstanceId && j.preparedAt && !j.conversationPath && !j.bindingEnded
        && ['running','inserted','submitted','attention'].includes(j.status)).slice(0,64)
      ...
      const idle={job:null,enabled:this.state.enabled,inventoryRequested,pendingBindings};
      ...
      return {job:{...job},enabled:this.state.enabled,inventoryRequested,pendingBindings};
    });
  }
  ...
  async nativeResult({id, result, error, deferred = false}) {
    return this.transaction(async () => {
      ...
      if (deferred) {
        ...
        if((job.action==='terminal.key' && typeof result?.keySent==='boolean') || (job.action==='terminal.queue' && typeof result?.tabSent==='boolean')) job.result=result;
      }
      else {
        ...
        if (job.status === 'completed' && ['terminal.new','terminal.focus','terminal.move','mainworkspace.inspect'].includes(job.action)
  ...
  async drain() {
    while(!this.closed){
      ...
      try{
        ...
        await this.coordinator.run({...next,images,modelConfig,
          ...
          onMessage:message=>this.transaction(async()=>{
            ...
            job.response=message.text;await this.save();
          }),
        });
