...
const mutating = new Set(['start', 'message', 'terminal.send', 'terminal.insert', 'terminal.focus', 'terminal.move', 'terminal.close', 'terminal.close.resolve', 'computer.input', 'computer.open']);
...
export class AssistantRuntime {
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      const route = j => `${['terminal.send','terminal.insert'].includes(j.action)?'terminal.text':j.action}:${j.args.tabId || ''}`;
