...
const terminalTextActions = new Set(['terminal.send','terminal.insert','terminal.clear','terminal.replace']);
const mutating = new Set(['start', 'message', 'terminal.send', 'terminal.insert', 'terminal.focus', 'terminal.move', 'terminal.close', 'terminal.close.resolve', 'computer.input', 'computer.open', ...inputEdits]);
  ...
  async command(request, {tool = false} = {}) {
    return this.transaction(async () => {
      ...
      if (action === 'voice.timing') {
        ...
        job.voiceTiming = Object.fromEntries(entries);
        await this.save();
        return {ok:true};
      }
      if (action === 'cancel') {
      ...
      } else if (action === 'message' || action === 'terminal.send' || action === 'terminal.insert') {
