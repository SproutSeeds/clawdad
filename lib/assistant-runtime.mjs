...
export class AssistantRuntime {
  ...
  async command(request, {tool = false, supervisor = null, observationOnly = false} = {}) {
    ...
    if (request?.action?.startsWith('research.')) {
      const assistantActions=['research.status','research.history','research.configure','research.start','research.pause','research.off','research.resume','research.restart','research.clear','research.steer'];
      if (tool && !assistantActions.includes(request.action)) throw new Error('Use the Assistant research management tools for Cody’s explicit instructions. Budget overrides remain in Cody’s bounded allowance controls.');
