...
export class AssistantRuntime {
  ...
  async command(request, {tool = false, supervisor = null, observationOnly = false} = {}) {
    ...
    if(request?.action?.startsWith('accounts.')){
      ...
      if(tool&&!['accounts.status','accounts.preview','accounts.reconcile'].includes(request.action))await this.transaction(()=>{
    ...
    return this.transaction(async () => {
      ...
      if(accountAdmission&&!accountAdmission.allowed&&action!=='message'&&!['terminal.inspect','terminal.observe','terminal.context','terminal.native.inspect','mainworkspace.status','mainworkspace.inspect'].includes(action))
  ...
  async nativePoll(observation = {}) {
    return this.transaction(async () => {
      ...
      if(this.accounts&&!(await this.accounts.admission()).allowed)return idle;
  ...
  async drain() {
    while(!this.closed){
      const next=await this.transaction(async()=>{
        ...
        if(this.accounts&&!(await this.accounts.admission()).allowed)return null;
