...
export async function readAccountWindow(root,id){
  ...
  try{
    ...
    if(record.draftPolicy!==undefined&&record.draftPolicy!==null&&record.draftPolicy!=='retainOnly')throw Error();
    for(const entry of record.entries){
      const retainedOnly=record.draftPolicy==='retainOnly'&&entry.draft?.text==null;
      if(!validId(entry.id)||!path.isAbsolute(entry.directory||'')||!['shell','codex'].includes(entry.kind)
        ||!retainedOnly&&(typeof entry.draft?.text!=='string'||Buffer.byteLength(entry.draft.text)>1024*1024)
...
export class CodexAccountWindowSwitch {
  ...
  async inspect(observed,operation){
    ...
    let captured=await readAccountWindow(this.adapter.root,operation.id);
    ...
    let included=observed.consumers.filter(c=>c.kind==='shared_app_server'||c.kind==='terminal_codex'&&
    ...
    if(!captured){
      // A cold tab has no cached AX-tab binding until this native capture visits
      // it. Capture only the explicitly selected physical window, then bind its
      // verified TTY/process/session evidence against a fresh process census.
      // Unknown processes in other windows are never visited or transitioned.
      await this.adapter.accounts.switchStage(operation.id,'capturing_window');
      await this.call(operation,'window.capture',{selection});
      captured=await readAccountWindow(this.adapter.root,operation.id);
      observed=await this.adapter.inventory();
    }
    if(!captured)throw failure('account_window_capture_pending','The native window capture is still pending. Keep the same switch request.');
    for(const tab of captured.tabs){
      const owners=observed.consumers.filter(c=>c.kind==='terminal_codex'&&c.tty===tab.tty);
      if(tab.kind==='codex'&&!tab.historical
        ?owners.length!==1||owners[0].processIdentity!==tab.owner||owners[0].sessionId!==tab.sessionId||owners[0].directory!==tab.directory
        :owners.length!==0)
        throw failure('account_window_owner_changed',`${tab.name||'A selected tab'}: its process or conversation changed during verification. Nothing was closed. Cancel and review this window.`);
    }
    included=observed.consumers.filter(c=>c.kind==='shared_app_server'||c.kind==='terminal_codex'&&captured.tabs.some(t=>t.tty===c.tty&&t.owner===c.processIdentity));
    if(!observed.complete||included.some(c=>c.busy!==false||c.pendingReceipts?.length))return {...observed,consumers:included};
    const current=windowConsumer(captured,selection);
    return {...observed,consumers:[current,...included.filter(c=>c.kind==='shared_app_server')],
      windowRecord:captured,outsideCount:observed.consumers.length-included.length};
  }
