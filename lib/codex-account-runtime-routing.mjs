...
import {CodexAppAccounts as CodexAccounts} from './codex-app-accounts.mjs';
...
export async function accountRouteForLegacyRequest({accounts,binary,requestId,projectPath}={}){
  ...
  return {accounts,receipt,launch:await accounts.selectedLaunch({allowUnselected:!!receipt})};
}

export async function ensureAccountSharedRuntime(options,{accounts,receipt=null,
  ensure=ensureCodexSharedRuntime,status=codexSharedRuntimeStatus,exclusive=withAccountSharedGate,verify=verifyAppAccountRuntime}={}){
  ...
  return exclusive(socketPath,async()=>{
    ...
    if(!gate.allowed){
      ...
      throw Object.assign(Error(gate.reason),{code:gate.reasonCode||'account_switch_pending'});
    }
    if(receipt)await accounts.assertDelivery(receipt);
    const accountLaunch=await accounts.selectedLaunch();
    const runtime=await ensure({...options,socketPath,accountLaunch,env:accountLaunch?.env||options.env});
    if(accountLaunch&&runtime.mode!=='isolated')await verify({socketPath,launch:accountLaunch});
    return runtime;
  });
}
