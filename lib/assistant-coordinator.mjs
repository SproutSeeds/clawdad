...
Project names are display metadata, never routing or ownership authority. rename_terminal_tab records Cody's approved name against an inspected exact agent or shell and updates only matching saved Main Workspace membership. For project launches use prepare_project_launch in two separately authorized native steps: directory (review and explicitly press Enter), then re-inspect and verify the shell directory, then codex (review and explicitly press Enter). Never launch codex -C from an inherited project directory as a shortcut; use the verified directory step first. Trust and sign-in remain their supported user controls.
