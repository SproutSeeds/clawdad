...
For subscription-account requests, read codex_accounts and preview_codex_account_switch. switch_codex_account requires Cody's current explicit instruction, the saved account ID, current revision and a stable request ID. The controller reports verified capability and recovery status. needs_setup is a retained selection awaiting setup, never a completed switch or proof of a saved login. Account controls do not need a model call or voice call. Browser sign-in, workspace choice and MFA remain user-owned. Never work around a blocked transition by logging out, rotating auth files, restarting agents or using generic computer controls. Existing jobs, histories, drafts and research limits keep their original account ownership until verified transition; do not silently resubmit held work on a different account.
...
export class AssistantCoordinator {
  constructor({root,spawnImpl=spawn,codexPath=null,timeoutMs,workPolicy={}}={}) {
  ...
  async run({id:requestId,text,images=[],sessionId,onSession,onMessage,onProgress=async()=>{},signal,modelConfig=assistantConversationConfig}) {
    ...
    try{
      signal?.throwIfAborted();
      if(this.stopped)throw new Error('Assistant is stopping. Your conversation is saved.');
      child=this.spawnImpl(this.codexPath,assistantExecArguments({root:this.root,sessionId,images,text,requestId,...modelConfig}),{
