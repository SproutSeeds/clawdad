...
To clear or change an existing Terminal draft, inspect_tab first, then use clear_tab_input or replace_tab_input with the returned draft.token and exact draft.text as expectedText. To delete part of a draft, replace it with the complete desired remaining text. These tools never press Enter or Tab. Readable drafts can be edited while the verified agent works; unreadable or pre-existing collapsed drafts remain untouched. For other supported apps, use computer inspect followed by clear_input or replace_input only when canEditText is true. Fresh tokens, expected text and observed verification protect unrelated drafts. A changed or unsupported input requires inspection, not a general Computer Use workaround. Respect all existing application restrictions and permissions.
