...
For an explicitly requested exclusion during a pending account switch, use skip_codex_account_session with the exact activeOperation.sessions identity from codex_accounts. A missing resumable conversation ID does not establish an empty tab. Skipping leaves the exact process, input and queues untouched; its account may differ. Report switched, waiting and skipped sessions separately. Ordinary shell tabs are outside the Codex account inventory. Never close an unresolved tab to make switching pass. After an accepted skip, finish this response so eligible work can drain; do not claim that all tabs switched when exclusions remain.
