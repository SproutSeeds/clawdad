...
import {turnControlInstructions} from './assistant-turn-control-instructions.mjs';
...
Discuss ideas until the user asks for action. Send approved project tasks through send_to_tab with a stable UUID for each intended delivery. Use the captured destination preference and the user's explicit target to choose a ClawDad app-server thread or an existing visible Terminal agent tab. Preserve drafts and queue work for busy agents. Check task_status after uncertain delivery; never duplicate the task.
...
The conversation runs through ClawDad's subscription-authenticated app server and does not own a Terminal tab. Keep the same conversation across calls. Do not open a Terminal window for yourself.
`;
...
For subscription-account requests, read codex_accounts and preview_codex_account_switch. A preview only reads a saved account and allowance. switch_codex_account activates that account for subsequent ClawDad app-server, main Assistant and supervisor-review work on this connected Mac. Require Cody’s current explicit instruction, exact saved account ID, revision and stable request ID. Existing accepted app work finishes with its original account; report the receipt and end this response so it can finish. Terminal authentication and all windows, tabs, drafts and native queues stay independent. Never wrap or block the Codex CLI, inspect or type /status in tabs, stop Terminal agents, or recreate windows for account activation. Manual workspace snapshots remain independent. control_codex_account_switch checks, cancels before transition or retries the same activation using its receipts. Retired whole-window switches are preserved as history and cannot be continued. Browser sign-in, workspace choice and MFA remain user-owned. Account activation never submits a message, enables a supervisor, releases a manual pause, transfers an account-specific budget exception or enables API billing.
...
export function assistantExecArguments({sessionId=null, images=[], text=null, model=assistantConversationConfig.model,
  ...
  // The headless coordinator cannot answer a second MCP permission dialog.
  // Trust only this app-owned discovery/receipt/control workflow at transport
  // level. Mutating controls still require the current user message, exact
  // identity, owner/account fences and a durable receipt in AssistantRuntime.
  // Other tools retain their existing policy; this is per-child configuration,
  // never a change to Terminal's config or the target turn's tool approvals.
  for(const tool of ['workspace','list_workspaces','list_threads','inspect_thread','read_thread_history',
    'task_status','reconcile_thread_request','steer_thread','interrupt_thread']){
    config[`mcp_servers.clawdad_assistant.tools.${tool}.approval_mode`]='approve';
  }
...
export class AssistantCoordinator {
  ...
  async run({id:requestId,text,images=[],sessionId,onSession,onMessage,onProgress=async()=>{},signal,modelConfig=assistantConversationConfig}) {
    if(this.turnRunner){
      await this.prepare();
      return this.turnRunner.run({id:requestId,text,images,sessionId,onSession,onMessage,onProgress,signal,modelConfig});
    }
    await this.prepare();
  ...
  cancel(requestId){
    if(this.turnRunner)return this.turnRunner.cancel(requestId);
  ...
  stop(){this.stopped=true;this.turnRunner?.stop();this.cancelActive?.('The Mac service stopped during this response. Your request and action receipts are saved; review them before continuing.');}
}
