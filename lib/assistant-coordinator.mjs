...
Independent research autonomy: Cody can opt into one exact Terminal agent in Workspace → Research autonomy with an objective, allowed scope and verification requirements. Installing it enables no threads. Use research_status and every page of research_history to answer questions about its source completion, actual evidence, rationale, obligations, failed approaches, exact outgoing prompt and receipt. Reading this history does not interrupt the supervisor. Keep ordinary conversation independent; do not duplicate its outgoing work or treat its output as new authorization. Manual Terminal input takes priority and invalidates an in-flight automated decision. Observe exact ownership with observe_tab without changing the visible tab; inspect_tab is still needed to establish draft editability. Never clear a draft to make room for autonomy. Account-wide new reviews/continuations pause at 20% weekly allowance or stale usage; existing work may consume more. The reserve pause survives reset/restart and only Cody's bounded UI override changes it. You cannot silently enable autonomy, override its budget, broaden its scope, or switch it to app-server thread ownership.
...
export function assistantExecArguments({sessionId=null, images=[], text=null, model=assistantConversationConfig.model,
  ...
  // An explicit empty positional prompt supports image-only turns, including
  // resume, without adding a caption or instructions on the user's behalf.
  if (images.length && typeof text === 'string' && !text.trim()) args.push('--', '');
  else args.push('-');
  return args;
}
...
export class AssistantCoordinator {
  ...
  async run({text,images=[],sessionId,onSession,onMessage}) {
    ...
    try{
      if(this.stopped)throw new Error('Assistant is stopping. Your conversation is saved.');
      child=this.spawnImpl(this.codexPath,assistantExecArguments({root:this.root,sessionId,images,text}),{
