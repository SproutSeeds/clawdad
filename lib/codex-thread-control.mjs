...
export class CodexSharedClient {
  ...
  async request(method,params={}){await this.connect();return this.raw(method,params);}
...
export async function codexProcessOwners({run=exec,socketPath=codexSharedSocketPath()}={}){
  ...
  const files=(await run('/usr/sbin/lsof',['-n','-P','-p',pids.join(','),'-Fpn'],{timeout:5000,maxBuffer:16*1024*1024})).stdout;
...
export function classifyThreadOwner(threadId,owners,loaded){
  ...
  if(foreign.length)return {kind:foreign.length===1&&foreign[0].tty!=='??'?'terminal':'other_runtime',processes:foreign,
    reason:'This conversation is owned by another live Codex process. Use its exact Terminal/native transport; app-server resume and delivery are blocked.'};
