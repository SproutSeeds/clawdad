...
import {pendingTurnControl} from './assistant-turn-control.mjs';
...
export async function captureAccountAppServerLocal(root,threadId){
  ...
  return {draftHash:hash(draft),receiptsResolved:!state.jobs.some(job=>(job.threadId||job.args?.threadId)===threadId&&(pending.has(job.status)||pendingTurnControl(job)))};
}
