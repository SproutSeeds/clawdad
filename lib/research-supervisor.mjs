...
export class ResearchSupervisor {
      ...
      if(!this.state){
        ...
        for(const thread of Object.values(this.state.threads)){
          if(thread.status==='reviewing'){
            const receipt=this.state.accountReviews?.[thread.activeAccountReview];
            if(receipt)receipt.status='retained_interrupted';
        ...
        if(recoveredReview)await this.save();
      }
      return fn();
  ...
  async accountActivated(selection){
    await this.budget.transaction(()=>{});
    await this.transaction(async()=>{
      for(const t of Object.values(this.state.threads)){
        if(t.accountKey===selection.accountKey)continue;
        const policy=this.budget.state.accounts[t.accountKey]?.policies?.[t.id];
        const previous=t.accountKey;
        if(policy?.mode==='override'){
          // An account-specific exception cannot authorize a different account.
          if(t.enabled&&!['paused','off','complete','blocked','needs_user','conflict'].includes(t.status))t.status='paused';
          t.reason='ClawDad’s account changed. Review this supervisor’s account-specific limit before resuming.';
          this.notice(t,'pause',t.reason);
        }else{
          t.accountKey=selection.accountKey;
          this.entry(t,'account','Future supervisor reviews use the explicitly activated ClawDad account. Terminal authentication is unchanged.',{previousAccountKey:previous,accountKey:t.accountKey,activationId:selection.operationId});
        }
      }
      await this.save();
    });
  ...
  async step(t){
    ...
    const receiptID='review:'+reviewId, fingerprint=researchHash(JSON.stringify({reviewId,config,completion,modelConfig}));
    const persist=stamp=>this.transaction(async()=>{
      if(!valid())throw Error('The supervisor changed before review admission.');
      t.status='reviewing';t.reason='';t.activeAccountReview=receiptID;
      this.entry(t,'review','Reviewing the completed response and approved evidence.',{reviewId,generation,config,modelConfig,completion:snapshotCopy(completion),evidence:snapshotCopy(evidence)});
      const receipt={id:receiptID,action:'research.review',fingerprint,status:'running',...stamp};
      (this.state.accountReviews||={})[receiptID]=receipt;await this.save();return receipt;
    });
    if(this.runtime.accounts?.withWorkAdmission)await this.runtime.accounts.withWorkAdmission({id:receiptID,action:'research.review',fingerprint},persist);
    else await persist({});
    const controller=new AbortController();this.reviewAbort={threadId:t.id,revision,controller};
    let result;
    try{if(!valid())return;result=await this.reviewer.run({id:reviewId,config,completion,evidence,modelConfig,
      history:priorHistory,signal:controller.signal});}
    catch(e){if(!valid()||controller.signal.aborted)return;throw e;}
    finally{
      if(this.reviewAbort?.controller===controller)this.reviewAbort=null;
      await this.transaction(async()=>{const receipt=this.state.accountReviews?.[receiptID];if(receipt)receipt.status='completed';await this.save();});
    }
    if(!valid())return;
