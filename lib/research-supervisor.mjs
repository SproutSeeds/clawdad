...
export class ResearchSupervisor {
    ...
    if(action==='research.budget'){
      await this.transaction(async()=>{
        ...
        for(const thread of affected){
          ...
          const text=args.scope==='account_default'?`Cody approved a shared default of ${args.threshold}% weekly allowance remaining.`:
            args.mode==='default'?'Cody selected the shared default for this supervisor.':`Cody approved a ${args.threshold}% weekly allowance reserve for this supervisor, through the current weekly cycle.`;
  ...
  async step(t){
    ...
    await this.transaction(async()=>{if(!valid())return;t.status='reviewing';t.reason='';this.entry(t,'review','Reviewing the completed response and approved evidence.',{reviewId,generation,config,completion:snapshotCopy(completion),evidence:snapshotCopy(evidence)});await this.save();});
    if(!valid())return;
    const controller=new AbortController();this.reviewAbort={threadId:t.id,revision,controller};
    let result;
    try{result=await this.reviewer.run({id:reviewId,config,completion,evidence,
