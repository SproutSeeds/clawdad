...
// An expired source login cannot prove usable subscription access. A held
// source can still be replaced using its exact OS/socket/home identity and
// fully verified idle conversation snapshots. Destinations always require
// fresh subscription verification. The empty-source shape supports old receipts.
export const emptySharedSourceAccountUnavailable=s=>s?.emptySourceAccountUnavailable===true
  &&s.accountVerified===false&&s.accountKey===null&&Array.isArray(s.threads)&&s.threads.length===0
  &&absolute(s.authorizationHome)&&absolute(s.executable)&&Array.isArray(s.serverOptions);
export const idleSharedSourceAccountUnavailable=s=>s?.sourceAccountUnavailable===true
  &&s.accountVerified===false&&s.accountKey===null&&digest(s.sourceAccountIdentityHash)
  &&Array.isArray(s.threads)&&s.threads.every(safeThread)
  &&absolute(s.authorizationHome)&&absolute(s.executable)&&Array.isArray(s.serverOptions);
const sourceAccountUnavailable=s=>emptySharedSourceAccountUnavailable(s)||idleSharedSourceAccountUnavailable(s);
...
export function validateSharedAccountCapture(source){
  if(!source||!absolute(source.socketPath)||!id(source.processIdentity)||!Number.isSafeInteger(source.pid)||source.pid<=0
    ||!(digest(source.accountKey)&&source.accountVerified===true||sourceAccountUnavailable(source))||source.inventoryComplete!==true
  ...
  async run({operationId,source,target,confirmed=false}){
    ...
    try{
      ...
      const original=v=>v.kind==='server'&&v.processIdentity===source.processIdentity&&v.pid===source.pid
        &&(sourceAccountUnavailable(source)
          ?sourceAccountUnavailable(v)&&(!source.sourceAccountIdentityHash||v.sourceAccountIdentityHash===source.sourceAccountIdentityHash)
            &&v.authorizationHome===source.authorizationHome&&v.executable===source.executable&&hash(v.serverOptions)===hash(source.serverOptions)
      ...
      if(original(v)){
        ...
        if(source.accountKey===target.accountKey&&source.authorizationHome===target.authorizationHome){r.phase='verified';r.alreadySelected=true;await this.save(file,r);return structuredClone(r);}
