...
// An expired login cannot prove an old account. An explicitly captured empty
// server can still be replaced by its exact OS/socket identity. This exception
// never applies to a destination or to a server retaining any conversations.
...
export function validateSharedAccountCapture(source){
  if(!source||!absolute(source.socketPath)||!id(source.processIdentity)||!Number.isSafeInteger(source.pid)||source.pid<=0
    ||!(digest(source.accountKey)&&source.accountVerified===true||emptySharedSourceAccountUnavailable(source))||source.inventoryComplete!==true
  ...
  async run({operationId,source,target,confirmed=false}){
    ...
    try{
      ...
      const original=v=>v.kind==='server'&&v.processIdentity===source.processIdentity&&v.pid===source.pid
        &&(emptySharedSourceAccountUnavailable(source)
          ?emptySharedSourceAccountUnavailable(v)&&v.authorizationHome===source.authorizationHome&&v.executable===source.executable&&hash(v.serverOptions)===hash(source.serverOptions)
