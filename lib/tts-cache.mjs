...
export function createTtsAudioId({
  ...
  if (Number(speed) !== 1) hash.update(`\0speed:${Number(speed)}`);
  return hash.digest("hex").slice(0, 40);
}

export function splitTtsPlaybackText(text, {chunkChars = 600} = {}) {
  const normalized = normalizeTtsText(text);
  const firstSentence = normalized.slice(0, 240).match(/^(.{30,}?[.!?。！？])(?:\s|$)/u)?.[1];
  if (!firstSentence || firstSentence.length === normalized.length) return splitTtsText(normalized, {chunkChars});
  return [firstSentence, ...splitTtsText(normalized.slice(firstSentence.length), {chunkChars})];
}
...
export function resolveTtsRuntimeConfig({ env = process.env, config = {} } = {}) {
  ...
  if (provider === docReaderTtsProvider) {
    ...
    return {
      ...
      engine,
      speed: Number(ttsConfig.speed || docReaderConfig.speed || 1),
      libraryUrl,
...
export async function ttsManifestIsReady(projectPath, manifest) {
  ...
  for (const part of manifest.parts) {
    ...
    if (part.audioHash) {
      const data = await readFile(ttsAudioFilePath(projectPath, manifest.audioId, part.fileName));
      if (crypto.createHash("sha256").update(data).digest("hex") !== part.audioHash) return false;
    }
  }
  return true;
}
...
async function synthesizeDocReaderSpeechFromUrl({
  ...
  try {
    if (!response.ok) {
      ...
      const detailText = detail ? `: ${detail.slice(0, 500)}` : "";
      throw Object.assign(new Error(`Doc Reader TTS request failed with status ${response.status}${detailText}`), {retryable: [502, 503, 504].includes(response.status)});
    }
    const audio = Buffer.from(await response.arrayBuffer());
    if (audio.length === 0) throw new Error("Doc Reader TTS response did not include audio");
    return audio;
  } catch (error) {
    if (error?.name === 'AbortError') throw new Error(`Doc Reader TTS request timed out after ${requestTimeoutMs}ms`);
    throw error;
  ...
  for (const url of urls) {
    ...
    try {
      return await synthesizeDocReaderSpeechFromUrl({
        ...
        voiceId,
        speed,
        requestTimeoutMs: Math.max(1000, requestTimeoutMs - (Date.now() - started)),
        text,
        fetchImpl,
      });
    } catch (error) {
      const transient = error.retryable || ['ECONNREFUSED', 'ECONNRESET', 'UND_ERR_SOCKET'].includes(error.cause?.code || error.code);
      if (attempt === 0 && transient && Date.now() - started + 1500 < requestTimeoutMs) {
...
export async function ensureCachedTtsAudio({
  ...
  const chunks = splitTtsPlaybackText(normalizedText, { chunkChars: runtimeConfig.chunkChars });
  ...
  if (await ttsManifestIsReady(projectPath, cachedManifest)) {
    return {
      cached: true,
  ...
  const chunkTextHashes = chunks.map(chunk => crypto.createHash("sha256").update(chunk).digest("hex"));
  const generationIdentity = crypto.createHash("sha256").update(JSON.stringify({
    provider: runtimeConfig.provider, engine: runtimeConfig.engine, model: runtimeConfig.modelId,
    voice: runtimeConfig.voiceId, speed: runtimeConfig.speed || 1, format: runtimeConfig.outputFormat,
    service: runtimeConfig.baseUrl, fallback: runtimeConfig.fallbackUrl, chunkTextHashes,
  })).digest("hex");
  // Published parts may already have been heard. Recovery only appends to an
  // independently verified prefix; it never regenerates or repartitions it.
  const parts = Array.isArray(cachedManifest?.parts) ? [...cachedManifest.parts] : [];
  if (parts.length) {
    if (cachedManifest.generationIdentity !== generationIdentity || parts.length > chunks.length) {
      throw new Error("Cached speech has a different voice or chunk plan. Its existing audio was preserved.");
    }
    for (const [index, part] of parts.entries()) {
      const data = await readFile(ttsAudioFilePath(projectPath, audioId, part.fileName));
      if (part.index !== index + 1 || part.textHash !== chunkTextHashes[index] || !part.audioHash
        || crypto.createHash("sha256").update(data).digest("hex") !== part.audioHash) {
        throw new Error("Cached speech failed its integrity check. Its existing audio was preserved.");
      }
    }
  }
  const baseManifest = {
    schema: "clawdad.tts-message/2",
    state: "generating",
    audioId,
    provider: runtimeConfig.provider,
    engine: runtimeConfig.engine || runtimeConfig.modelId,
    voiceId: runtimeConfig.voiceId,
    modelId: runtimeConfig.modelId,
    outputFormat: runtimeConfig.outputFormat,
  ...
  try {
    for (const [index, chunk] of chunks.entries()) {
      ...
      parts.push({
        ...
        textHash: chunkTextHashes[index],
