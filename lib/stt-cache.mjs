...
export const defaultDocReaderSttBaseUrl = "http://127.0.0.1:8766";
...
export async function transcribeDocReaderAudio({
  ...
  if (!fetchImpl) {
    throw new Error("fetch is not available for Doc Reader transcription");
  }
  const buffer = Buffer.isBuffer(audio) ? audio : Buffer.from(audio || "");
  if (buffer.length === 0) {
    throw new Error("missing audio");
  ...
  const response = await fetchImpl(`${baseUrl}/api/transcribe`, {
    method: "POST",
    headers,
    body: buffer,
  });
  ...
  if (!response.ok) {
    const detail = typeof body === "string"
      ? body
      : pickString(body?.error?.message, body?.message, JSON.stringify(body));
    throw new Error(`Doc Reader transcription request failed with status ${response.status}: ${detail.slice(0, 500)}`);
  }
  ...
  if (typeof text !== "string") {
    throw new Error("Doc Reader transcription response did not include text");
  }
