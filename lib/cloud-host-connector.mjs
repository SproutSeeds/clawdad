...
import {deliverNotificationOutboxes} from './notification-outbox-delivery.mjs';
...
import {TerminalNotificationMonitor, notificationStatePath, resolveTerminalNotification} from './terminal-notifications.mjs';
...
const cloudSpeechPrepareTimeoutMs = 5 * 60 * 1000;
...
export function defaultCloudHostCapabilities(platform = process.platform) {
  ...
  if (normalizeCloudHostPlatform(platform) === "macos") {
    shared.push(
      ...
      ...(process.platform === "darwin" && process.env.CLAWDAD_NATIVE_RUNTIME_VERSION ? ["assistant.local"] : []),
    );
  }
  return shared.sort();
}
...
async function localJson(config, pathname, options = {}) {
  ...
  if (!response.ok) {
    throw localResponseError(response.status, payload, `local ClawDad request failed with ${response.status}`);
  }
  return payload;
}

function localResponseError(status, payload, fallback) {
  return Object.assign(new Error(payload?.error || payload?.message || fallback), {
    status, errorCode: payload?.errorCode, audioState: payload?.audio?.state,
  });
}

async function localSpeechOperation(operation) {
  try { return await operation(); }
  catch (error) {
    const networkCode = error?.cause?.code || error?.code;
    const pending = error.errorCode === "audio_not_ready" ||
      (error.status === 404 && /audio is not ready/iu.test(error.message));
    const transient = pending || [408, 425, 429, 502, 503, 504].includes(error.status) ||
      error.name === "TimeoutError" ||
      ["ECONNREFUSED", "ECONNRESET", "EPIPE", "ETIMEDOUT", "UND_ERR_SOCKET", "UND_ERR_CONNECT_TIMEOUT"].includes(networkCode);
    // Only local readiness/transport failures enter the polling retry path.
    // Invalid audio, permissions, and failed generation keep their real error.
    if (transient && error.audioState !== "failed") error.retryableLocalSpeech = true;
    throw error;
  }
}
...
async function localTtsAudio(config, value) {
  return localSpeechOperation(async () => {
    ...
    if (!response.ok) {
      ...
      throw localResponseError(response.status, payload, `local ClawDad audio request failed with ${response.status}`);
    }
...
function replyEnvelope(type, sourceEnvelope, body, config) {
  return signOutgoingEnvelope({
    ...
    body: { ...body, inReplyTo: sourceEnvelope.id },
  }, config);
}
...
function cloudSpeechRequestPayload(body = {}, { poll = false } = {}) {
  ...
  if (!project && !(body.source === "remote-assist" && String(body.text || "").trim())) {
  ...
  return {
    ...
    ...(body.voiceSelection ? {voiceSelection: body.voiceSelection} : {}),
    prepare: true,
    poll,
  };
}

function waitForCloudSpeechPoll(delay = cloudSpeechPreparePollMs) {
  return new Promise((resolve) => setTimeout(resolve, delay));
}

async function prepareCloudSpeechAudio(config, body = {}, onProgress = null) {
  if (!body.voiceSelection) {
    // A single selection owns this read; Settings changes affect the next read.
    try {
      const settings = await localJson(config, "/v1/tts/voices", {signal: AbortSignal.timeout(10_000)});
      if (settings.selection) body = {...body, voiceSelection: settings.selection};
    } catch { /* Older hosts can still use their configured speech model. */ }
  }
  ...
  while (Date.now() - startedAt <= cloudSpeechPrepareTimeoutMs) {
    try {
      ...
      const prepared = await localSpeechOperation(() => localJson(config, "/v1/tts/message", {
        method: "POST",
        signal: AbortSignal.timeout(30_000),
      ...
      if (onProgress && Array.isArray(audio.parts) && audio.parts.length > 0 && state !== "failed") {
        await onProgress(audio, Boolean(prepared.cached));
      }
      ...
      if (!error.retryableLocalSpeech) throw error;
      transientFailures += 1;
      // Keep the same text, voice, job and transfer offset. The original tap
      // remains preparing; completed parts are never restarted or resent.
      await waitForCloudSpeechPoll(Math.min(5_000, cloudSpeechPreparePollMs * 2 ** Math.min(transientFailures - 1, 3)));
    }
    poll = true;
  }
  throw new Error("The Mac could not finish preparing this audio. Check that its speech service is available.");
}

async function sendCloudSpeechAudio(envelope, config, sendCloud, prepared, transfer = {partIndex: 0, totalBytes: 0}) {
  ...
  const partCount = Number(audio.chunkCount || parts.length);

  for (const [partIndex, part] of parts.entries()) {
    if (partIndex < transfer.partIndex) continue;
    const localAudio = await localTtsAudio(config, part?.url);
    transfer.totalBytes += localAudio.buffer.length;
    if (transfer.totalBytes > maxCloudSpeechAudioBytes) {
    ...
    transfer.partIndex = partIndex + 1;
  }
...
export async function handleCloudEnvelope(envelopeInput, config, sendCloud) {
  ...
  try {
    ...
    if (envelope.type === "notification.open.request") {
      const target = await resolveTerminalNotification(config, envelope.body?.eventId);
      await sendCloud(replyEnvelope("notification.opened", envelope, {...target, inReplyTo:envelope.id}, config));
      return {ok:true};
    }
    ...
    if (["speech.voices.request", "speech.voices.update"].includes(envelope.type)) {
      const settings = await localJson(config, "/v1/tts/voices", envelope.type === "speech.voices.update"
        ? {method: "POST", body: JSON.stringify({selection: envelope.body?.selection})} : {});
      await sendCloud(replyEnvelope("speech.voices", envelope, {
        ...settings, requestId: String(envelope.body?.requestId || ""),
      }, config));
      return {ok: true};
    }

    if (envelope.type === 'usage.request') {
      const usage = await localJson(config, '/v1/codex/weekly-usage');
      await sendCloud(replyEnvelope('usage.snapshot', envelope, {...usage, inReplyTo: envelope.id}, config));
      return {ok: true};
    }
    ...
    if (envelope.type === "speech.synthesize.request") {
      ...
      const transfer = {partIndex: 0, totalBytes: 0};
      let completed = false;
      await prepareCloudSpeechAudio(config, body, async (audio, cached) => {
        await sendCloudSpeechAudio(envelope, config, sendCloud, {audio, cached}, transfer);
        completed = audio.state === "ready";
      });
      if (!completed) throw new Error("The local audio transfer did not complete.");
      return { ok: true };
    }
...
export async function connectCloudHostOnce(config, { onConnected = () => {} } = {}) {
  await ensureCloudHostIdentity(config);
  ...
  const sendCloud = async (envelope) => {
    ...
    while (socket.bufferedAmount > 512 * 1024 && socket.readyState === WebSocket.OPEN) {
      if (Date.now() >= deadline) throw new Error("cloud-host audio transfer stalled");
...
`);
  ...
  const notifications = process.platform === 'darwin' ? new TerminalNotificationMonitor({
    statePath:notificationStatePath(config),
    deliver:async event => {
      // Routine autonomous cycles live in their compact decision history.
      // Supervisor milestone/pause/completion events have their own durable IDs.
      const research=await localJson(config,'/v1/assistant/research/notifications');
      if(research.suppressedSessions?.includes(event.sessionId))return;
      const result = await cloudAccessJson(config,'/notifications/events',{
        method:'POST',body:JSON.stringify({...event,hostName:config.hostName}),signal:AbortSignal.timeout(15_000),
      });
      if (!result.accepted) throw new Error('Notification relay is temporarily unavailable');
    },
  }) : null;
  let usageDeliveryPending = false;
  const usageDelivery = async () => {
    if (usageDeliveryPending) return;
    usageDeliveryPending = true;
    try {
      await deliverNotificationOutboxes({hostName:config.hostName,local:(...args)=>localJson(config,...args),
        relay:event=>cloudAccessJson(config,'/notifications/events',{
          method:'POST',body:JSON.stringify(event),signal:AbortSignal.timeout(15_000),
        })});
    } catch { /* Durable event IDs retry without issuing a new alert. */ }
    finally { usageDeliveryPending = false; }
  };
  const usageTimer = setInterval(() => { void usageDelivery(); }, 30_000); usageTimer.unref?.();
  try {
  do {
    try {
      ...
      const closed = await connectCloudHostOnce(config, { onConnected: () => {
        connectedOnce = true;
        failureCount = 0;
        notifications?.start();
        void usageDelivery();
  ...
  } finally { clearInterval(usageTimer); await notifications?.stop(); }
}
