...
`);
  ...
  const usageDelivery = async () => {
    ...
    try {
      for(const route of ['/v1/codex/weekly-usage','/v1/assistant/research']){
      const {events = []} = await localJson(config, route + '/outbox');
      for (const event of events) {
        const result = await cloudAccessJson(config, '/notifications/events', {
          method: 'POST', body: JSON.stringify({...event, hostName: config.hostName}), signal: AbortSignal.timeout(15_000),
        });
        if (result.accepted) await localJson(config, route + '/delivered', {method: 'POST', body: JSON.stringify({id: event.id})});
      }
      }
