...
export async function resolveCloudHostConfig(rawOptions = {}) {
  ...
  return {
    ...
    localToken: pickString(rawOptions.localToken, process.env.CLAWDAD_CLOUD_LOCAL_TOKEN, config.localToken),
...
async function persistCloudEntitlement(envelope, config) {
  ...
  const entitlement = {
    ...
    transactionId: String(verifiedStoreKit?.transactionId || "").trim(),
    originalTransactionId: String(
      verifiedStoreKit?.originalTransactionId || "",
    ).trim(),
    purchasedAt: String(verifiedStoreKit?.purchasedAt || "").trim(),
    ...
    environment: String(verifiedStoreKit?.environment || "").trim(),
    observedAt: String(body.observedAt || "").trim(),
    syncedAt,
    deviceId: envelope.sourceDeviceId,
...
export async function handleCloudEnvelope(envelopeInput, config, sendCloud) {
  ...
  try {
    ...
    if (envelope.type === "pair.request") {
      ...
      await sendCloud(replyEnvelope("pair.accepted", envelope, paired, config));
      return { ok: true };
    }
    ...
    if (envelope.type === "catalog.request") {
      ...
      if (project) {
        query.set("syncProject", project);
      }
      const snapshot = await localJson(config, `/v1/projects?${query.toString()}`);
      await sendCloud(replyEnvelope("catalog.snapshot", envelope, snapshot, config));
      return { ok: true };
    }
