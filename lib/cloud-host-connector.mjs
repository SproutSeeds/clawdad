...
`);
  ...
  const usageDelivery = async () => {
    ...
    try {
      const {events = []} = await localJson(config, '/v1/codex/weekly-usage/outbox');
      for (const event of events) {
        const result = await cloudAccessJson(config, '/notifications/events', {
          method: 'POST', body: JSON.stringify({...event, hostName: config.hostName}), signal: AbortSignal.timeout(15_000),
        });
        if (result.accepted) await localJson(config, '/v1/codex/weekly-usage/delivered', {method: 'POST', body: JSON.stringify({id: event.id})});
      }
