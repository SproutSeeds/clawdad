...
export class ResearchReviewRunner {
  constructor({root,binary='codex',resolveBinary=null,launch=spawn,timeoutMs=240_000}) { Object.assign(this,{root,binary,resolveBinary,launch,timeoutMs}); }
  async run({id,config,completion,evidence,history,signal,modelConfig={model:'gpt-6-astra',reasoningEffort:'medium'}}) {
    ...
    signal?.throwIfAborted();
    const child=this.launch(binary,args,{cwd:directory,stdio:['pipe','pipe','pipe']});
