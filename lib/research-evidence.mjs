...
export class ResearchReviewRunner {
  constructor({root,binary='codex',resolveBinary=null,launch=spawn,timeoutMs=240_000,resolveAccountLaunch=null,turnRunner=null}) { Object.assign(this,{root,binary,resolveBinary,launch,timeoutMs,resolveAccountLaunch,turnRunner}); }
  async run({id,config,completion,evidence,history,signal,modelConfig={model:'gpt-6-astra',reasoningEffort:'medium'}}) {
    ...
    await researchSave(path.join(directory,'input.json'),{config,completion,evidence,history,modelConfig});
    signal?.throwIfAborted();
    if(this.turnRunner){
      let output='';
      await this.turnRunner.run({id,text:prompt,cwd:directory,modelConfig,signal:signal?AbortSignal.any([signal,AbortSignal.timeout(this.timeoutMs)]):AbortSignal.timeout(this.timeoutMs),
        nativeTools:false,disableTools:true,ephemeral:true,source:'research_review',outputSchema:researchDecisionSchema,
        policy:{mode:'read-only',reviewer:'auto_review',nativeTools:false,computerUse:false},
        config:{web_search:'disabled',project_doc_max_bytes:0,'features.shell_tool':false,'features.unified_exec':false,
          'features.plugins':false,'features.multi_agent':false,'features.code_mode':false,'features.code_mode_host':false},
        onMessage:async message=>{output=message.text;}});
      let value;try{value=JSON.parse(output);}catch{throw Error('The supervisor did not return a structured review.');}
      validateResearchDecision(value,config,evidence);await researchSave(path.join(directory,'decision.json'),value);return value;
    }
    ...
    const args=['exec','--ignore-user-config','--ephemeral','--skip-git-repo-check','--json','--sandbox','read-only',
      '--model',modelConfig.model,'--config',`model_reasoning_effort=${JSON.stringify(modelConfig.reasoningEffort)}`,'--config','web_search="disabled"',
    ...
    args.push('-');
    const binary=this.resolveBinary?await this.resolveBinary():this.binary;
    const accountLaunch=await this.resolveAccountLaunch?.({requestId:id,kind:'research_review'});
    signal?.throwIfAborted();
    const child=this.launch(binary,withCodexAccountLaunch(args,accountLaunch),{cwd:directory,stdio:['pipe','pipe','pipe'],...(accountLaunch?{env:accountLaunch.env}:{})});
