...
const methods=new Set(['initialize','config/read','model/list','account/read','account/rateLimits/read',
...
export class CodexAccountProfileProcess {
  constructor({home,binary,launch=spawn,timeoutMs=30_000,configurationOnly=false}={}) {
    Object.assign(this,{home,binary,launch,timeoutMs,configurationOnly});this.pending=new Map();this.listeners=new Set();this.next=0;this.closed=false;
  }
  async connect() {
    if(this.configurationOnly){
      // Existing canonical homes may deliberately be 0755. Read configuration
      // without changing their permissions or creating a replacement home.
      const s=await fs.lstat(this.home);
      if(!path.isAbsolute(this.home)||path.normalize(this.home)!==this.home||!s.isDirectory()||s.isSymbolicLink()
        ||await fs.realpath(this.home)!==this.home||s.uid!==process.getuid()||s.mode&0o022)
        throw Error('The existing configuration home needs verified local ownership.');
    }else await privateAccountDirectory(this.home);
    const guard=await claimAccountProfile(this.home,{allowReadableHome:this.configurationOnly});
    try { return await this.connectGuarded(); } finally { await guard.release(); }
  }
  async connectGuarded() {
    ...
    try {
      ...
      if(!this.configurationOnly)await this.verifyStorage();return this;
    }catch(error){this.close();throw error;}
  }
  request(method,params={}) {
    if(!methods.has(method)||this.configurationOnly&&!['initialize','config/read'].includes(method)||this.closed||!this.child)return Promise.reject(failure());
