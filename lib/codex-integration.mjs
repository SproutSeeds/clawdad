...
function parseJsonLineStream(buffer) {
...
async function codexAppServerHooksList({ codexBinary, codexHome, projectPath }) {
  return await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      settle(reject, new Error("timed out waiting for codex app-server hooks/list"));
    }, 5000);
    ...
    child.stdout.on("data", (chunk) => {
      stdout += chunk;
      for (const message of parseJsonLineStream(stdout)) {
        if (message?.id === 2) {
          if (message.error) {
            settle(reject, new Error(message.error.message || JSON.stringify(message.error)));
            return;
          }
    ...
    setTimeout(() => {
      ...
      child.stdin.write(`${JSON.stringify(hooksList)}\n`);
    }, 100);

    child.once("exit", (code) => {
      if (!settled) {
        settle(reject, new Error(`codex app-server exited before hooks/list completed (code ${code ?? "unknown"}): ${stderr.trim()}`));
      }
    });
  });
}

async function codexHookTrustCheck({ codexHome, projectPath, hooksJsonPath }) {
  const codexBinary = process.env.CLAWDAD_CODEX || "codex";
  try {
    const result = await codexAppServerHooksList({ codexBinary, codexHome, projectPath });
    const entry = Array.isArray(result?.data)
      ? result.data.find((item) => path.resolve(String(item?.cwd || "")) === path.resolve(projectPath))
      : null;
    const hooks = Array.isArray(entry?.hooks) ? entry.hooks : [];
    const projectHooks = hooks.filter((hook) =>
      path.resolve(String(hook?.sourcePath || "")) === path.resolve(hooksJsonPath) &&
      (String(hook?.command || "").includes(hookScriptName) || String(hook?.command || "").includes("clawdad codex hook"))
    );
    if (!projectHooks.length) {
      return check("warn", "Codex hook trust", "Codex did not list Clawdad project hooks; project trust may be missing");
    }
    const needingTrust = projectHooks.filter((hook) => ["untrusted", "modified"].includes(String(hook?.trustStatus || "")));
    const disabled = projectHooks.filter((hook) => hook?.enabled === false);
    if (disabled.length) {
      return check("warn", "Codex hook trust", `${disabled.length} Clawdad hook${disabled.length === 1 ? "" : "s"} disabled in Codex`, {
        disabledCount: disabled.length,
        hooks: projectHooks,
      });
    }
    if (needingTrust.length) {
      return check("warn", "Codex hook trust", `${needingTrust.length} Clawdad hook${needingTrust.length === 1 ? "" : "s"} need review before they can run`, {
        reviewCount: needingTrust.length,
        hooks: projectHooks,
      });
    }
    return check("pass", "Codex hook trust", "Clawdad project hooks are trusted in Codex", {
      hooks: projectHooks,
    });
  } catch (error) {
    return check("warn", "Codex hook trust", `could not inspect Codex hook trust: ${error.message}`);
  }
}
...
export async function buildCodexIntegrationReport({
  ...
  } else if (/^\s*(?:codex_hooks|hooks)\s*=\s*false\s*$/mu.test(configText)) {
    checks.push(check("fail", "Codex hooks feature", ".codex/config.toml disables Codex hooks"));
  } else if (/^\s*codex_hooks\s*=/mu.test(configText)) {
    checks.push(check("warn", "Codex config", ".codex/config.toml uses deprecated features.codex_hooks; use features.hooks"));
  } else if (/^\s*hooks\s*=\s*true\s*$/mu.test(configText)) {
    checks.push(check("pass", "Codex config", ".codex/config.toml enables features.hooks"));
  } else {
    checks.push(check("warn", "Codex config", ".codex/config.toml is present but does not explicitly enable features.hooks"));
  }
  ...
  if (verifyHookTrust) {
    checks.push(await codexHookTrustCheck({
      codexHome,
      projectPath: normalizedProjectPath,
      hooksJsonPath: paths.hooksJson,
    }));
  } else {
    checks.push(check("warn", "Codex hook trust", "hook trust check skipped for bootstrap install"));
  }
