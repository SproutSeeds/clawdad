...
export class CodexAccounts {
  ...
  async observe({operation=null}={}){
    ...
    const inventory=await this.inspectConsumers({operation:drain.ready?operation:null});
    ...
    return {...identity,reading,drain,observedAt:new Date(this.clock()).toISOString(),fingerprint:hash(identity),
      reasons:(inventory.reasons||[]).filter(r=>clean(r,500)),inventoryDiagnostics:inventory.diagnostics??null,ready:drain.ready&&inventory.complete===true&&consumers.every(c=>c.recoverable&&c.busy===false
        &&c.accountVerified&&/^[a-f0-9]{64}$/.test(c.accountKey||'')&&!c.pendingReceipts.some(r=>!['completed','cancelled'].includes(r.status)))};
  }
  ...
  async advance(){
    ...
    try{
      ...
      try{
        if(op.phase==='preflight'){
          ...
          if(!observed.ready){
            ...
            const reason=!observed.drain.complete?'Accepted-work receipts could not be read completely. Restore that inventory before switching.'
              ...
              :pending.length?`Waiting for ${pending.length} accepted request${pending.length===1?'':'s'} to finish on the current account. New work is held; you can cancel this switch.`
            ...
            return await update({status:'waiting',reasonCode:!observed.drain.complete?'work_inventory_incomplete':unresolved?'work_receipts_unresolved':pending.length?'accepted_work_running':'sessions_not_ready',reason,observation:observed,consumers:observed.consumers});
          }
        ...
        if(recoveryReasons[error.code]){item.reasonCode=error.code;item.reason=recoveryReasons[error.code];}
        else item.reason=`Account ${item.phase==='authenticate'?'authentication':'session transition'} needs recovery. Check the original operation; uncertain actions will be reconciled before retrying.`;await save();});}
