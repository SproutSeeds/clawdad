...
import {validateSharedAccountCapture,emptySharedSourceAccountUnavailable} from './codex-account-shared-handoff.mjs';
...
function recoveryRecord(observed, captured) {
  ...
  const entries=observed.consumers.map(owner=>{
    ...
    if(matches.length!==1||entry.processIdentity!==owner.processIdentity||entry.sessionId!==owner.sessionId
      ||entry.directory!==owner.directory||entry.accountKey!==owner.accountKey
      ||owner.accountVerified!==true&&!(owner.kind==='shared_app_server'&&owner.emptySourceAccountUnavailable&&emptySharedSourceAccountUnavailable(entry.shared)))throw Error('Recovery owner changed');
    ...
    if(entry.shared){
      ...
      shared=Object.fromEntries(['kind','socketPath','processIdentity','pid','accountKey','accountVerified','emptySourceAccountUnavailable','inventoryComplete','dispatchHeld','authorizationHome','executable','serverOptions'].map(k=>[k,copy(value[k])]));
...
export class CodexAccounts {
  ...
  async observe({operation=null}={}){
    ...
    const consumers=(inventory.consumers||[]).map(c=>({id:c.id,kind:c.kind,pid:c.pid??null,processIdentity:c.processIdentity??null,
      ...
      emptySourceAccountUnavailable:c.kind==='shared_app_server'&&c.emptySourceAccountUnavailable===true&&c.accountKey===null&&c.accountVerified===false,
    ...
    return {...identity,reading,drain,observedAt:new Date(this.clock()).toISOString(),fingerprint:hash(identity),
      reasons:(inventory.reasons||[]).filter(r=>clean(r,500)),inventoryDiagnostics:inventory.diagnostics??null,ready:drain.ready&&inventory.complete===true&&consumers.every(c=>c.recoverable&&c.busy===false
        &&(c.kind==='terminal_window'?c.windowVerified:c.emptySourceAccountUnavailable||c.accountVerified&&/^[a-f0-9]{64}$/.test(c.accountKey||''))&&!c.pendingReceipts.some(r=>!['completed','cancelled'].includes(r.status)))};
  }
      ...
      try{
        if(op.phase==='preflight'){
          ...
          if(!observed.ready){
            ...
            // Before window capture, every idle tab has an uncaptured recovery
            // placeholder. Report the actual busy/uncertain owner first rather
            // than blaming unrelated drafts that have not been inspected yet.
            const actionable=blocked.filter(c=>c.busy!==false||c.reason||c.pendingReceipts?.some(r=>!['completed','cancelled'].includes(r.status)));
            const reported=immediate&&actionable.length?actionable:blocked;
            const details=reported.slice(0,3).map(c=>`${c.title||'A session'}: ${c.busy===true?'still working':c.busy===null?'activity could not be verified':c.reason||(immediate?'its window recovery capture has not completed':'its draft or pending deliveries need verification')}.`).join(' ');
