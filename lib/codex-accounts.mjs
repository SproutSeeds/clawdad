...
export class CodexAccounts {
  constructor({root=codexAccountsRoot(),usage,inspectConsumers=async()=>({complete:false,consumers:[],reasons:['Runtime inventory is unavailable.']}),
    adapter=null,authorizations=null,clock=Date.now,lease=acquireCodexDeliveryClaim}={}) {
    Object.assign(this,{root,usage,inspectConsumers,adapter,authorizations,clock,lease});this.file=path.join(root,'switch-state.json');this.lock=Promise.resolve();
  }
