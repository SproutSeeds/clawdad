      ...
      try{
        if(op.phase==='preflight'){
          ...
          if(!observed.ready){
            ...
            return await update({status:'waiting',reasonCode:!observed.drain.complete?'work_inventory_incomplete':unresolved?'work_receipts_unresolved':pending.length?'accepted_work_running':'sessions_not_ready',reason,observation:observed,consumers:observed.consumers,skippedConsumers:observed.skippedConsumers});
          }
