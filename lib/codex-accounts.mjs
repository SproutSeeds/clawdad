...
// The installed protocol has managed login, but no verified independently
// renewable multi-account slots or universal runtime-account adoption. This
// gate has no environment/UI bypass. Live adapters require reviewed evidence.
export const installedAccountSwitchCapabilities=Object.freeze({ready:false,managedLogin:true,
  retainedAuthorizations:false,consumerAccountAdoption:false,sessionTransition:false,
  reasons:[{code:'secure_account_storage_unverified',message:'Keeping both Codex accounts signed in independently still needs the isolated two-account verification.'},
    {code:'runtime_account_adoption_unverified',message:'Existing Terminal and app-server processes must be verified on the selected account before live switching is enabled.'}]});
...
export class CodexAccounts {
  constructor({root=codexAccountsRoot(),usage,inspectConsumers=async()=>({complete:false,consumers:[],reasons:['Runtime inventory is unavailable.']}),
    adapter=null,clock=Date.now,lease=acquireCodexDeliveryClaim}={}) {
    Object.assign(this,{root,usage,inspectConsumers,adapter,clock,lease});this.file=path.join(root,'switch-state.json');this.lock=Promise.resolve();
  }
  ...
  async stop(){
    ...
    this.runnerStarted=false;clearTimeout(this.runnerTimer);await this.runnerTask;
  }
  ...
  async snapshot(){
    ...
    return this.transaction(state=>({version:1,revision:state.revision,epoch:state.epoch,capabilities:copy(this.capabilities()),
      ...
      accounts:copy(state.accounts),usage,activeOperation:copy(state.operations[state.activeOperationId]||null),
  ...
  async add({email,workspaceLabel='',requestId,expectedRevision}){
    ...
    return this.transaction(async(state,save)=>{
      ...
      if(!Number.isSafeInteger(expectedRevision)||expectedRevision!==state.revision)throw rejected('The account list changed. Review the refreshed list before saving again.');
      ...
      state.requests[requestId]=receipt;await save();return copy(receipt);
    });
  }
  async observe(){
    ...
    return {...identity,reading,observedAt:new Date(this.clock()).toISOString(),fingerprint:hash(identity),
      reasons:(inventory.reasons||[]).filter(r=>clean(r,500)),ready:inventory.complete===true&&consumers.every(c=>c.recoverable&&c.busy===false
  ...
  async request({accountId,requestId,expectedRevision,confirmed}){
    ...
    return this.transaction(async(state,save)=>{
      ...
      if(!Number.isSafeInteger(expectedRevision)||state.revision!==expectedRevision)throw rejected('Account state changed. Review the refreshed state before selecting again.');
  ...
  async admission(){
    ...
    return {allowed:!op?.fenced,epoch:state.epoch,operationId:op?.id??null,
  ...
  async cancel({operationId,requestId}){
    ...
    return this.transaction(async(state,save)=>{
      ...
      state.requests[requestId]={fingerprint:input,operationId};await save();return copy(op);
    });
  }
  async advance(){
    ...
    try{
      ...
      try{
        if(op.phase==='preflight'){
          ...
          if(!observed.ready)return await update({status:'waiting',reason:'Waiting for running work, drafts, queues or unverifiable sessions.',observation:observed,consumers:observed.consumers});
        ...
        if(op.phase==='verify'){
          ...
          await this.transaction(async(s,save)=>{const item=s.operations[op.id];if(item.cancelRequested)return;
            ...
            const account=s.accounts.find(a=>a.id===item.targetId);Object.assign(account,{authentication:'verified',accountKey:item.destinationAccountKey,verifiedAt:item.completedAt});await save();});
        }
