...
export async function inspectManagedAccountConsumers(runtime,{socketPath,readNative=()=>runtime.accountConsumerInventory(),
  ...
  if(native?.complete!==true||native.processesComplete!==true||!Number.isFinite(native.processesObservedAt)
    ||Math.abs(clock()-native.processesObservedAt)>5000)
    return {complete:false,consumers:[],reasons:['Refresh the native process inventory. The installed Mac worker may need the account-control update.']};
  ...
  return {complete,consumers,reasons,excludedApplications:scoped.foreign.map(p=>({pid:p.pid,application:p.application})),helperCount:scoped.helpers.length};
}
