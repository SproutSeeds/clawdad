...
node bin/clawdad-certify record \
  ...
  --state pass \
  --evidence "Build 31 launched from a clean TestFlight install."
```
...
## Native Beta 10 And iPhone Build 31 Candidate Automation

| Check | State | Evidence |
| --- | --- | --- |
| Node integration suite | Pass | 448 tests passed on the merged release checkpoint |
| iPhone Swift unit suite | Pass | `swift test --package-path apps/ios/ClawDadMobile`: 35 tests passed |
| Remote Assist protocol suite | Pass | `swift test --package-path native/ClawDadRemoteAssistProtocol`: 22 tests passed |
| Mac Swift unit suite | Pass | `swift test --package-path native/macos`: 43 tests passed |
| Native runtime bundle and ownership | Pass | Release coverage verifies the Mac app embeds and owns runtime `0.7.0-beta.10` |
| Mac signed package | Pass | The local beta 10 DMG and ZIP checksums pass; Mac build 26 is installed on the paired Mac |
| Mac notarization and stapling | Pass | The app and DMG are notarized and stapled; Gatekeeper accepts installed build 26 |
...
| Candidate readiness identity | Pass | Snapshot logic requires native-private runtime beta 10, TestFlight build 31, installed iPhone build 31, and installed Mac build 26 before physical certification can become ready |
...
Command-T, composer Cut, keyboard-safe controls, and corrected Read Aloud
playback. The installed Mac lane is `0.7.0 (26)` with embedded runtime
`0.7.0-beta.10`. Physical certification remains separate from artifact and
channel verification and must be recorded against exact Mac build 26 and iPhone
build 31.
...
| Mac artifact and installation | Pass | Beta 10 build 26 is signed, notarized, stapled, checksum-verified, Gatekeeper-approved, and installed |
| Internal TestFlight companion | Pass | Build 31 is `VALID` and assigned only to `ClawDad Internal` |
...
| Check | State | Acceptance |
| --- | --- | --- |
| Native Mac workspace parity | Pending visual read-back | Build 26 is installed; fresh packaged-app UI inspection remains a physical gate |
| Fresh TestFlight install | Pending | Build 31 launches to the subscription or pairing surface without stale workspace flash |
...
| Remote Assist | Pending | Every connected display can be selected in one session; switching pauses input until commit; negative-origin click mapping, disconnect fallback, landscape, pointer, scrolling, commands, keyboard, clipboard, timeout, and retry work |
...
artifacts and installed build pass, TestFlight build 31 remains `VALID` in
...
iPhone and multi-display rows pass against installed Mac build 26 and TestFlight
build 31. External TestFlight, Beta App Review, and App Store submission require
