...
node bin/clawdad-certify record \
  ...
  --state pass \
  --evidence "Build 25 launched from a clean TestFlight install."
```
...
## Beta 6 And iPhone Build 25 Candidate Automation
...
| Candidate readiness identity | Pass | Snapshot logic requires exact npm beta 6, TestFlight build 25, installed iPhone build 25, and installed Mac build 23 before physical certification can become ready |
| Compact Remote Assist controls | Pass | Source coverage and a signed iOS build prove the persistent title pill and control row are replaced by one accessible menu button whose actions collapse the menu |
...
The beta 6 source checkpoint publishes the Mac parity release. iPhone build 25
is its focused Remote Assist follow-up. Physical certification remains separate
from channel publication and must be recorded against exact Mac build 23 and
iPhone build 25.
...
| Fresh TestFlight install | Pending | Build 25 launches to the subscription or pairing surface without stale workspace flash |
...
| Remote Assist | Pending | Landscape, pointer, scrolling, compact one-icon controls, Exit, keyboard, Enter, paste, copy, zoom reset, timeout, and retry work |
