...
| Mac Swift unit suite | Pass | `swift test --package-path native/macos`: 52 tests passed |
| Native runtime bundle and ownership | Pass | Release coverage verifies the Mac app embeds and owns runtime `0.7.0-beta.10` |
| Mac signed package | Pass | The local beta 10 DMG and ZIP checksums pass; Mac build 28 is installed on the paired Mac |
| Mac notarization and stapling | Pass | The app and DMG are notarized and stapled; Gatekeeper accepts installed build 28 |
...
| Candidate readiness identity | Pass | Snapshot logic requires native-private runtime beta 10, TestFlight build 32, installed iPhone build 32, and installed Mac build 28 before physical certification can become ready |
...
| Remote Terminal tab switcher | Pass | Shared protocol, Mac, and iPhone tests cover bounded catalog requests, opaque IDs, explicit Back and Refresh controls, exact tab focus, catalog revision checks, busy/selected state, locked-Mac rejection, timeouts, and Automation permission failures |
...
corrected Read Aloud playback. The installed Mac lane is `0.7.0 (28)` with
embedded runtime `0.7.0-beta.10`. Physical certification remains separate from artifact and
channel verification and must be recorded against exact Mac build 28 and iPhone
build 32.
...
| Mac artifact and installation | Pass | Beta 10 build 28 is signed, notarized, stapled, checksum-verified, Gatekeeper-approved, and installed |
...
| Check | State | Acceptance |
| --- | --- | --- |
| Native Mac workspace parity | Pending visual read-back | Build 28 is installed; fresh packaged-app UI inspection remains a physical gate |
...
iPhone and Remote Assist rows pass against installed Mac build 28 and TestFlight
