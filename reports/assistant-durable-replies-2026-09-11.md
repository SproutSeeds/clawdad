...
- **Production relay bundle:** all 13 notification tests passed against the actual deployed bundle with only the Assistant additions applied (`cloud-live-push-test.log`). This covers original-account rejection, exact-reply payloads, durable retries, duplicate event IDs, notification opt-out, Apple error handling, credential-free diagnostics and authenticated routes. The active editor source was copied back after deployment and matched the tested candidate byte for byte.
...
iPhone **89** has a successful signed archive and upload to App Store Connect (`ios89-archive.log`, `ios89-upload.log`). Apple processed it as **VALID**, and it is assigned to **ClawDad Internal**, status **IN_BETA_TESTING** (`testflight89-release.json`, build ID `b64f37ea-df2d-4355-9be6-61beadd1424a`). After relay deployment, the temporary pending-activation sentence was removed from TestFlight instructions and the final text was read back successfully; the build group was not reassigned or renotified. The upload reported the existing vendor WebRTC dSYM warning; Apple accepted the package. The physical iPhone's installed build was not changed remotely or claimed verified.

**Relay deployed and verified:** after Cody explicitly approved the existing Google account sign-in, `clawdad-cloud` advanced from `c332026c` to active/latest version **`9f1faa09`**, verified at `2026-09-12T01:45:04Z`. The exact prior live source was preserved before editing (SHA-256 `a9f8e16a353e90d231d0a1af33e075d2a3d664e87894b27e74690c00a7ee594d`). The tested and deployed bundle SHA-256 is `77f28f40c9f52931ebea78459ae2f994a11d4f1085cde9d98cfe9559dc96f1d9`. The scoped diff adds only Assistant notification normalization, the original-owner check, payload and event dispatch. Existing production differences and bindings were preserved. Evidence: `cloud-patch-proof.json`, `cloud-live-before.js`, `cloud-live-after.js`, `cloud-active-state.txt` and `cloud-deployment-verification.json`.

Both production `/healthz` and `/privacy` returned HTTP 200. Authenticated notification status reported configured APNs, one registered device and zero pending relay events. Its ten recent existing notification receipts recorded Apple HTTP acceptance. The installed Assistant outbox contained no new completion events, so these existing receipts are not claimed as live Assistant-specific Apple acceptance. No synthetic reply pointing to a nonexistent real conversation was pushed to Cody's phone. A genuine new Assistant reply notification and physical exact-reply/audible-readback test remain device checks. The Assistant notification capability is deployed and ready.
