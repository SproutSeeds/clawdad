...
**Released:** signed/notarized Mac 133 is installed and healthy. iPhone 88 is available in ClawDad Internal TestFlight (`VALID`, `IN_BETA_TESTING`). Mac 132 completed the exact close/restore and normal-window sizing cycle; Mac 133 adds the final scripting-census repair described below. A final QA-window teardown remains pending because incoming input activity stopped the guarded close. Intermediate failures are retained as evidence, not successful closure proof.
...
The original imported setup is selected again without restoring or changing any membership. Two clearly named QA snapshots and the disposable three-tab QA window remain intact for cleanup during a coordinated idle interval. `archive-qa-snapshots.py` is prepared to archive only those exact QA records after verified closure; it has not been run. Real project windows, drafts, supervisors and history remain intact.
...
Final installed-133 whole-window teardown still needs an uninterrupted idle interval. Save/close/recreate passed on 132; the additional census change, installed-133 Save and control-priority stops are separately verified above. Physical iPhone swipe actions, VoiceOver/context-menu interaction and slow/reconnecting phone behavior require hands-on confirmation. Both connected Mac displays were verified with actual native bounds and screenshots. No Mac power-cycle, real Terminal-window closure or research-task resubmission was performed. Physical power-loss recovery remains untested; automated tests exercise interrupted writes and restore boundaries.
