...
Status: scoped implementation and automated checks passed; release coordination and final device/build checks in progress. Owner: speech/settings Codex lane, September 12, 2026.
...
Terminal's changes were independently committed as `564c53d`, including its exact shared runtime/MCP hunks; the remaining diff in those two files is speech-only. Preserve its Mac 136 release ownership. The release/storage, plugin workflow, branding and infrastructure/site buckets listed at the top are unrelated and remain for their owners. Generated candidate evidence is in the canonical ignored candidate directory. No real Terminal tab, draft or queue was changed by this lane. Final ORP hygiene must report zero unclassified paths; keep unrelated dirty buckets intentionally dirty rather than sweep them into the speech commit.
