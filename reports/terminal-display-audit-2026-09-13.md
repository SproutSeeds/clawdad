...
Cody subsequently confirmed that the shrinking happened while switching tabs, and that it had stopped occurring by his follow-up. This establishes tab switching as the reported trigger, while the intermittent resize initiator remains unconfirmed. The report does not establish whether those switches used Terminal directly, Remote Assist, or an Assistant action. The next useful evidence is a before/after physical-window frame and exact operation/selected identity at an actual shrink. Avoid claiming the title conflict also explains the height change without this evidence.
...
2. Include the intermittent tab-switch resize in the potential patch. First compare physical-window frames around each supported selection path using disposable fixtures; if a path restores stale per-tab geometry, correct that path to preserve the current normal-window rectangle. Distinguish an explicit user resize and a changed display from unintended geometry restoration. Apply “fill available display” only on authorized restore/fit actions, rather than installing a timer that fights Cody's manual sizing. If the defect remains unreproduced, report the gap and retain focused regression coverage instead of claiming the cause is fixed.
3. Verify with disposable windows that repeated native and Assistant tab changes preserve size, including tabs with different saved grids, Codex/shell transitions and same-directory sessions. Exercise both first visits and repeated switches, since Cody reports that the shrinking stopped after initially occurring. Record actual frames before/after and verify manual resizing still works; retain the unchanged full-screen preference. Run repeated animated-title tests that require stable visible names throughout, rather than eventual correction after one overwrite.
